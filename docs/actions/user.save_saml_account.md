...
- The saml attribute mapping can have a list of 'meeting_mappers' that can be used to assign users meeting related data. (See example below. A full example can be found in the [organization.update.md](organization.update.md))
    ...
    - If 'allow_update' is set to false, the mapper is only used to create a new participant if the user does not already exist in the meeting. If it is not given it defaults to true. This should only be left at true if you are sure that users aren't deleted from meetings. When set to true participants can be locked out from the meeting, in preference to removing them from the meeting.
...
- Values for groups and structure levels can be specified either as a list of strings or as a single comma-separated string.
