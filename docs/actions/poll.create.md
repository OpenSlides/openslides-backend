...
The `live_voting_enabled` could be set for named motion and certain named assignment polls. Assignment polls either need pollmethod `Y` and not `global_yes` and `max_votes_amount` of 1, pollmethod `YNA` or `YN` and only one option or need to be a list poll, to set the option.
