...
-- MODELS_YML_CHECKSUM = '34e1e169e3781274d7240604dad8f60c'
-- Type definitions
DO $$
BEGIN
...
CREATE TABLE IF NOT EXISTS motionT (
    ...
    block_id integer,
    editor_id integer,
    working_group_speaker_id integer,
...
(select array_agg(m1.id) from motionT m1 where m1.editor_id = m.id) as editor_for_motion_ids,
(select array_agg(m1.id) from motionT m1 where m1.working_group_speaker_id = m.id) as working_group_speaker_for_motion_ids,
...
ALTER TABLE motionT ADD FOREIGN KEY(editor_id) REFERENCES meeting_userT(id);
ALTER TABLE motionT ADD FOREIGN KEY(working_group_speaker_id) REFERENCES meeting_userT(id);
...
SQL nt:1t => meeting_user/editor_for_motion_ids:-> motion/editor_id
SQL nt:1t => meeting_user/working_group_speaker_for_motion_ids:-> motion/working_group_speaker_id
...
FIELD 1t:nt => motion/editor_id:-> meeting_user/editor_for_motion_ids
FIELD 1t:nt => motion/working_group_speaker_id:-> meeting_user/working_group_speaker_for_motion_ids
