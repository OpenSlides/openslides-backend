...
class KeycloakTestHelper:
    ...
    def authenticate_user(
        ...
        username: str,
        password: str,
        client_id: str = "openslides-client",
        client_secret: str = "openslides-secret",
    ) -> dict[str, Any]:
        """
        Authenticate a user via the resource owner password credentials grant.

        Returns the full token response dict (access_token, refresh_token,
        session_state, etc.).
        """
        base_url = self.admin_url.split("/admin/")[0]
        token_url = f"{base_url}/realms/{self.realm}/protocol/openid-connect/token"
        ...
        if response.status_code != 200:
            raise RuntimeError(
                f"Failed to authenticate user {username}: "
                f"{response.status_code} - {response.text}"
            )

        return response.json()

    def get_user_sessions(self, keycloak_id: str) -> list[dict[str, Any]]:
        """
        Get active sessions for a Keycloak user via Admin API.

        Returns list of session dicts (each has id, userId, ipAddress, etc.).
        """
        return self._make_request("GET", f"users/{keycloak_id}/sessions") or []
