...
def test_simple(write, finalize, assert_model):
    ...
    finalize("0075_remove_groupless_users")
...
def test_migration_complex(write, finalize, assert_model):
            ...
            collection: {
                ...
                for id_, date in {
                    1: {
                        ...
                        "motion_id": 11,
                        "meeting_user_id": 14,
                        **({} if collection == "motion_supporter" else {"weight": 1}),
                    },
                    2: {
                        "meeting_id": 1,
                        "motion_id": 11,
                        "meeting_user_id": 15,
                        **({} if collection == "motion_supporter" else {"weight": 2}),
                    },
                    3: {
                        "meeting_id": 3,
                        "motion_id": 31,
    ...
    assert_model("motion_supporter/1", {"motion_id": 11, "meeting_id": 1})
    assert_model("motion_supporter/2", {"motion_id": 11, "meeting_id": 1})
    assert_model("motion_supporter/3", {"motion_id": 31, "meeting_id": 3})
    assert_model(
        "motion_supporter/4",
        {
            **collection_to_id_to_data["motion_supporter"][4],
            "meta_deleted": True,
        },
    )
