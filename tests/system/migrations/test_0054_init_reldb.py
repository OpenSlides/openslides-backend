...
TEST_MODULE_PATH = (
    "openslides_backend.migrations.migrations_reldb.0054_init_reldb"
)
...
def test_migration() -> None:
    ...
    with os_conn_pool.connection() as conn:
        with conn.cursor() as cur:
            ...
            for fqid, model in data.items():
                ...
                cur.execute("INSERT INTO models VALUES (%s, %s, false, now());", [fqid, json_blob])
    ...
    with os_conn_pool.connection() as conn:
        with conn.cursor() as cur:
            ...
            cur.execute("SELECT tagged_id FROM gm_organization_tag_tagged_ids_t WHERE organization_tag_id=1 AND tagged_id LIKE 'committee/1';")
            assert cur.fetchone() is not None
            cur.execute("SELECT tagged_id FROM gm_organization_tag_tagged_ids_t WHERE organization_tag_id=1 AND tagged_id LIKE 'meeting/1';")
            assert cur.fetchone() is not None

            # 6.3) n:m relation
            cur.execute("SELECT user_id FROM nm_committee_user_ids_user_t WHERE committee_id=1 ORDER BY user_id;")
            res = cur.fetchall()
            assert res[0]["user_id"] == 1 and res[1]["user_id"] == 2 and res[2]["user_id"] == 3
    # END TEST CASES
# END TEST
