...
class UserSetPasswordActionTest(ScopePermissionsTestMixin, BaseActionTestCase):
    ...
    def test_scope_meeting_no_permission(self) -> None:
        ...
        self.assertIn(
            "You are not allowed to perform action user.set_password. Missing permissions: OrganisationManagementLevel can_manage_users in organisation 1 or CommitteeManagementLevel can_manage in committee 1 or Permission user.can_change_own_password in meeting 1",
    ...
    def test_scope_committee_no_permission(self) -> None:
        ...
        self.assert_status_code(response, 403)
        self.assertIn(
            "You are not allowed to perform action user.set_password. Missing permissions: OrganisationManagementLevel can_manage_users in organisation 1 or CommitteeManagementLevel can_manage in committee 1",
            response.json["message"],
        )
    ...
    def test_scope_organisation_no_permission(self) -> None:
        ...
        self.assert_status_code(response, 403)
        self.assertIn(
            "You are not allowed to perform action user.set_password. Missing permission: OrganisationManagementLevel can_manage_users in organisation 1",
            response.json["message"],
        )
