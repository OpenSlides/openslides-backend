...
class StructureLevelCreateTest(BaseActionTestCase):
    ...
    def test_create_duplicate_name(self) -> None:
        ...
        self.assert_status_code(response, 400)
        self.assertIn('structure_level/2: duplicate key value violates unique constraint "unique_structure_level_meeting_id_name"', response.json["message"])
        self.assertIn('DETAIL:  Key (meeting_id, name)=(1, test) already exists.', response.json["message"])
