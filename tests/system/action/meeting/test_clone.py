...
class MeetingClone(BaseActionTestCase):
    def setUp(self) -> None:
        ...
        self.test_models: dict[str, dict[str, Any]] = {
            "committee/2": {"name": "Receiver", "organization_id": 1},
            "organization_tag/1": {
                "name": "TEST",
                "color": "#eeeeee",
                "organization_id": 1,
    ...
    def set_test_data_with_admin(self):
        self.set_test_data()
    ...
    def test_clone_with_users_min_vote_weight_N1_N(self) -> None:
        ...
        self.set_models({"user/1": {"default_vote_weight": "1.000000"}})
        self.set_test_data_with_admin()
        response = self.request("meeting.clone", {"meeting_id": 1})
        self.assert_status_code(response, 200)
        self.assert_model_exists("meeting/1", {"user_ids": [1]})
    ...
    def test_clone_with_users_min_vote_weight_0X_1(self) -> None:
        ...
        self.set_models({"user/1": {"default_vote_weight": "1.000000"}})
        self.set_test_data_with_admin()
        self.set_models({"meeting_user/1": {"vote_weight": "0.000000"}})
        response = self.request("meeting.clone", {"meeting_id": 1})
        self.assert_status_code(response, 200)
        self.assert_model_exists("meeting/1", {"user_ids": [1]})
        self.assert_model_exists("meeting/2", {"user_ids": [1]})
    ...
    def test_clone_with_users_min_vote_weight_N0_1(self) -> None:
        ...
        self.assert_model_exists(
            "user/1",
            {
                ...
                "committee_ids": [60],
                "organization_id": 1,
                "default_vote_weight": Decimal("0.000000"),
            },
        )
        self.assert_model_exists(
            "meeting_user/2",
    ...
    def test_clone_with_ex_users(self) -> None:
        ...
        response = self.request(
            "meeting.clone",
            {
                "committee_id": 2,
                "meeting_id": 1,
                "admin_ids": [admin_ids_user],
                "user_ids": [user_ids_user],
            },
        )
        self.assert_status_code(response, 200)
        ...
        self.assert_model_exists(
            "group/4",
            {"name": "default group", "meeting_id": 2, "meeting_user_ids": [3, 4, 5]},
        )
        self.assert_model_exists(
            "group/5", {"name": "admin group", "meeting_id": 2, "meeting_user_ids": [6]}
        )
        ...
        self.assert_model_exists(
            "meeting_user/2",
            {"user_id": original_user_ids_user, "meeting_id": 1, "group_ids": [1]},
        )
        self.assert_model_exists(
            "meeting_user/4",
            {"user_id": original_user_ids_user, "meeting_id": 2, "group_ids": [4]},
        )
        ...
        self.assert_model_exists(
            "meeting_user/6",
            {"user_id": admin_ids_user, "meeting_id": 2, "group_ids": [5]},
        )
        ...
        self.assert_model_exists(
            "meeting_user/5",
            {"user_id": user_ids_user, "meeting_id": 2, "group_ids": [4]},
        )
    ...
    def test_clone_user_ids_and_admin_ids(self) -> None:
        self.meeting_data["template_for_organization_id"] = None
        self.set_test_data()
        ...
        response = self.request(
            "meeting.clone",
            {
                "meeting_id": 1,
                "user_ids": [
                    new_default_group_user,
                    new_and_old_default_group_user,
                    new_default_group_old_admin_user,
                ],
                "admin_ids": [new_admin_user],
        ...
        self.assert_model_exists(
            "meeting/2",
            {"template_for_organization_id": None, "user_ids": [2, 3, 4, 5]},
        )
        self.assert_model_exists("group/4", {"meeting_user_ids": [3, 4, 5]})
        self.assert_model_exists("group/5", {"meeting_user_ids": [5, 6]})
        self.assert_model_exists(
            "user/13",
            {
    ...
    def test_clone_new_committee_and_user_with_group(self) -> None:
        ...
        self.assert_status_code(response, 200)
        self.assert_model_exists("meeting/2", {"committee_id": 2, "user_ids": [1, 2]})
        self.assert_model_exists(
            "committee/2",
    ...
    #         {
    #             "template_for_organization_id": 1,
    #             "name": "Template meeting with font",
    #         }
    #     )
    #     self.test_models_with_admin[ONE_ORGANIZATION_FQID].update(
    ...
    #     self.assert_status_code(response, 200)
    #     self.assert_model_exists(
    #         "meeting/3", {"motion_ids": [5, 6], "name": "OpenSlides - Copy"}
    #     )
    #     self.assert_model_exists(
    #         "motion/5",
    #         {
    ...
    #     response = self.request("meeting.clone", {"meeting_id": 1, "committee_id": 2})
    #     self.assert_status_code(response, 400)
    #     assert "Cannot clone locked meeting.", response.json["message"]
