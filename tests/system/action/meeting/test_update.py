...
class MeetingUpdateActionTest(BaseActionTestCase):
    ...
    def basic_test(
        ...
        self.create_meeting(meeting_data={"name": "test_name", **meeting_data})
        if extra_models:
            self.set_models(extra_models)
        response = self.request("meeting.update", {"id": 1, **datapart})
        if check_200:
            self.assert_status_code(response, 200)
        else:
            self.assert_status_code(response, 400)
        self.assert_model_exists("meeting/1", {"name": "test_name", "committee_id": 60})
        return response
    ...
    def test_update_broken_email(self) -> None:
        response = self.basic_test({"users_email_replyto": "broken@@"}, False)
        self.assertEqual(
            "users_email_replyto must be valid email.", response.json["message"]
        )

    def test_update_broken_sender(self) -> None:
        response = self.basic_test({"users_email_sender": "Openslides[Test"}, False)
        self.assertEqual(
    ...
    def test_update_projector_related_fields2(self) -> None:
        self.basic_test(
            datapart={"default_projector_current_los_ids": [2]},
            extra_models={
                "projector/2": {
                    "name": "Projector 2",
                    "meeting_id": 1,
    ...
    def test_update_reference_projector_to_null_error(self) -> None:
        response = self.basic_test({"reference_projector_id": None}, check_200=False)
        self.assertEqual(
    ...
    def test_update_reference_projector_to_not_existing_projector_error(self) -> None:
        response = self.basic_test({"reference_projector_id": 10}, check_200=False)
        self.assertEqual(
    ...
    def test_update_reference_projector_to_internal_projector_error(self) -> None:
        response = self.basic_test(
            datapart={"reference_projector_id": 2},
            check_200=False,
            extra_models={
                "projector/2": {
                    "name": "Projector 2",
    ...
    def test_update_only_one_time_1(self) -> None:
        response = self.basic_test({"start_time": 150000}, check_200=False)
        self.assertEqual(
    ...
    def test_update_only_one_time_2(self) -> None:
        response = self.basic_test({"end_time": 156000}, check_200=False)
        self.assertEqual(
            "Only one of start_time and end_time is not allowed.",
            response.json["message"],
        )
    ...
    def test_update_check_jitsi_domain_2(self) -> None:
        response = self.basic_test({"jitsi_domain": "test.com/"}, check_200=False)
        self.assertEqual(
            "It is not allowed to end jitsi_domain with '/'.", response.json["message"]
        )
