...
from openslides_backend.presenter.export_meeting import export_meeting
...
class MeetingImport(BaseActionTestCase):
    ...
    def test_import_with_export_meeting(self) -> None:
        self.set_models(
            {
                "meeting/1": {
                    "name": "Test",
        ...
        with self.datastore.get_database_context():
            export = export_meeting(self.datastore, 1)
        export["user"] = []
        response = self.request(
            "meeting.import", {"committee_id": 1, "meeting": export}
        )
        self.assert_status_code(response, 200)
