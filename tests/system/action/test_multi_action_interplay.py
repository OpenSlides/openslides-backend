...
class TestMultiActionInterplay(BaseActionTestCase):
    ...
    case_actions: list[list[tuple[str, list[dict[str,Any]],ActionResults|None, dict[str,dict[str,Any]|None]]]] = [
        [
            ("organization_tag.create", [{
                ...
                "color": "#789abc",
                "organization_id": 1
            }],[{"id": id_} for id_ in [1,2]],{"organization_tag/1":{
                "name": "Never",
                "color": "#123456",
                "organization_id": 1
            }, "organization_tag/2":{
            ...
            }],[None],{"meeting_user/1": {"user_id": 1, "meeting_id": 1, "group_ids": [2]}}), # legacy for now
            ("theme.create", [
                {
                    "name": "Never",
                    "primary_500": "#aaaaaa",
        ...
        [
            ("organization_tag.delete", [{"id":id_} for id_ in [1,2]],None,{"organization_tag/2": None}),
            ("motion.create", [{
                "meeting_id": 1,
                "title": "Never gonna",
                "text": "SAYYYYYYY goodbye."
            }],[{"id": 2, "sequential_number":2}],{"motion/2":{
                "meeting_id": 1,
                "title": "Never gonna",
                "text": "SAYYYYYYY goodbye."
            }, "list_of_speakers/2": {"content_object_id": "motion/2"}}), # legacy for now
            ("motion_workflow.update", [
                {
                    "id": 2,
                    "name": "Never"
                }
            ],[None],{"motion_workflow/2":{"name": "Never"}}),
            ("topic.create", [
                {
                    "title": "gonna",
                    "meeting_id": 1
            ...
            ],[None, None],{"theme/2": {"name": "tell"},"theme/3": {"name": "a"}}),
            ("user.create", [{
                "username": "lie",
                "email": "and.hurt@you.de"
            }],[{"id": 2}],{"user/2":{
                "username": "lie",
                "email": "and.hurt@you.de"
            }}) # legacy for now
        ...
        [
            ...
            ("organization.delete_history_information", [{"id":1}],None,{
                f"history_position/{id_}": None for id_ in [1,2,3,4,5]
            }),
            ("meeting.update", [{"id": id_, "time_zone": "Europe/Berlin"} for id_ in [1,2]],[None,None],{
    ...
    def test_test_validity(self) -> None:
        ...
        assert has_mixed_tests, "This test order needs to be switched up or this testclass is pointless"
        assert count_legacy > 0, f"No legacy actions left."
    ...
    def test_parse_actions_normal(self) -> None:
        for i, case_iteration in enumerate(self.case_actions):
            ...
            assert response.json["results"] == all_results, f"Guessed results wrong for iter {i}"
