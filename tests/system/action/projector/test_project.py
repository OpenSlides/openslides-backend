...
class ProjectorProject(BaseActionTestCase):
    ...
    def test_meeting_as_content_object_ok(self) -> None:
        ...
        self.assert_model_exists(
            "projector/65", {"current_projection_ids": [112], "meeting_id": 1}
        )
        self.assert_model_exists(
            "projection/112",
            {
    ...
    def test_meeting_as_content_object_error(self) -> None:
        response = self.request(
            "projector.project",
            {
                "ids": [65],
                "content_object_id": "meeting/2",
                "stable": True,
                "type": "current-list-of-speakers",
            },
        )
        self.assert_status_code(response, 400)
        self.assertIn(
            "The following models do not belong to meeting 2: [FullQualifiedId('projector/65')]",
