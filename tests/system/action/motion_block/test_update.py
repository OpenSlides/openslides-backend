...
class MotionBlockActionTest(BaseActionTestCase):
    def test_update_correct(self) -> None:
        self.create_model(get_fqid("motion_block/111"), {"title": "title_srtgb123"})
        response = self.client.post(
            "/",
            json=[
                {
                    "action": "motion_block.update",
                    "data": [{"id": 111, "title": "title_Xcdfgee"}],
                }
            ],
        )
        self.assertEqual(response.status_code, 200)
        self.assert_model_exists(get_fqid("motion_block/111"))
        model = self.datastore.get(get_fqid("motion_block/111"))
