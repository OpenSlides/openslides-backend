...
class CommitteeUpdateActionTest(BaseActionTestCase):
    ...
    def test_update_correctly(self) -> None:
        ...
        response = self.request("gender.update", {"id": self.gender_id, "name": new_name})
    ...
    def test_update_empty(self) -> None:
        ...
        self.assert_status_code(response, 400)
        self.assertIn("data must contain ['id', 'name'] properties", response.json["message"])
        model = self.get_model(self.gender_fqid)
        self.assertEqual(model.get("name"), self.gender_name)
    ...
    def test_update_no_permission(self) -> None:
        ...
        response = self.request("gender.update", {"id": self.gender_id, "name": "testy"})
        self.assert_status_code(response, 403)
        assert (
            "Missing OrganizationManagementLevel: can_manage_organization"
            in response.json["message"]
        )
        self.assert_model_exists(self.gender_fqid, {"id": self.gender_id, "name": self.gender_name},)
    ...
    def test_update_permission(self) -> None:
        ...
        response = self.request("gender.update", {"id": self.gender_id, "name": "testy"})
        self.assert_status_code(response, 200)
        self.assert_model_exists(self.gender_fqid, {"id": self.gender_id, "name": "testy"})
