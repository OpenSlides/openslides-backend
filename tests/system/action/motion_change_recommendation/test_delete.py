...
class MotionChangeRecommendationActionTest(BaseActionTestCase):
    def test_delete_correct(self) -> None:
        self.create_model(
            get_fqid("motion_change_recommendation/111"), {"name": "name_srtgb123"}
        )
        ...
        with self.assertRaises(DatabaseException):
            self.datastore.get(get_fqid("motion_change_recommendation/111"))

    def test_delete_wrong_id(self) -> None:
        self.create_model(
            get_fqid("motion_change_recommendation/112"), {"name": "name_srtgb123"}
        )
        with self.assertRaises(DatabaseException):
            self.client.post(
                "/",
        ...
        self.assert_model_exists(get_fqid("motion_change_recommendation/112"))
