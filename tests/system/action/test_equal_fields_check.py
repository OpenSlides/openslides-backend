...
class TestEqualFieldsCheck(BaseActionTestCase):
    def test_simple_pass(self) -> None:
        ...
        response = self.client.post(
            ...
            json=[
                {
                    "action": "fake_model_ef_b.create",
                    "data": [{"reference_field": 1, "c_id": 1}],
                }
            ],
        )
    ...
    def test_simple_fail(self) -> None:
        self.create_model("fake_model_ef_a/1", {})
        self.create_model("fake_model_ef_a/2", {"c_id": 1})
        self.create_model("fake_model_ef_c/1", {"reference_field": 2})
        response = self.client.post(
            "/",
            json=[
                {
    ...
    def test_simple_update_pass(self) -> None:
        ...
        response = self.client.post(
            "/",
            json=[{"action": "fake_model_ef_b.update", "data": [{"id": 1, "c_id": 2}]}],
        )
        self.assert_status_code(response, 200)
    ...
    def test_list_pass(self) -> None:
        ...
        response = self.client.post(
            ...
            json=[
                {
                    "action": "fake_model_ef_b.create",
                    "data": [{"reference_field": 1, "c_ids": [1]}],
                }
            ],
        )
        self.assert_status_code(response, 200)
