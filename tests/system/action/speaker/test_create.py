...
class SpeakerCreateActionTest(BaseActionTestCase):
    ...
    def test_create_add_2_speakers_in_2_actions(self) -> None:
        ...
        self.assert_status_code(response, 400)
        self.assertIn(
            "Datastore service sends HTTP 400. The following locks were broken: 'user/8/speaker_$7844_ids', 'user/8/speaker_$_ids', 'user/8/speaker_$_ids', 'list_of_speakers/23/speaker_ids', 'meeting/7844/speaker_ids', 'speaker/list_of_speakers_id', 'speaker/weight'",
            response.json["message"],
        )
