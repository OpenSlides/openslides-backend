...
def test_delete_1_1_not_null_success(
    db_connection: Connection[rows.DictRow],
) -> None:
    create_models_for_1_1_tests()
    with get_new_os_conn() as conn:
...
def test_delete_1_n_not_null_success(
    db_connection: Connection[rows.DictRow],
) -> None:
    data: list[dict[str, Any]] = get_group_base_data()
    create_models(data)
    ...
    with get_new_os_conn() as conn:
        ...
        conn.commit()
    assert_no_model("group/1")
    assert_no_model("committee/1")
    assert_no_model("projector/1")
    assert_no_model("motion_state/1")
    assert_no_model("motion_workflow/1")
    assert_no_model("meeting/1")
...
def test_delete_n_m_not_null_success(
    db_connection: Connection[rows.DictRow],
) -> None:
    create_models_for_n_m_tests()
    with get_new_os_conn() as conn:
