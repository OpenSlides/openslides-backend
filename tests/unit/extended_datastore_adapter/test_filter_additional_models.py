...
def test_sql_to_filter_code_simple() -> None:
    ...
    with patch("openslides_backend.services.datastore.extended_adapter.eval") as mock:
        ds._filter_additional_models(MagicMock(), FilterOperator("test", "=", 1))
    assert (
        mock.call_args[0][0]
        == "{model['id']: model for fqid, model in self.additional_relation_models.items() if fqid.collection == collection and (model.get(\"test\") == 1)}"
    )
...
def test_sql_to_filter_code_complex() -> None:
    ...
    with patch("openslides_backend.services.datastore.extended_adapter.eval") as mock:
        ds._filter_additional_models(MagicMock(), _filter)
    assert (
        mock.call_args[0][0]
        == '{model[\'id\']: model for fqid, model in self.additional_relation_models.items() if fqid.collection == collection and ((model.get("test") == 1) or ((model.get("test") == 1) and (not (model.get("test") == 1))))}'
    )
