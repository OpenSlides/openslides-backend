...
from ...shared.env import DEV_PASSWORD, is_truthy
...
class ActionView(BaseView):
    ...
    def action_route(self, request: Request) -> Tuple[ResponseBody, Optional[str]]:
        ...
        # Handle request.
        handler = ActionHandler(self.config, self.services, self.logging)
    ...
    def migrations_route(self, request: Request) -> Tuple[ResponseBody, Optional[str]]:
        ...
        self.check_internal_auth_password(request)
        handler = MigrationHandler(self.config, self.services, self.logging)
    ...
    def check_internal_auth_password(self, request: Request) -> None:
        ...
        if is_truthy(self.config["OPENSLIDES_DEVELOPMENT"]):
            secret_password = DEV_PASSWORD
        else:
            filename = self.config["INTERNAL_AUTH_PASSWORD_FILE"]
            if not filename:
