...
migration_model_registry: dict[str, type["MigrationModel"]] = dict()
...
class MigrationModelMetaClass(type):
    def __new__(metaclass, class_name, class_parents, class_attributes):  # type: ignore
        ...
        if getattr(new_class, "collection", None):
            for attr_name in class_attributes:
                attr = getattr(new_class, attr_name)
                if isinstance(attr, fields.Field):
                    attr.own_field_name = attr_name
            migration_model_registry[new_class.collection] = new_class
        return new_class


class MigrationModel(metaclass=MigrationModelMetaClass):
    collection: str
...
class ActionWorker(MigrationModel):
...
class AgendaItem(MigrationModel, AgendaItemModelMixin):
...
class Assignment(MigrationModel):
...
class AssignmentCandidate(MigrationModel):
...
class ChatGroup(MigrationModel):
...
class ChatMessage(MigrationModel):
...
class Committee(MigrationModel):
...
class HistoryEntry(MigrationModel):
...
class HistoryPosition(MigrationModel):
...
class ImportPreview(MigrationModel):
...
class ListOfSpeakers(MigrationModel):
...
class Mediafile(MigrationModel):
...
class Meeting(MigrationModel, MeetingModelMixin):
    collection = "meeting"
...
class MeetingMediafile(MigrationModel):
...
class MeetingUser(MigrationModel):
...
class MotionBlock(MigrationModel):
...
class MotionCategory(MigrationModel):
...
class MotionChangeRecommendation(MigrationModel):
...
class MotionComment(MigrationModel):
...
class MotionCommentSection(MigrationModel):
...
class MotionEditor(MigrationModel):
...
class MotionState(MigrationModel):
...
class MotionSubmitter(MigrationModel):
...
class MotionSupporter(MigrationModel):
...
class MotionWorkflow(MigrationModel):
...
class MotionWorkingGroupSpeaker(MigrationModel):
...
class Option(MigrationModel):
    ...
    poll_id = fields.RelationField(to={"poll": "option_ids"}, constant=True)
    used_as_global_option_in_poll_id = fields.RelationField(
        to={"poll": "global_option_id"}, is_view_field=True, constant=True
    )
    vote_ids = fields.RelationListField(
        to={"vote": "option_id"},
        on_delete=fields.OnDelete.CASCADE,
...
class Organization(MigrationModel):
...
class OrganizationTag(MigrationModel):
...
class PersonalNote(MigrationModel):
...
class PointOfOrderCategory(MigrationModel):
...
class Poll(MigrationModel, PollModelMixin):
    collection = "poll"
    verbose_name = "poll"
...
class PollCandidate(MigrationModel):
    collection = "poll_candidate"
    verbose_name = "poll candidate"

    id = fields.IntegerField(required=True, constant=True)
    poll_candidate_list_id = fields.RelationField(
        to={"poll_candidate_list": "poll_candidate_ids"}, required=True, constant=True
    )
...
class PollCandidateList(MigrationModel):
...
class Projection(MigrationModel):
...
class Projector(MigrationModel):
...
class ProjectorCountdown(MigrationModel):
...
class ProjectorMessage(MigrationModel):
...
class StructureLevel(MigrationModel):
...
class StructureLevelListOfSpeakers(MigrationModel):
