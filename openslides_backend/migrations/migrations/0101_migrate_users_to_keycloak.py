...
2. For each local user (no keycloak_id, no saml_id):
   ...
   - For SHA512 legacy hashes: cannot be imported, user needs password reset
   - Updates user_t with keycloak_id and sets can_change_own_password=TRUE
"""
...
def _build_argon2_credential(password_hash: str) -> dict[str, Any] | None:
    ...
    # additionalParameters values must be arrays of strings (MultivaluedHashMap)
    # Version must be in Keycloak format: "19" -> "1.3", "16" -> "1.0"
    version_map = {"19": "1.3", "16": "1.0"}
    kc_version = version_map.get(parsed["version"], parsed["version"])

    # Compute hashLength from the base64-encoded hash
    import base64

    try:
        hash_length = len(base64.b64decode(parsed["hash"] + "=="))
    except Exception:
        hash_length = 32  # Default Argon2 hash length

    credential_data = {
        "hashIterations": parsed["iterations"],
        "algorithm": "argon2",
        "additionalParameters": {
            "hashLength": [str(hash_length)],
            "type": [parsed["type"].replace("argon2", "")],  # ["id"], ["i"], or ["d"]
            "version": [kc_version],
            "memory": [str(parsed["memory"])],
            "parallelism": [str(parsed["parallelism"])],
        },
    }
...
def data_manipulation(curs: Cursor[DictRow]) -> None:
    ...
    4. For each user:
       ...
       - Imports Argon2 password hash directly via Keycloak credential API
       - For SHA512/no password: user will need password reset
       - Updates user_t with keycloak_id, sets can_change_own_password=TRUE
    ...
    for user in users:
        ...
        # Strategy: Prefer real password hash (Argon2), then default_password (plaintext)
        credentials: list[dict[str, Any]] = []

        if password_hash and _is_argon2_hash(password_hash):
            # Import Argon2 hash directly into Keycloak
            argon2_credential = _build_argon2_credential(password_hash)
            if argon2_credential:
        ...
        if keycloak_id:
            ...
            curs.execute(
                """
                UPDATE user_t
                SET keycloak_id = %s, can_change_own_password = TRUE
                WHERE id = %s
                """,
                (keycloak_id, user_id),
            )
