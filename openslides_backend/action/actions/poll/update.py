...
class PollUpdateAction(
    ...
    def update_instance(self, instance: dict[str, Any]) -> dict[str, Any]:
        ...
        content_object_id = self.datastore.get(fqid_from_collection_and_id("option", option_ids[0]), ["content_object_id"]).get("content_object_id") if length_for_check else None
        is_list_poll = collection_from_fqid(content_object_id) == "poll_candidate_list" if content_object_id else False
        if instance.get("live_voting_enabled") and not (
            ...
            and (
                ...
                or (
                    ...
                    or (
                        (
                            pollmethod_for_check == "YNA"
                            or pollmethod_for_check == "YN"
                        )
