...
class UserDelete(UserScopeMixin, DeleteAction):
    ...
    def get_updated_instances(self, action_data: ActionData) -> ActionData:
        self.check_meeting_admin_integrity([user_id for date in action_data if (user_id:= date.get("id"))])
    ...
    def check_meeting_admin_integrity(self, delete_data: list[int]=[]) -> None:
        ...
        users = self.datastore.get_many([GetManyRequest("user", delete_data, ["meeting_ids", "meeting_user_ids"])])["user"]
        ...
        if len(meeting_ids_to_user_ids):
            meetings = self.datastore.get_many([GetManyRequest("meeting", list(meeting_ids_to_user_ids.keys()), ["admin_group_id", "template_for_organization_id"])])["meeting"]
            ...
            if not len(meeting_ids_to_user_ids):
                return
            meeting_users = self.datastore.filter("meeting_user", Or(*[FilterOperator("user_id", "=", user_id) for user_id in delete_data]), ["group_ids", "user_id"])
            ...
            for group_data in groups.values():
                if group_data.get("meeting_user_ids") and not any(m_user_id not in meeting_users for m_user_id in group_data.get("meeting_user_ids", [])):
