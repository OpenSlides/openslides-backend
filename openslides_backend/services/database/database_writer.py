...
class DatabaseWriter(SqlQueryHelper):
    ...
    def execute_sql(
        ...
        try:
            ...
            column = e.args[0].split('"')[1]  # Only this is not covered. and at this stage I want to add a helper method
            raise InvalidFormat(
                f"Field '{column}' does not exist in collection '{collection}': {e}"
            )
        except UndefinedTable as e:
            table = e.args[0].split('"')[1]  # Only this and next are not covered. How?
            ...
            column = e.args[0].split('"')[1]  # Why only this line but not anything after?
            raise InvalidFormat(
