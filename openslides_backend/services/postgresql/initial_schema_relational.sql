...
CREATE TYPE enum_ballot_paper_selection AS ENUM (
    'NUMBER_OF_DELEGATES', 'NUMBER_OF_ALL_PARTICIPANTS', 'CUSTOM_NUMBER'
);

CREATE TYPE enum_poll_backends AS ENUM ('long', 'fast');

CREATE TYPE enum_onehundred_percent_bases AS ENUM (
    'Y',
    'YN',
    'YNA',
    'N',
...
CREATE TYPE enum_group_permissions AS ENUM (
...
CREATE TYPE enum_import_preview_name AS ENUM (
    'account', 'participant', 'topic', 'committee', 'motion'
);

CREATE TYPE enum_import_preview_state AS ENUM ('warning', 'error', 'done');

CREATE TYPE enum_meeting_applause_type AS ENUM (
    'applause-type-bar', 'applause-type-particles'
);

CREATE TYPE enum_meeting_export_csv_encoding AS ENUM ('utf-8', 'iso-8859-15');

CREATE TYPE enum_meeting_export_pdf_pagenumber_alignment AS ENUM (
    'left', 'right', 'center'
);
...
CREATE TYPE enum_meeting_agenda_item_creation AS ENUM (
    'always', 'never', 'default_yes', 'default_no'
);

CREATE TYPE enum_meeting_agenda_new_items_default_visibility AS ENUM (
    'common', 'internal', 'hidden'
);

CREATE TYPE enum_meeting_motions_default_line_numbering AS ENUM (
    'outside', 'inline', 'none'
);

CREATE TYPE enum_meeting_motions_recommendation_text_mode AS ENUM (
    'original', 'changed', 'diff', 'agreed'
);

CREATE TYPE enum_meeting_motions_default_sorting AS ENUM ('number', 'weight');

CREATE TYPE enum_meeting_motions_number_type AS ENUM (
    'per_category', 'serially_numbered', 'manually'
);

CREATE TYPE enum_meeting_motions_amendments_text_mode AS ENUM (
    'freestyle', 'fulltext', 'paragraph'
);

CREATE TYPE enum_meeting_motion_poll_projection_name_order_first AS ENUM (
    'first_name', 'last_name'
);

CREATE TYPE enum_meeting_users_pdf_wlan_encryption AS ENUM (
    '', 'WEP', 'WPA', 'nopass'
);

CREATE TYPE enum_motion_change_recommendation_type AS ENUM (
    'replacement', 'insertion', 'deletion', 'other'
);

CREATE TYPE enum_motion_state_css_class AS ENUM (
    'grey', 'red', 'green', 'lightblue', 'yellow'
);

CREATE TYPE enum_motion_state_restrictions AS ENUM (
    'motion.can_see_internal',
    'motion.can_manage_metadata',
    'motion.can_manage',
    'is_submitter'
...
CREATE TYPE enum_motion_state_merge_amendment_into_final AS ENUM (
    'do_not_merge', 'undefined', 'do_merge'
);

CREATE TYPE enum_poll_type AS ENUM (
    'analog', 'named', 'pseudoanonymous', 'cryptographic'
);

CREATE TYPE enum_poll_pollmethod AS ENUM ('Y', 'YN', 'YNA', 'N');

CREATE TYPE enum_poll_state AS ENUM (
    'created', 'started', 'finished', 'published'
);

CREATE TYPE enum_speaker_speech_state AS ENUM (
    'contribution', 'pro', 'contra', 'intervention', 'interposed_question'
);

CREATE TYPE enum_user_organization_management_level AS ENUM (
    'superadmin', 'can_manage_organization', 'can_manage_users'
);


-- Function and meta table definitions

-- included in standard postgres-installations, check for alpine
...
CREATE OR REPLACE FUNCTION raise_equality_exception_conditionally(
    check_column text,
    ref_column text,
    own_collection text,
    own_id integer,
...
CREATE TABLE agenda_item_t (
    ...
    content_object_id_motion_id integer
        ...
        CONSTRAINT generated_always_as_agenda_item_content_object_id_motion_id GENERATED ALWAYS AS (
            CASE
                WHEN
                    split_part(content_object_id, '/', 1) = 'motion'
                    THEN cast(split_part(content_object_id, '/', 2) AS integer)
            END
        ) STORED,
    content_object_id_motion_block_id integer
        CONSTRAINT unique_agenda_item_content_object_id_motion_block_id UNIQUE
        CONSTRAINT generated_always_as_agenda_item_content_object_id_motion6edbb3a GENERATED ALWAYS AS (
            CASE
                WHEN
                    split_part(content_object_id, '/', 1) = 'motion_block'
                    THEN cast(split_part(content_object_id, '/', 2) AS integer)
            END
        ) STORED,
    content_object_id_assignment_id integer
        CONSTRAINT unique_agenda_item_content_object_id_assignment_id UNIQUE
        CONSTRAINT generated_always_as_agenda_item_content_object_id_assignment_id GENERATED ALWAYS AS (
            CASE
                WHEN
                    split_part(content_object_id, '/', 1) = 'assignment'
                    THEN cast(split_part(content_object_id, '/', 2) AS integer)
            END
        ) STORED,
    content_object_id_topic_id integer
        CONSTRAINT unique_agenda_item_content_object_id_topic_id UNIQUE
        CONSTRAINT generated_always_as_agenda_item_content_object_id_topic_id GENERATED ALWAYS AS (
            CASE
                WHEN
                    split_part(content_object_id, '/', 1) = 'topic'
                    THEN cast(split_part(content_object_id, '/', 2) AS integer)
            END
        ) STORED,
    CONSTRAINT valid_agenda_item_content_object_id_part1
        CHECK (split_part(content_object_id, '/', 1) IN ('motion', 'motion_block', 'assignment', 'topic')),
    parent_id integer,
    meeting_id integer
...
CREATE TABLE assignment_t (
    ...
    CONSTRAINT unique_assignment_sequential_number_meeting_id
        UNIQUE (sequential_number, meeting_id),
    meeting_id integer
...
CREATE TABLE group_t (
    ...
    CONSTRAINT unique_group_meeting_id_external_id
...
CREATE TABLE history_entry_t (
    ...
    model_id_user_id integer
        CONSTRAINT generated_always_as_history_entry_model_id_user_id GENERATED ALWAYS AS (
            CASE
                WHEN
                    split_part(model_id, '/', 1) = 'user'
                    THEN cast(split_part(model_id, '/', 2) AS integer)
            END
        ) STORED,
    model_id_motion_id integer
        CONSTRAINT generated_always_as_history_entry_model_id_motion_id GENERATED ALWAYS AS (
            CASE
                WHEN
                    split_part(model_id, '/', 1) = 'motion'
                    THEN cast(split_part(model_id, '/', 2) AS integer)
            END
        ) STORED,
    model_id_assignment_id integer
        CONSTRAINT generated_always_as_history_entry_model_id_assignment_id GENERATED ALWAYS AS (
            CASE
                WHEN
                    split_part(model_id, '/', 1) = 'assignment'
                    THEN cast(split_part(model_id, '/', 2) AS integer)
            END
        ) STORED,
    CONSTRAINT valid_history_entry_model_id_part1
        CHECK (split_part(model_id, '/', 1) IN ('user', 'motion', 'assignment')),
    position_id integer
...
CREATE TABLE list_of_speakers_t (
    ...
    CONSTRAINT unique_list_of_speakers_sequential_number_meeting_id
        UNIQUE (sequential_number, meeting_id),
    ...
    content_object_id_motion_id integer
        ...
        CONSTRAINT generated_always_as_list_of_speakers_content_object_id_m4822372 GENERATED ALWAYS AS (
            CASE
                WHEN
                    split_part(content_object_id, '/', 1) = 'motion'
                    THEN cast(split_part(content_object_id, '/', 2) AS integer)
    ...
    content_object_id_motion_block_id integer
        ...
        CONSTRAINT generated_always_as_list_of_speakers_content_object_id_m50027ab GENERATED ALWAYS AS (
            CASE
                WHEN
                    split_part(content_object_id, '/', 1) = 'motion_block'
                    THEN cast(split_part(content_object_id, '/', 2) AS integer)
    ...
    content_object_id_assignment_id integer
        ...
        CONSTRAINT generated_always_as_list_of_speakers_content_object_id_abe56f76 GENERATED ALWAYS AS (
            CASE
                WHEN
                    split_part(content_object_id, '/', 1) = 'assignment'
                    THEN cast(split_part(content_object_id, '/', 2) AS integer)
    ...
    content_object_id_topic_id integer
        ...
        CONSTRAINT generated_always_as_list_of_speakers_content_object_id_topic_id GENERATED ALWAYS AS (
            CASE
                WHEN
                    split_part(content_object_id, '/', 1) = 'topic'
                    THEN cast(split_part(content_object_id, '/', 2) AS integer)
    ...
    content_object_id_meeting_mediafile_id integer
        ...
        CONSTRAINT generated_always_as_list_of_speakers_content_object_id_m07d8b8d GENERATED ALWAYS AS (
            CASE
                WHEN
                    split_part(content_object_id, '/', 1) = 'meeting_mediafile'
                    THEN cast(split_part(content_object_id, '/', 2) AS integer)
            END
        ) STORED,
    CONSTRAINT valid_list_of_speakers_content_object_id_part1
        CHECK (split_part(content_object_id, '/', 1) IN ('motion', 'motion_block', 'assignment', 'topic', 'meeting_mediafile')),
    meeting_id integer
...
CREATE TABLE mediafile_t (
    ...
    owner_id_meeting_id integer
        CONSTRAINT generated_always_as_mediafile_owner_id_meeting_id GENERATED ALWAYS AS (
            CASE
                WHEN
                    split_part(owner_id, '/', 1) = 'meeting'
                    THEN cast(split_part(owner_id, '/', 2) AS integer)
            END
        ) STORED,
    owner_id_organization_id integer
        CONSTRAINT generated_always_as_mediafile_owner_id_organization_id GENERATED ALWAYS AS (
            CASE
                WHEN
                    split_part(owner_id, '/', 1) = 'organization'
                    THEN cast(split_part(owner_id, '/', 2) AS integer)
            END
        ) STORED,
    CONSTRAINT valid_mediafile_owner_id_part1
        CHECK (split_part(owner_id, '/', 1) IN ('meeting', 'organization')),
    CONSTRAINT unique_mediafile_title_parent_id_owner_id
        UNIQUE NULLS NOT DISTINCT (title, parent_id, owner_id)
);
...
CREATE TABLE meeting_t (
    ...
    projector_countdown_warning_time integer
        ...
        CONSTRAINT minimum_meeting_projector_countdown_warning_time CHECK (
    ...
    export_pdf_fontsize integer
        CONSTRAINT minimum_meeting_export_pdf_fontsize CHECK (
            export_pdf_fontsize >= 10
        )
        CONSTRAINT maximum_meeting_export_pdf_fontsize CHECK (
            export_pdf_fontsize <= 12
        )
        CONSTRAINT default_meeting_export_pdf_fontsize DEFAULT 10,
    export_pdf_line_height double precision
        CONSTRAINT minimum_meeting_export_pdf_line_height CHECK (
            export_pdf_line_height >= 1.0
        )
        CONSTRAINT default_meeting_export_pdf_line_height DEFAULT 1.25,
    export_pdf_page_margin_left integer
        CONSTRAINT minimum_meeting_export_pdf_page_margin_left CHECK (
            export_pdf_page_margin_left >= 0
        )
        CONSTRAINT default_meeting_export_pdf_page_margin_left DEFAULT 20,
    export_pdf_page_margin_top integer
        CONSTRAINT minimum_meeting_export_pdf_page_margin_top CHECK (
            export_pdf_page_margin_top >= 0
        )
        CONSTRAINT default_meeting_export_pdf_page_margin_top DEFAULT 25,
    export_pdf_page_margin_right integer
        CONSTRAINT minimum_meeting_export_pdf_page_margin_right CHECK (
            export_pdf_page_margin_right >= 0
        )
        CONSTRAINT default_meeting_export_pdf_page_margin_right DEFAULT 20,
    export_pdf_page_margin_bottom integer
        CONSTRAINT minimum_meeting_export_pdf_page_margin_bottom CHECK (
            export_pdf_page_margin_bottom >= 0
        )
    ...
    list_of_speakers_amount_last_on_projector integer
        CONSTRAINT minimum_meeting_list_of_speakers_amount_last_on_projector CHECK (
    ...
    list_of_speakers_amount_next_on_projector integer
        CONSTRAINT minimum_meeting_list_of_speakers_amount_next_on_projector CHECK (
    ...
    list_of_speakers_default_structure_level_time integer
        CONSTRAINT minimum_meeting_list_of_speakers_default_structure_level_time CHECK (
    ...
    motions_line_length integer
        CONSTRAINT minimum_meeting_motions_line_length CHECK (
            motions_line_length >= 40
        )
    ...
    motions_block_slide_columns integer
        CONSTRAINT minimum_meeting_motions_block_slide_columns CHECK (
    ...
    motions_supporters_min_amount integer
        CONSTRAINT minimum_meeting_motions_supporters_min_amount CHECK (
...
CREATE TABLE meeting_mediafile_t (
    ...
    CONSTRAINT unique_meeting_mediafile_mediafile_id_meeting_id
        UNIQUE (mediafile_id, meeting_id)
);
...
CREATE TABLE meeting_user_t (
    ...
    vote_weight decimal(16, 6)
        CONSTRAINT minimum_meeting_user_vote_weight CHECK (
    ...
    CONSTRAINT unique_meeting_user_meeting_id_user_id
...
CREATE TABLE motion_t (
    ...
    CONSTRAINT unique_motion_sequential_number_meeting_id
        UNIQUE (sequential_number, meeting_id),
    title varchar(256)
    ...
    start_line_number integer
        CONSTRAINT minimum_motion_start_line_number CHECK (
            start_line_number >= 1
        )
...
CREATE TABLE motion_block_t (
    ...
    CONSTRAINT unique_motion_block_sequential_number_meeting_id
        UNIQUE (sequential_number, meeting_id),
    meeting_id integer
...
CREATE TABLE motion_category_t (
    ...
    CONSTRAINT unique_motion_category_sequential_number_meeting_id
        UNIQUE (sequential_number, meeting_id),
    parent_id integer,
    meeting_id integer
...
CREATE TABLE motion_change_recommendation_t (
    ...
    line_from integer
        ...
        CONSTRAINT minimum_motion_change_recommendation_line_from CHECK (
            line_from >= 0
        ),
    line_to integer
        CONSTRAINT required_motion_change_recommendation_line_to NOT NULL
        CONSTRAINT minimum_motion_change_recommendation_line_to CHECK (
            line_to >= 0
        ),
    text text,
...
CREATE TABLE motion_comment_t (
    ...
    CONSTRAINT unique_motion_comment_motion_id_section_id
        UNIQUE (motion_id, section_id)
);
...
CREATE TABLE motion_comment_section_t (
    ...
    CONSTRAINT unique_motion_comment_section_sequential_number_meeting_id
        UNIQUE (sequential_number, meeting_id),
...
CREATE TABLE motion_editor_t (
    ...
    CONSTRAINT unique_motion_editor_meeting_user_id_motion_id
        UNIQUE (meeting_user_id, motion_id)
);
...
CREATE TABLE motion_submitter_t (
    ...
    CONSTRAINT unique_motion_submitter_meeting_user_id_motion_id
        UNIQUE (meeting_user_id, motion_id)
);
...
CREATE TABLE motion_supporter_t (
    ...
    CONSTRAINT unique_motion_supporter_meeting_user_id_motion_id
        UNIQUE (meeting_user_id, motion_id)
);
...
CREATE TABLE motion_workflow_t (
    ...
    CONSTRAINT unique_motion_workflow_sequential_number_meeting_id
        UNIQUE (sequential_number, meeting_id),
...
CREATE TABLE motion_working_group_speaker_t (
    ...
    CONSTRAINT unique_motion_working_group_speaker_meeting_user_id_motion_id
        UNIQUE (meeting_user_id, motion_id)
);
...
CREATE TABLE option_t (
    ...
    content_object_id varchar(100),
    content_object_id_motion_id integer
        CONSTRAINT generated_always_as_option_content_object_id_motion_id GENERATED ALWAYS AS (
            CASE
                WHEN
                    split_part(content_object_id, '/', 1) = 'motion'
                    THEN cast(split_part(content_object_id, '/', 2) AS integer)
    ...
    content_object_id_user_id integer
        CONSTRAINT generated_always_as_option_content_object_id_user_id GENERATED ALWAYS AS (
            CASE
                WHEN
                    split_part(content_object_id, '/', 1) = 'user'
                    THEN cast(split_part(content_object_id, '/', 2) AS integer)
            END
        ) STORED,
    content_object_id_poll_candidate_list_id integer
        CONSTRAINT unique_option_content_object_id_poll_candidate_list_id UNIQUE
        CONSTRAINT generated_always_as_option_content_object_id_poll_candidd7449d9 GENERATED ALWAYS AS (
            CASE
                WHEN
                    split_part(content_object_id, '/', 1)
                    = 'poll_candidate_list'
                    THEN cast(split_part(content_object_id, '/', 2) AS integer)
            END
        ) STORED,
    CONSTRAINT valid_option_content_object_id_part1
        CHECK (split_part(content_object_id, '/', 1) IN ('motion', 'user', 'poll_candidate_list')),
    meeting_id integer
        CONSTRAINT required_option_meeting_id NOT NULL,
    CONSTRAINT unique_option_content_object_id_poll_id
        UNIQUE (content_object_id, poll_id)
);
...
CREATE TABLE organization_t (
    ...
    limit_of_meetings integer
        CONSTRAINT minimum_organization_limit_of_meetings CHECK (
            limit_of_meetings >= 0
        )
        CONSTRAINT default_organization_limit_of_meetings DEFAULT 0,
    limit_of_users integer
        CONSTRAINT minimum_organization_limit_of_users CHECK (
            limit_of_users >= 0
        )
    ...
    time_zone text
        CONSTRAINT timezone_organization_time_zone CHECK (
            is_timezone(time_zone)
        ),
...
CREATE TABLE organization_tag_t (
    ...
    color varchar(7)
        CONSTRAINT color_organization_tag_color CHECK (
            color IS Null OR color ~* '^#[a-f0-9]{6}$'
        )
...
CREATE TABLE personal_note_t (
    ...
    content_object_id_motion_id integer
        CONSTRAINT generated_always_as_personal_note_content_object_id_motion_id GENERATED ALWAYS AS (
            CASE
                WHEN
                    split_part(content_object_id, '/', 1) = 'motion'
                    THEN cast(split_part(content_object_id, '/', 2) AS integer)
    ...
    CONSTRAINT valid_personal_note_content_object_id_part1
        CHECK (split_part(content_object_id, '/', 1) IN ('motion')),
    meeting_id integer
        CONSTRAINT required_personal_note_meeting_id NOT NULL,
    CONSTRAINT unique_personal_note_meeting_user_id_content_object_id
        UNIQUE (meeting_user_id, content_object_id)
);
...
CREATE TABLE poll_t (
    ...
    max_votes_per_option integer
        CONSTRAINT minimum_poll_max_votes_per_option CHECK (
    ...
    CONSTRAINT unique_poll_sequential_number_meeting_id
        UNIQUE (sequential_number, meeting_id),
    content_object_id varchar(100)
        CONSTRAINT required_poll_content_object_id NOT NULL,
    content_object_id_motion_id integer
        CONSTRAINT generated_always_as_poll_content_object_id_motion_id GENERATED ALWAYS AS (
            CASE
                WHEN
                    split_part(content_object_id, '/', 1) = 'motion'
                    THEN cast(split_part(content_object_id, '/', 2) AS integer)
    ...
    content_object_id_assignment_id integer
        CONSTRAINT generated_always_as_poll_content_object_id_assignment_id GENERATED ALWAYS AS (
            CASE
                WHEN
                    split_part(content_object_id, '/', 1) = 'assignment'
                    THEN cast(split_part(content_object_id, '/', 2) AS integer)
    ...
    content_object_id_topic_id integer
        CONSTRAINT generated_always_as_poll_content_object_id_topic_id GENERATED ALWAYS AS (
            CASE
                WHEN
                    split_part(content_object_id, '/', 1) = 'topic'
                    THEN cast(split_part(content_object_id, '/', 2) AS integer)
    ...
    CONSTRAINT valid_poll_content_object_id_part1
        CHECK (split_part(content_object_id, '/', 1) IN ('motion', 'assignment', 'topic')),
...
CREATE TABLE projection_t (
    ...
    content_object_id_meeting_id integer
        CONSTRAINT generated_always_as_projection_content_object_id_meeting_id GENERATED ALWAYS AS (
            CASE
                WHEN
                    split_part(content_object_id, '/', 1) = 'meeting'
                    THEN cast(split_part(content_object_id, '/', 2) AS integer)
            END
        ) STORED,
    content_object_id_motion_id integer
        CONSTRAINT generated_always_as_projection_content_object_id_motion_id GENERATED ALWAYS AS (
            CASE
                WHEN
                    split_part(content_object_id, '/', 1) = 'motion'
                    THEN cast(split_part(content_object_id, '/', 2) AS integer)
    ...
    content_object_id_meeting_mediafile_id integer
        CONSTRAINT generated_always_as_projection_content_object_id_meeting51b9977 GENERATED ALWAYS AS (
            CASE
                WHEN
                    split_part(content_object_id, '/', 1) = 'meeting_mediafile'
                    THEN cast(split_part(content_object_id, '/', 2) AS integer)
    ...
    content_object_id_list_of_speakers_id integer
        CONSTRAINT generated_always_as_projection_content_object_id_list_of6e51c6f GENERATED ALWAYS AS (
            CASE
                WHEN
                    split_part(content_object_id, '/', 1) = 'list_of_speakers'
                    THEN cast(split_part(content_object_id, '/', 2) AS integer)
            END
        ) STORED,
    content_object_id_motion_block_id integer
        CONSTRAINT generated_always_as_projection_content_object_id_motion_31fedd3 GENERATED ALWAYS AS (
            CASE
                WHEN
                    split_part(content_object_id, '/', 1) = 'motion_block'
                    THEN cast(split_part(content_object_id, '/', 2) AS integer)
    ...
    content_object_id_assignment_id integer
        CONSTRAINT generated_always_as_projection_content_object_id_assignment_id GENERATED ALWAYS AS (
            CASE
                WHEN
                    split_part(content_object_id, '/', 1) = 'assignment'
                    THEN cast(split_part(content_object_id, '/', 2) AS integer)
    ...
    content_object_id_agenda_item_id integer
        CONSTRAINT generated_always_as_projection_content_object_id_agenda_item_id GENERATED ALWAYS AS (
            CASE
                WHEN
                    split_part(content_object_id, '/', 1) = 'agenda_item'
                    THEN cast(split_part(content_object_id, '/', 2) AS integer)
            END
        ) STORED,
    content_object_id_topic_id integer
        CONSTRAINT generated_always_as_projection_content_object_id_topic_id GENERATED ALWAYS AS (
            CASE
                WHEN
                    split_part(content_object_id, '/', 1) = 'topic'
                    THEN cast(split_part(content_object_id, '/', 2) AS integer)
    ...
    content_object_id_poll_id integer
        CONSTRAINT generated_always_as_projection_content_object_id_poll_id GENERATED ALWAYS AS (
            CASE
                WHEN
                    split_part(content_object_id, '/', 1) = 'poll'
                    THEN cast(split_part(content_object_id, '/', 2) AS integer)
            END
        ) STORED,
    content_object_id_projector_message_id integer
        CONSTRAINT generated_always_as_projection_content_object_id_project05617d4 GENERATED ALWAYS AS (
            CASE
                WHEN
                    split_part(content_object_id, '/', 1) = 'projector_message'
                    THEN cast(split_part(content_object_id, '/', 2) AS integer)
            END
        ) STORED,
    content_object_id_projector_countdown_id integer
        CONSTRAINT generated_always_as_projection_content_object_id_project708e3f9 GENERATED ALWAYS AS (
            CASE
                WHEN
                    split_part(content_object_id, '/', 1)
                    = 'projector_countdown'
    ...
    CONSTRAINT valid_projection_content_object_id_part1
        CHECK (split_part(content_object_id, '/', 1) IN ('meeting', 'motion', 'meeting_mediafile', 'list_of_speakers', 'motion_block', 'assignment', 'agenda_item', 'topic', 'poll', 'projector_message', 'projector_countdown')),
    meeting_id integer
...
CREATE TABLE projector_t (
    ...
    aspect_ratio_numerator integer
        CONSTRAINT minimum_projector_aspect_ratio_numerator CHECK (
            aspect_ratio_numerator >= 1
        )
        CONSTRAINT default_projector_aspect_ratio_numerator DEFAULT 16,
    aspect_ratio_denominator integer
        CONSTRAINT minimum_projector_aspect_ratio_denominator CHECK (
            aspect_ratio_denominator >= 1
        )
        CONSTRAINT default_projector_aspect_ratio_denominator DEFAULT 9,
    color varchar(7)
        CONSTRAINT color_projector_color CHECK (
            color IS Null OR color ~* '^#[a-f0-9]{6}$'
        )
        CONSTRAINT default_projector_color DEFAULT '#000000',
    background_color varchar(7)
        CONSTRAINT color_projector_background_color CHECK (
            background_color IS Null OR background_color ~* '^#[a-f0-9]{6}$'
        )
        CONSTRAINT default_projector_background_color DEFAULT '#ffffff',
    header_background_color varchar(7)
        CONSTRAINT color_projector_header_background_color CHECK (
            header_background_color IS Null
            OR header_background_color ~* '^#[a-f0-9]{6}$'
        )
        CONSTRAINT default_projector_header_background_color DEFAULT '#317796',
    header_font_color varchar(7)
        CONSTRAINT color_projector_header_font_color CHECK (
            header_font_color IS Null OR header_font_color ~* '^#[a-f0-9]{6}$'
        )
        CONSTRAINT default_projector_header_font_color DEFAULT '#f5f5f5',
    header_h1_color varchar(7)
        CONSTRAINT color_projector_header_h1_color CHECK (
            header_h1_color IS Null OR header_h1_color ~* '^#[a-f0-9]{6}$'
        )
        CONSTRAINT default_projector_header_h1_color DEFAULT '#317796',
    chyron_background_color varchar(7)
        CONSTRAINT color_projector_chyron_background_color CHECK (
            chyron_background_color IS Null
            OR chyron_background_color ~* '^#[a-f0-9]{6}$'
        )
        CONSTRAINT default_projector_chyron_background_color DEFAULT '#317796',
    chyron_background_color_2 varchar(7)
        CONSTRAINT color_projector_chyron_background_color_2 CHECK (
            chyron_background_color_2 IS Null
            OR chyron_background_color_2 ~* '^#[a-f0-9]{6}$'
        )
        CONSTRAINT default_projector_chyron_background_color_2 DEFAULT '#134768',
    chyron_font_color varchar(7)
        CONSTRAINT color_projector_chyron_font_color CHECK (
            chyron_font_color IS Null OR chyron_font_color ~* '^#[a-f0-9]{6}$'
        )
        CONSTRAINT default_projector_chyron_font_color DEFAULT '#ffffff',
    chyron_font_color_2 varchar(7)
        CONSTRAINT color_projector_chyron_font_color_2 CHECK (
            chyron_font_color_2 IS Null
            OR chyron_font_color_2 ~* '^#[a-f0-9]{6}$'
        )
    ...
    CONSTRAINT unique_projector_sequential_number_meeting_id
        UNIQUE (sequential_number, meeting_id),
...
CREATE TABLE projector_countdown_t (
    ...
    CONSTRAINT unique_projector_countdown_meeting_id_title
        UNIQUE (meeting_id, title)
);
...
CREATE TABLE structure_level_t (
    ...
    color varchar(7)
        CONSTRAINT color_structure_level_color CHECK (
            color IS Null OR color ~* '^#[a-f0-9]{6}$'
        ),
    default_time integer
        CONSTRAINT minimum_structure_level_default_time CHECK (
            default_time >= 0
        ),
    meeting_id integer
...
CREATE TABLE structure_level_list_of_speakers_t (
    ...
    initial_time integer
        ...
        CONSTRAINT minimum_structure_level_list_of_speakers_initial_time CHECK (
            initial_time >= 1
        ),
    ...
    CONSTRAINT unique_structure_level_list_of_speakers_meeting_id_struce047abe
        UNIQUE (meeting_id, structure_level_id, list_of_speakers_id)
);
...
CREATE TABLE theme_t (
    ...
    accent_100 varchar(7)
        CONSTRAINT color_theme_accent_100 CHECK (
            accent_100 IS Null OR accent_100 ~* '^#[a-f0-9]{6}$'
        ),
    accent_200 varchar(7)
        CONSTRAINT color_theme_accent_200 CHECK (
            accent_200 IS Null OR accent_200 ~* '^#[a-f0-9]{6}$'
        ),
    accent_300 varchar(7)
        CONSTRAINT color_theme_accent_300 CHECK (
            accent_300 IS Null OR accent_300 ~* '^#[a-f0-9]{6}$'
        ),
    accent_400 varchar(7)
        CONSTRAINT color_theme_accent_400 CHECK (
            accent_400 IS Null OR accent_400 ~* '^#[a-f0-9]{6}$'
        ),
    accent_50 varchar(7)
        CONSTRAINT color_theme_accent_50 CHECK (
            accent_50 IS Null OR accent_50 ~* '^#[a-f0-9]{6}$'
        ),
    accent_500 varchar(7)
        CONSTRAINT color_theme_accent_500 CHECK (
            accent_500 IS Null OR accent_500 ~* '^#[a-f0-9]{6}$'
        )
        CONSTRAINT required_theme_accent_500 NOT NULL
        CONSTRAINT default_theme_accent_500 DEFAULT '#2196f3',
    accent_600 varchar(7)
        CONSTRAINT color_theme_accent_600 CHECK (
            accent_600 IS Null OR accent_600 ~* '^#[a-f0-9]{6}$'
        ),
    accent_700 varchar(7)
        CONSTRAINT color_theme_accent_700 CHECK (
            accent_700 IS Null OR accent_700 ~* '^#[a-f0-9]{6}$'
        ),
    accent_800 varchar(7)
        CONSTRAINT color_theme_accent_800 CHECK (
            accent_800 IS Null OR accent_800 ~* '^#[a-f0-9]{6}$'
        ),
    accent_900 varchar(7)
        CONSTRAINT color_theme_accent_900 CHECK (
            accent_900 IS Null OR accent_900 ~* '^#[a-f0-9]{6}$'
        ),
    accent_a100 varchar(7)
        CONSTRAINT color_theme_accent_a100 CHECK (
            accent_a100 IS Null OR accent_a100 ~* '^#[a-f0-9]{6}$'
        ),
    accent_a200 varchar(7)
        CONSTRAINT color_theme_accent_a200 CHECK (
            accent_a200 IS Null OR accent_a200 ~* '^#[a-f0-9]{6}$'
        ),
    accent_a400 varchar(7)
        CONSTRAINT color_theme_accent_a400 CHECK (
            accent_a400 IS Null OR accent_a400 ~* '^#[a-f0-9]{6}$'
        ),
    accent_a700 varchar(7)
        CONSTRAINT color_theme_accent_a700 CHECK (
            accent_a700 IS Null OR accent_a700 ~* '^#[a-f0-9]{6}$'
        ),
    primary_100 varchar(7)
        CONSTRAINT color_theme_primary_100 CHECK (
            primary_100 IS Null OR primary_100 ~* '^#[a-f0-9]{6}$'
        ),
    primary_200 varchar(7)
        CONSTRAINT color_theme_primary_200 CHECK (
            primary_200 IS Null OR primary_200 ~* '^#[a-f0-9]{6}$'
        ),
    primary_300 varchar(7)
        CONSTRAINT color_theme_primary_300 CHECK (
            primary_300 IS Null OR primary_300 ~* '^#[a-f0-9]{6}$'
        ),
    primary_400 varchar(7)
        CONSTRAINT color_theme_primary_400 CHECK (
            primary_400 IS Null OR primary_400 ~* '^#[a-f0-9]{6}$'
        ),
    primary_50 varchar(7)
        CONSTRAINT color_theme_primary_50 CHECK (
            primary_50 IS Null OR primary_50 ~* '^#[a-f0-9]{6}$'
        ),
    primary_500 varchar(7)
        CONSTRAINT color_theme_primary_500 CHECK (
            primary_500 IS Null OR primary_500 ~* '^#[a-f0-9]{6}$'
        )
        CONSTRAINT required_theme_primary_500 NOT NULL
        CONSTRAINT default_theme_primary_500 DEFAULT '#317796',
    primary_600 varchar(7)
        CONSTRAINT color_theme_primary_600 CHECK (
            primary_600 IS Null OR primary_600 ~* '^#[a-f0-9]{6}$'
        ),
    primary_700 varchar(7)
        CONSTRAINT color_theme_primary_700 CHECK (
            primary_700 IS Null OR primary_700 ~* '^#[a-f0-9]{6}$'
        ),
    primary_800 varchar(7)
        CONSTRAINT color_theme_primary_800 CHECK (
            primary_800 IS Null OR primary_800 ~* '^#[a-f0-9]{6}$'
        ),
    primary_900 varchar(7)
        CONSTRAINT color_theme_primary_900 CHECK (
            primary_900 IS Null OR primary_900 ~* '^#[a-f0-9]{6}$'
        ),
    primary_a100 varchar(7)
        CONSTRAINT color_theme_primary_a100 CHECK (
            primary_a100 IS Null OR primary_a100 ~* '^#[a-f0-9]{6}$'
        ),
    primary_a200 varchar(7)
        CONSTRAINT color_theme_primary_a200 CHECK (
            primary_a200 IS Null OR primary_a200 ~* '^#[a-f0-9]{6}$'
        ),
    primary_a400 varchar(7)
        CONSTRAINT color_theme_primary_a400 CHECK (
            primary_a400 IS Null OR primary_a400 ~* '^#[a-f0-9]{6}$'
        ),
    primary_a700 varchar(7)
        CONSTRAINT color_theme_primary_a700 CHECK (
            primary_a700 IS Null OR primary_a700 ~* '^#[a-f0-9]{6}$'
        ),
    warn_100 varchar(7)
        CONSTRAINT color_theme_warn_100 CHECK (
            warn_100 IS Null OR warn_100 ~* '^#[a-f0-9]{6}$'
        ),
    warn_200 varchar(7)
        CONSTRAINT color_theme_warn_200 CHECK (
            warn_200 IS Null OR warn_200 ~* '^#[a-f0-9]{6}$'
        ),
    warn_300 varchar(7)
        CONSTRAINT color_theme_warn_300 CHECK (
            warn_300 IS Null OR warn_300 ~* '^#[a-f0-9]{6}$'
        ),
    warn_400 varchar(7)
        CONSTRAINT color_theme_warn_400 CHECK (
            warn_400 IS Null OR warn_400 ~* '^#[a-f0-9]{6}$'
        ),
    warn_50 varchar(7)
        CONSTRAINT color_theme_warn_50 CHECK (
            warn_50 IS Null OR warn_50 ~* '^#[a-f0-9]{6}$'
        ),
    warn_500 varchar(7)
        CONSTRAINT color_theme_warn_500 CHECK (
            warn_500 IS Null OR warn_500 ~* '^#[a-f0-9]{6}$'
        )
        CONSTRAINT required_theme_warn_500 NOT NULL
        CONSTRAINT default_theme_warn_500 DEFAULT '#f06400',
    warn_600 varchar(7)
        CONSTRAINT color_theme_warn_600 CHECK (
            warn_600 IS Null OR warn_600 ~* '^#[a-f0-9]{6}$'
        ),
    warn_700 varchar(7)
        CONSTRAINT color_theme_warn_700 CHECK (
            warn_700 IS Null OR warn_700 ~* '^#[a-f0-9]{6}$'
        ),
    warn_800 varchar(7)
        CONSTRAINT color_theme_warn_800 CHECK (
            warn_800 IS Null OR warn_800 ~* '^#[a-f0-9]{6}$'
        ),
    warn_900 varchar(7)
        CONSTRAINT color_theme_warn_900 CHECK (
            warn_900 IS Null OR warn_900 ~* '^#[a-f0-9]{6}$'
        ),
    warn_a100 varchar(7)
        CONSTRAINT color_theme_warn_a100 CHECK (
            warn_a100 IS Null OR warn_a100 ~* '^#[a-f0-9]{6}$'
        ),
    warn_a200 varchar(7)
        CONSTRAINT color_theme_warn_a200 CHECK (
            warn_a200 IS Null OR warn_a200 ~* '^#[a-f0-9]{6}$'
        ),
    warn_a400 varchar(7)
        CONSTRAINT color_theme_warn_a400 CHECK (
            warn_a400 IS Null OR warn_a400 ~* '^#[a-f0-9]{6}$'
        ),
    warn_a700 varchar(7)
        CONSTRAINT color_theme_warn_a700 CHECK (
            warn_a700 IS Null OR warn_a700 ~* '^#[a-f0-9]{6}$'
        ),
    headbar varchar(7)
        CONSTRAINT color_theme_headbar CHECK (
            headbar IS Null OR headbar ~* '^#[a-f0-9]{6}$'
        ),
    yes varchar(7)
        CONSTRAINT color_theme_yes CHECK (
            yes IS Null OR yes ~* '^#[a-f0-9]{6}$'
        ),
    no varchar(7)
        CONSTRAINT color_theme_no CHECK (no IS Null OR no ~* '^#[a-f0-9]{6}$'),
    abstain varchar(7)
        CONSTRAINT color_theme_abstain CHECK (
            abstain IS Null OR abstain ~* '^#[a-f0-9]{6}$'
        ),
    organization_id integer
...
CREATE TABLE topic_t (
    ...
    CONSTRAINT unique_topic_sequential_number_meeting_id
        UNIQUE (sequential_number, meeting_id),
    meeting_id integer
...
CREATE TABLE user_t (
    ...
    default_vote_weight decimal(16, 6)
        CONSTRAINT minimum_user_default_vote_weight CHECK (
            default_vote_weight >= 0.000001
        )
...
CREATE TABLE nm_chat_group_read_group_ids_group_t (
    chat_group_id integer
        ...
        CONSTRAINT fk_nm_chat_group_read_group_ids_group_t_chat_group_id_chc0b2569 REFERENCES chat_group_t(
            id
        )
        ON DELETE CASCADE
        INITIALLY DEFERRED,
    group_id integer
        ...
        CONSTRAINT fk_nm_chat_group_read_group_ids_group_t_group_id_group_t_id REFERENCES group_t(
            id
        )
        ON DELETE CASCADE
        INITIALLY DEFERRED,
    CONSTRAINT pk_nm_chat_group_read_group_ids_group_t
        PRIMARY KEY (chat_group_id, group_id)
);
CREATE INDEX idx_nm_chat_group_read_group_ids_group_t_chat_group_id ON nm_chat_group_read_group_ids_group_t (
    chat_group_id
);
CREATE INDEX idx_nm_chat_group_read_group_ids_group_t_group_id ON nm_chat_group_read_group_ids_group_t (
    group_id
);
...
CREATE TABLE nm_chat_group_write_group_ids_group_t (
    chat_group_id integer
        ...
        CONSTRAINT fk_nm_chat_group_write_group_ids_group_t_chat_group_id_cc085d6c REFERENCES chat_group_t(
            id
        )
        ON DELETE CASCADE
        INITIALLY DEFERRED,
    group_id integer
        ...
        CONSTRAINT fk_nm_chat_group_write_group_ids_group_t_group_id_group_t_id REFERENCES group_t(
            id
        )
        ON DELETE CASCADE
        INITIALLY DEFERRED,
    CONSTRAINT pk_nm_chat_group_write_group_ids_group_t
        PRIMARY KEY (chat_group_id, group_id)
);
CREATE INDEX idx_nm_chat_group_write_group_ids_group_t_chat_group_id ON nm_chat_group_write_group_ids_group_t (
    chat_group_id
);
CREATE INDEX idx_nm_chat_group_write_group_ids_group_t_group_id ON nm_chat_group_write_group_ids_group_t (
    group_id
);
...
CREATE TABLE nm_committee_manager_ids_user_t (
    committee_id integer
        ...
        CONSTRAINT fk_nm_committee_manager_ids_user_t_committee_id_committee_t_id REFERENCES committee_t(
            id
        )
        ON DELETE CASCADE
        INITIALLY DEFERRED,
    user_id integer
        ...
        CONSTRAINT fk_nm_committee_manager_ids_user_t_user_id_user_t_id REFERENCES user_t(
            id
        )
        ON DELETE CASCADE
        INITIALLY DEFERRED,
    CONSTRAINT pk_nm_committee_manager_ids_user_t
        PRIMARY KEY (committee_id, user_id)
);
CREATE INDEX idx_nm_committee_manager_ids_user_t_committee_id ON nm_committee_manager_ids_user_t (
    committee_id
);
CREATE INDEX idx_nm_committee_manager_ids_user_t_user_id ON nm_committee_manager_ids_user_t (
    user_id
);
...
CREATE TABLE nm_committee_all_child_ids_committee_t (
    all_parent_id integer
        ...
        CONSTRAINT fk_nm_committee_all_child_ids_committee_t_all_parent_id_014ed42 REFERENCES committee_t(
            id
        )
        ON DELETE CASCADE
        INITIALLY DEFERRED,
    all_child_id integer
        ...
        CONSTRAINT fk_nm_committee_all_child_ids_committee_t_all_child_id_cc86a8b2 REFERENCES committee_t(
            id
        )
        ON DELETE CASCADE
        INITIALLY DEFERRED,
    CONSTRAINT pk_nm_committee_all_child_ids_committee_t
        PRIMARY KEY (all_parent_id, all_child_id)
);
CREATE INDEX idx_nm_committee_all_child_ids_committee_t_all_parent_id ON nm_committee_all_child_ids_committee_t (
    all_parent_id
);
CREATE INDEX idx_nm_committee_all_child_ids_committee_t_all_child_id ON nm_committee_all_child_ids_committee_t (
    all_child_id
);
...
CREATE TABLE nm_committee_forward_to_committee_ids_committee_t (
    receive_forwardings_from_committee_id integer
        ...
        CONSTRAINT fk_nm_committee_forward_to_committee_ids_committee_t_rec0dc00a3 REFERENCES committee_t(
            id
        )
        ON DELETE CASCADE
        INITIALLY DEFERRED,
    forward_to_committee_id integer
        ...
        CONSTRAINT fk_nm_committee_forward_to_committee_ids_committee_t_fora987475 REFERENCES committee_t(
            id
        )
        ON DELETE CASCADE
        INITIALLY DEFERRED,
    CONSTRAINT pk_nm_committee_forward_to_committee_ids_committee_t
        PRIMARY KEY (
            receive_forwardings_from_committee_id, forward_to_committee_id
        )
);
CREATE INDEX idx_nm_committee_forward_to_committee_ids_committee_t_re9215b1e ON nm_committee_forward_to_committee_ids_committee_t (
    receive_forwardings_from_committee_id
);
CREATE INDEX idx_nm_committee_forward_to_committee_ids_committee_t_fo5507b60 ON nm_committee_forward_to_committee_ids_committee_t (
...
CREATE TABLE nm_group_meeting_user_ids_meeting_user_t (
    group_id integer
        ...
        CONSTRAINT fk_nm_group_meeting_user_ids_meeting_user_t_group_id_group_t_id REFERENCES group_t(
            id
        )
        ON DELETE CASCADE
        INITIALLY DEFERRED,
    meeting_user_id integer
        ...
        CONSTRAINT fk_nm_group_meeting_user_ids_meeting_user_t_meeting_userd442927 REFERENCES meeting_user_t(
            id
        )
        ON DELETE CASCADE
        INITIALLY DEFERRED,
    CONSTRAINT pk_nm_group_meeting_user_ids_meeting_user_t
        PRIMARY KEY (group_id, meeting_user_id)
);
CREATE INDEX idx_nm_group_meeting_user_ids_meeting_user_t_group_id ON nm_group_meeting_user_ids_meeting_user_t (
    group_id
);
CREATE INDEX idx_nm_group_meeting_user_ids_meeting_user_t_meeting_user_id ON nm_group_meeting_user_ids_meeting_user_t (
    meeting_user_id
);
...
CREATE TABLE nm_group_mmagi_meeting_mediafile_t (
    group_id integer
        ...
        CONSTRAINT fk_nm_group_mmagi_meeting_mediafile_t_group_id_group_t_id REFERENCES group_t(
            id
        )
        ON DELETE CASCADE
        INITIALLY DEFERRED,
    meeting_mediafile_id integer
        ...
        CONSTRAINT fk_nm_group_mmagi_meeting_mediafile_t_meeting_mediafile_6a1f41a REFERENCES meeting_mediafile_t(
            id
        )
        ON DELETE CASCADE
        INITIALLY DEFERRED,
    CONSTRAINT pk_nm_group_mmagi_meeting_mediafile_t
        PRIMARY KEY (group_id, meeting_mediafile_id)
);
CREATE INDEX idx_nm_group_mmagi_meeting_mediafile_t_group_id ON nm_group_mmagi_meeting_mediafile_t (
    group_id
);
CREATE INDEX idx_nm_group_mmagi_meeting_mediafile_t_meeting_mediafile_id ON nm_group_mmagi_meeting_mediafile_t (
...
CREATE TABLE nm_group_mmiagi_meeting_mediafile_t (
    group_id integer
        ...
        CONSTRAINT fk_nm_group_mmiagi_meeting_mediafile_t_group_id_group_t_id REFERENCES group_t(
            id
        )
        ON DELETE CASCADE
        INITIALLY DEFERRED,
    meeting_mediafile_id integer
        ...
        CONSTRAINT fk_nm_group_mmiagi_meeting_mediafile_t_meeting_mediafile1728d31 REFERENCES meeting_mediafile_t(
            id
        )
        ON DELETE CASCADE
        INITIALLY DEFERRED,
    CONSTRAINT pk_nm_group_mmiagi_meeting_mediafile_t
        PRIMARY KEY (group_id, meeting_mediafile_id)
);
CREATE INDEX idx_nm_group_mmiagi_meeting_mediafile_t_group_id ON nm_group_mmiagi_meeting_mediafile_t (
    group_id
);
CREATE INDEX idx_nm_group_mmiagi_meeting_mediafile_t_meeting_mediafile_id ON nm_group_mmiagi_meeting_mediafile_t (
    meeting_mediafile_id
);
...
CREATE TABLE nm_group_read_comment_section_ids_motion_comment_section_t (
    group_id integer
        ...
        CONSTRAINT fk_nm_group_read_comment_section_ids_motion_comment_sectd992bd3 REFERENCES group_t(
            id
        )
        ON DELETE CASCADE
        INITIALLY DEFERRED,
    motion_comment_section_id integer
        ...
        CONSTRAINT fk_nm_group_read_comment_section_ids_motion_comment_sect2d17ce5 REFERENCES motion_comment_section_t(
            id
        )
        ON DELETE CASCADE
        INITIALLY DEFERRED,
    CONSTRAINT pk_nm_group_read_comment_section_ids_motion_comment_section_t
        PRIMARY KEY (group_id, motion_comment_section_id)
);
CREATE INDEX idx_nm_group_read_comment_section_ids_motion_comment_secb9c23c0 ON nm_group_read_comment_section_ids_motion_comment_section_t (
    group_id
);
CREATE INDEX idx_nm_group_read_comment_section_ids_motion_comment_sec520054a ON nm_group_read_comment_section_ids_motion_comment_section_t (
...
CREATE TABLE nm_group_write_comment_section_ids_motion_comment_section_t (
    group_id integer
        ...
        CONSTRAINT fk_nm_group_write_comment_section_ids_motion_comment_sec39cad7b REFERENCES group_t(
            id
        )
        ON DELETE CASCADE
        INITIALLY DEFERRED,
    motion_comment_section_id integer
        ...
        CONSTRAINT fk_nm_group_write_comment_section_ids_motion_comment_sec9ffa691 REFERENCES motion_comment_section_t(
            id
        )
        ON DELETE CASCADE
        INITIALLY DEFERRED,
    CONSTRAINT pk_nm_group_write_comment_section_ids_motion_comment_section_t
        PRIMARY KEY (group_id, motion_comment_section_id)
);
CREATE INDEX idx_nm_group_write_comment_section_ids_motion_comment_sed5732f3 ON nm_group_write_comment_section_ids_motion_comment_section_t (
    group_id
);
CREATE INDEX idx_nm_group_write_comment_section_ids_motion_comment_se3ab0450 ON nm_group_write_comment_section_ids_motion_comment_section_t (
    motion_comment_section_id
);
...
CREATE TABLE nm_group_poll_ids_poll_t (
    group_id integer
        ...
        CONSTRAINT fk_nm_group_poll_ids_poll_t_group_id_group_t_id REFERENCES group_t(
            id
        )
        ON DELETE CASCADE
        INITIALLY DEFERRED,
    poll_id integer
        ...
        CONSTRAINT fk_nm_group_poll_ids_poll_t_poll_id_poll_t_id REFERENCES poll_t(
            id
        )
        ON DELETE CASCADE
        INITIALLY DEFERRED,
...
CREATE INDEX idx_nm_group_poll_ids_poll_t_group_id ON nm_group_poll_ids_poll_t (
    group_id
);
CREATE INDEX idx_nm_group_poll_ids_poll_t_poll_id ON nm_group_poll_ids_poll_t (
    poll_id
);
...
CREATE TABLE nm_meeting_present_user_ids_user_t (
    meeting_id integer
        ...
        CONSTRAINT fk_nm_meeting_present_user_ids_user_t_meeting_id_meeting_t_id REFERENCES meeting_t(
            id
        )
        ON DELETE CASCADE
        INITIALLY DEFERRED,
    user_id integer
        ...
        CONSTRAINT fk_nm_meeting_present_user_ids_user_t_user_id_user_t_id REFERENCES user_t(
            id
        )
        ON DELETE CASCADE
        INITIALLY DEFERRED,
    CONSTRAINT pk_nm_meeting_present_user_ids_user_t
        PRIMARY KEY (meeting_id, user_id)
);
CREATE INDEX idx_nm_meeting_present_user_ids_user_t_meeting_id ON nm_meeting_present_user_ids_user_t (
    meeting_id
);
CREATE INDEX idx_nm_meeting_present_user_ids_user_t_user_id ON nm_meeting_present_user_ids_user_t (
    user_id
);
...
CREATE TABLE gm_meeting_mediafile_attachment_ids_t (
    meeting_mediafile_id integer
        ...
        CONSTRAINT fk_gm_meeting_mediafile_attachment_ids_t_meeting_mediaficc00c2e REFERENCES meeting_mediafile_t(
            id
        )
        ON DELETE CASCADE
        INITIALLY DEFERRED,
    ...
    attachment_id_motion_id integer
        CONSTRAINT generated_always_as_meeting_mediafile_attachment_id GENERATED ALWAYS AS (
            CASE
                WHEN
                    split_part(attachment_id, '/', 1) = 'motion'
                    THEN cast(split_part(attachment_id, '/', 2) AS integer)
            END
        ) STORED
        CONSTRAINT fk_gm_meeting_mediafile_attachment_ids_t_attachment_id_mec23f0c REFERENCES motion_t(
            id
        )
        ON DELETE CASCADE
        INITIALLY DEFERRED,
    attachment_id_topic_id integer
        CONSTRAINT generated_always_as_meeting_mediafile_attachment_id GENERATED ALWAYS AS (
            CASE
                WHEN
                    split_part(attachment_id, '/', 1) = 'topic'
                    THEN cast(split_part(attachment_id, '/', 2) AS integer)
            END
        ) STORED
        CONSTRAINT fk_gm_meeting_mediafile_attachment_ids_t_attachment_id_tf2c2308 REFERENCES topic_t(
            id
        )
        ON DELETE CASCADE
        INITIALLY DEFERRED,
    attachment_id_assignment_id integer
        CONSTRAINT generated_always_as_meeting_mediafile_attachment_id GENERATED ALWAYS AS (
            CASE
                WHEN
                    split_part(attachment_id, '/', 1) = 'assignment'
                    THEN cast(split_part(attachment_id, '/', 2) AS integer)
            END
        ) STORED
        CONSTRAINT fk_gm_meeting_mediafile_attachment_ids_t_attachment_id_af0f87e8 REFERENCES assignment_t(
            id
        )
        ON DELETE CASCADE
        INITIALLY DEFERRED,
    CONSTRAINT valid_meeting_mediafile_attachment_id_part1
        CHECK (split_part(attachment_id, '/', 1) IN ('motion', 'topic', 'assignment')),
    CONSTRAINT unique_meeting_mediafile_id_attachment_id
        UNIQUE (meeting_mediafile_id, attachment_id)
);
CREATE INDEX idx_gm_meeting_mediafile_attachment_ids_t_meeting_mediafile_id ON gm_meeting_mediafile_attachment_ids_t (
    meeting_mediafile_id
);
CREATE INDEX idx_gm_meeting_mediafile_attachment_ids_t_attachment_id ON gm_meeting_mediafile_attachment_ids_t (
    attachment_id
);
CREATE INDEX idx_gm_meeting_mediafile_attachment_ids_t_attachment_id_3c67b77 ON gm_meeting_mediafile_attachment_ids_t (
    attachment_id_motion_id
);
CREATE INDEX idx_gm_meeting_mediafile_attachment_ids_t_attachment_id_8abf47a ON gm_meeting_mediafile_attachment_ids_t (
    attachment_id_topic_id
);
CREATE INDEX idx_gm_meeting_mediafile_attachment_ids_t_attachment_id_66fb18e ON gm_meeting_mediafile_attachment_ids_t (
...
CREATE TABLE nm_meeting_user_structure_level_ids_structure_level_t (
    meeting_user_id integer
        ...
        CONSTRAINT fk_nm_meeting_user_structure_level_ids_structure_level_t8c0bc42 REFERENCES meeting_user_t(
            id
        )
        ON DELETE CASCADE
        INITIALLY DEFERRED,
    structure_level_id integer
        ...
        CONSTRAINT fk_nm_meeting_user_structure_level_ids_structure_level_ta594d12 REFERENCES structure_level_t(
            id
        )
        ON DELETE CASCADE
        INITIALLY DEFERRED,
    CONSTRAINT pk_nm_meeting_user_structure_level_ids_structure_level_t
        PRIMARY KEY (meeting_user_id, structure_level_id)
);
CREATE INDEX idx_nm_meeting_user_structure_level_ids_structure_level_a842d49 ON nm_meeting_user_structure_level_ids_structure_level_t (
    meeting_user_id
);
CREATE INDEX idx_nm_meeting_user_structure_level_ids_structure_level_abd5dca ON nm_meeting_user_structure_level_ids_structure_level_t (
    structure_level_id
);
...
CREATE TABLE nm_motion_all_derived_motion_ids_motion_t (
    all_origin_id integer
        ...
        CONSTRAINT fk_nm_motion_all_derived_motion_ids_motion_t_all_origin_c37696a REFERENCES motion_t(
            id
        )
        ON DELETE CASCADE
        INITIALLY DEFERRED,
    all_derived_motion_id integer
        ...
        CONSTRAINT fk_nm_motion_all_derived_motion_ids_motion_t_all_derived87fec12 REFERENCES motion_t(
            id
        )
        ON DELETE CASCADE
        INITIALLY DEFERRED,
    CONSTRAINT pk_nm_motion_all_derived_motion_ids_motion_t
        PRIMARY KEY (all_origin_id, all_derived_motion_id)
);
CREATE INDEX idx_nm_motion_all_derived_motion_ids_motion_t_all_origin_id ON nm_motion_all_derived_motion_ids_motion_t (
    all_origin_id
);
CREATE INDEX idx_nm_motion_all_derived_motion_ids_motion_t_all_derivee757fda ON nm_motion_all_derived_motion_ids_motion_t (
...
CREATE TABLE nm_motion_identical_motion_ids_motion_t (
    identical_motion_id_1 integer
        ...
        CONSTRAINT fk_nm_motion_identical_motion_ids_motion_t_identical_motcb3785b REFERENCES motion_t(
            id
        )
        ON DELETE CASCADE
        INITIALLY DEFERRED,
    identical_motion_id_2 integer
        ...
        CONSTRAINT fk_nm_motion_identical_motion_ids_motion_t_identical_mot4e10b0c REFERENCES motion_t(
            id
        )
        ON DELETE CASCADE
        INITIALLY DEFERRED,
    CONSTRAINT pk_nm_motion_identical_motion_ids_motion_t
        PRIMARY KEY (identical_motion_id_1, identical_motion_id_2)
);
CREATE INDEX idx_nm_motion_identical_motion_ids_motion_t_identical_mo6988270 ON nm_motion_identical_motion_ids_motion_t (
    identical_motion_id_1
);
CREATE INDEX idx_nm_motion_identical_motion_ids_motion_t_identical_moe0b67bf ON nm_motion_identical_motion_ids_motion_t (
...
CREATE TABLE gm_motion_state_extension_reference_ids_t (
    motion_id integer
        ...
        CONSTRAINT fk_gm_motion_state_extension_reference_ids_t_motion_id_m49e5f09 REFERENCES motion_t(
            id
        )
        ON DELETE CASCADE
        INITIALLY DEFERRED,
    ...
    state_extension_reference_id_motion_id integer
        CONSTRAINT generated_always_as_motion_state_extension_reference_id GENERATED ALWAYS AS (
            CASE
                WHEN
                    split_part(state_extension_reference_id, '/', 1) = 'motion'
                    THEN
                        cast(
                            split_part(
                                state_extension_reference_id, '/', 2
                            ) AS integer
                        )
            END
        ) STORED
        CONSTRAINT fk_gm_motion_state_extension_reference_ids_t_state_exten1eb8dcc REFERENCES motion_t(
            id
        )
        ON DELETE CASCADE
        INITIALLY DEFERRED,
    CONSTRAINT valid_motion_state_extension_reference_id_part1
        CHECK (split_part(state_extension_reference_id, '/', 1) IN ('motion')),
    CONSTRAINT unique_motion_id_state_extension_reference_id
        UNIQUE (motion_id, state_extension_reference_id)
);
CREATE INDEX idx_gm_motion_state_extension_reference_ids_t_motion_id ON gm_motion_state_extension_reference_ids_t (
    motion_id
);
CREATE INDEX idx_gm_motion_state_extension_reference_ids_t_state_exte869c61b ON gm_motion_state_extension_reference_ids_t (
    state_extension_reference_id
);
CREATE INDEX idx_gm_motion_state_extension_reference_ids_t_state_extee77cee3 ON gm_motion_state_extension_reference_ids_t (
...
CREATE TABLE gm_motion_recommendation_extension_reference_ids_t (
    motion_id integer
        ...
        CONSTRAINT fk_gm_motion_recommendation_extension_reference_ids_t_mo331611e REFERENCES motion_t(
            id
        )
        ON DELETE CASCADE
        INITIALLY DEFERRED,
    ...
    recommendation_extension_reference_id_motion_id integer
        CONSTRAINT generated_always_as_motion_recommendation_extension_refe8d13a13 GENERATED ALWAYS AS (
            CASE
                WHEN
                    split_part(recommendation_extension_reference_id, '/', 1)
                    = 'motion'
                    THEN
                        cast(
                            split_part(
                                recommendation_extension_reference_id, '/', 2
                            ) AS integer
                        )
            END
        ) STORED
        CONSTRAINT fk_gm_motion_recommendation_extension_reference_ids_t_re6acbf83 REFERENCES motion_t(
            id
        )
        ON DELETE CASCADE
        INITIALLY DEFERRED,
    CONSTRAINT valid_motion_recommendation_extension_reference_id_part1
        CHECK (split_part(recommendation_extension_reference_id, '/', 1) IN ('motion')),
    CONSTRAINT unique_motion_id_recommendation_extension_reference_id
        UNIQUE (motion_id, recommendation_extension_reference_id)
);
CREATE INDEX idx_gm_motion_recommendation_extension_reference_ids_t_m81631d0 ON gm_motion_recommendation_extension_reference_ids_t (
    motion_id
);
CREATE INDEX idx_gm_motion_recommendation_extension_reference_ids_t_r6488b59 ON gm_motion_recommendation_extension_reference_ids_t (
    recommendation_extension_reference_id
);
CREATE INDEX idx_gm_motion_recommendation_extension_reference_ids_t_r1489537 ON gm_motion_recommendation_extension_reference_ids_t (
...
CREATE TABLE nm_motion_state_next_state_ids_motion_state_t (
    previous_state_id integer
        ...
        CONSTRAINT fk_nm_motion_state_next_state_ids_motion_state_t_previou40712f2 REFERENCES motion_state_t(
            id
        )
        ON DELETE CASCADE
        INITIALLY DEFERRED,
    next_state_id integer
        ...
        CONSTRAINT fk_nm_motion_state_next_state_ids_motion_state_t_next_st820d55c REFERENCES motion_state_t(
            id
        )
        ON DELETE CASCADE
        INITIALLY DEFERRED,
    CONSTRAINT pk_nm_motion_state_next_state_ids_motion_state_t
        PRIMARY KEY (previous_state_id, next_state_id)
);
CREATE INDEX idx_nm_motion_state_next_state_ids_motion_state_t_previoa964ca1 ON nm_motion_state_next_state_ids_motion_state_t (
    previous_state_id
);
CREATE INDEX idx_nm_motion_state_next_state_ids_motion_state_t_next_state_id ON nm_motion_state_next_state_ids_motion_state_t (
    next_state_id
);
...
CREATE TABLE gm_organization_tag_tagged_ids_t (
    organization_tag_id integer
        ...
        CONSTRAINT fk_gm_organization_tag_tagged_ids_t_organization_tag_id_4c0ab0b REFERENCES organization_tag_t(
            id
        )
        ON DELETE CASCADE
        INITIALLY DEFERRED,
    ...
    tagged_id_committee_id integer
        CONSTRAINT generated_always_as_organization_tag_tagged_id GENERATED ALWAYS AS (
            CASE
                WHEN
                    split_part(tagged_id, '/', 1) = 'committee'
                    THEN cast(split_part(tagged_id, '/', 2) AS integer)
            END
        ) STORED
        CONSTRAINT fk_gm_organization_tag_tagged_ids_t_tagged_id_committee_c4b8172 REFERENCES committee_t(
            id
        )
        ON DELETE CASCADE
        INITIALLY DEFERRED,
    tagged_id_meeting_id integer
        CONSTRAINT generated_always_as_organization_tag_tagged_id GENERATED ALWAYS AS (
            CASE
                WHEN
                    split_part(tagged_id, '/', 1) = 'meeting'
                    THEN cast(split_part(tagged_id, '/', 2) AS integer)
            END
        ) STORED
        CONSTRAINT fk_gm_organization_tag_tagged_ids_t_tagged_id_meeting_id97a619f REFERENCES meeting_t(
            id
        )
        ON DELETE CASCADE
        INITIALLY DEFERRED,
    CONSTRAINT valid_organization_tag_tagged_id_part1
        CHECK (split_part(tagged_id, '/', 1) IN ('committee', 'meeting')),
    CONSTRAINT unique_organization_tag_id_tagged_id
        UNIQUE (organization_tag_id, tagged_id)
);
CREATE INDEX idx_gm_organization_tag_tagged_ids_t_organization_tag_id ON gm_organization_tag_tagged_ids_t (
    organization_tag_id
);
CREATE INDEX idx_gm_organization_tag_tagged_ids_t_tagged_id ON gm_organization_tag_tagged_ids_t (
    tagged_id
);
CREATE INDEX idx_gm_organization_tag_tagged_ids_t_tagged_id_committee_id ON gm_organization_tag_tagged_ids_t (
    tagged_id_committee_id
);
CREATE INDEX idx_gm_organization_tag_tagged_ids_t_tagged_id_meeting_id ON gm_organization_tag_tagged_ids_t (
...
CREATE TABLE nm_poll_voted_ids_user_t (
    poll_id integer
        ...
        CONSTRAINT fk_nm_poll_voted_ids_user_t_poll_id_poll_t_id REFERENCES poll_t(
            id
        )
        ON DELETE CASCADE
        INITIALLY DEFERRED,
    user_id integer
        ...
        CONSTRAINT fk_nm_poll_voted_ids_user_t_user_id_user_t_id REFERENCES user_t(
            id
        )
        ON DELETE CASCADE
        INITIALLY DEFERRED,
...
CREATE INDEX idx_nm_poll_voted_ids_user_t_poll_id ON nm_poll_voted_ids_user_t (
    poll_id
);
CREATE INDEX idx_nm_poll_voted_ids_user_t_user_id ON nm_poll_voted_ids_user_t (
    user_id
);
...
CREATE TABLE gm_tag_tagged_ids_t (
    ...
    tagged_id_agenda_item_id integer
        CONSTRAINT generated_always_as_tag_tagged_id GENERATED ALWAYS AS (
            CASE
                WHEN
                    split_part(tagged_id, '/', 1) = 'agenda_item'
                    THEN cast(split_part(tagged_id, '/', 2) AS integer)
            END
        ) STORED
        CONSTRAINT fk_gm_tag_tagged_ids_t_tagged_id_agenda_item_id_agenda_item_id REFERENCES agenda_item_t(
            id
        )
        ON DELETE CASCADE
        INITIALLY DEFERRED,
    tagged_id_assignment_id integer
        CONSTRAINT generated_always_as_tag_tagged_id GENERATED ALWAYS AS (
            CASE
                WHEN
                    split_part(tagged_id, '/', 1) = 'assignment'
                    THEN cast(split_part(tagged_id, '/', 2) AS integer)
            END
        ) STORED
        CONSTRAINT fk_gm_tag_tagged_ids_t_tagged_id_assignment_id_assignment_id REFERENCES assignment_t(
            id
        )
        ON DELETE CASCADE
        INITIALLY DEFERRED,
    tagged_id_motion_id integer
        CONSTRAINT generated_always_as_tag_tagged_id GENERATED ALWAYS AS (
            CASE
                WHEN
                    split_part(tagged_id, '/', 1) = 'motion'
                    THEN cast(split_part(tagged_id, '/', 2) AS integer)
            END
        ) STORED
        CONSTRAINT fk_gm_tag_tagged_ids_t_tagged_id_motion_id_motion_id REFERENCES motion_t(
            id
        )
        ON DELETE CASCADE
        INITIALLY DEFERRED,
    CONSTRAINT valid_tag_tagged_id_part1
        CHECK (split_part(tagged_id, '/', 1) IN ('agenda_item', 'assignment', 'motion')),
...
CREATE INDEX idx_gm_tag_tagged_ids_t_tagged_id ON gm_tag_tagged_ids_t (
    tagged_id
);
CREATE INDEX idx_gm_tag_tagged_ids_t_tagged_id_agenda_item_id ON gm_tag_tagged_ids_t (
    tagged_id_agenda_item_id
);
CREATE INDEX idx_gm_tag_tagged_ids_t_tagged_id_assignment_id ON gm_tag_tagged_ids_t (
    tagged_id_assignment_id
);
CREATE INDEX idx_gm_tag_tagged_ids_t_tagged_id_motion_id ON gm_tag_tagged_ids_t (
    tagged_id_motion_id
);
...
CREATE VIEW "action_worker" AS SELECT * FROM action_worker_t;


CREATE VIEW "agenda_item" AS SELECT
*,
(SELECT array_agg(ai.id
ORDER BY ai.id) FROM agenda_item_t AS ai
WHERE ai.parent_id = a.id
) AS child_ids,
(SELECT array_agg(g.tag_id
ORDER BY g.tag_id) FROM gm_tag_tagged_ids_t AS g
WHERE g.tagged_id_agenda_item_id = a.id
) AS tag_ids,
(SELECT array_agg(p.id
ORDER BY p.id) FROM projection_t AS p
WHERE p.content_object_id_agenda_item_id = a.id
) AS projection_ids
FROM agenda_item_t AS a;


CREATE VIEW "assignment" AS SELECT
*,
(SELECT array_agg(ac.id
ORDER BY ac.id) FROM assignment_candidate_t AS ac
WHERE ac.assignment_id = a.id
) AS candidate_ids,
(SELECT array_agg(p.id
ORDER BY p.id) FROM poll_t AS p
WHERE p.content_object_id_assignment_id = a.id
) AS poll_ids,
(SELECT ai.id FROM agenda_item_t AS ai
WHERE ai.content_object_id_assignment_id = a.id
) AS agenda_item_id,
(SELECT l.id FROM list_of_speakers_t AS l
WHERE l.content_object_id_assignment_id = a.id
) AS list_of_speakers_id,
(SELECT array_agg(g.tag_id
ORDER BY g.tag_id) FROM gm_tag_tagged_ids_t AS g
WHERE g.tagged_id_assignment_id = a.id
) AS tag_ids,
(SELECT array_agg(g.meeting_mediafile_id
ORDER BY g.meeting_mediafile_id) FROM gm_meeting_mediafile_attachment_ids_t AS g
WHERE g.attachment_id_assignment_id = a.id
) AS attachment_meeting_mediafile_ids,
(SELECT array_agg(p.id
ORDER BY p.id) FROM projection_t AS p
WHERE p.content_object_id_assignment_id = a.id
) AS projection_ids,
(SELECT array_agg(h.id
ORDER BY h.id) FROM history_entry_t AS h
WHERE h.model_id_assignment_id = a.id
) AS history_entry_ids
FROM assignment_t AS a;


CREATE VIEW "assignment_candidate" AS SELECT * FROM assignment_candidate_t;


CREATE VIEW "chat_group" AS SELECT
*,
(SELECT array_agg(cm.id
ORDER BY cm.id) FROM chat_message_t AS cm
WHERE cm.chat_group_id = c.id
) AS chat_message_ids,
(SELECT array_agg(n.group_id
ORDER BY n.group_id) FROM nm_chat_group_read_group_ids_group_t AS n
WHERE n.chat_group_id = c.id
) AS read_group_ids,
(SELECT array_agg(n.group_id
ORDER BY n.group_id) FROM nm_chat_group_write_group_ids_group_t AS n
WHERE n.chat_group_id = c.id
) AS write_group_ids
FROM chat_group_t AS c;


CREATE VIEW "chat_message" AS SELECT * FROM chat_message_t;


CREATE VIEW "committee" AS SELECT
*,
(SELECT array_agg(m.id
ORDER BY m.id) FROM meeting_t AS m
...
ORDER BY user_id)
  FROM (
    ...
    SELECT mu.user_id
    FROM meeting_t AS m
    INNER JOIN meeting_user_t AS mu ON m.id = mu.meeting_id
    WHERE m.committee_id = c.id
    ...
    SELECT cmu.user_id
    FROM nm_committee_manager_ids_user_t AS cmu
...
,
(SELECT array_agg(n.user_id
ORDER BY n.user_id) FROM nm_committee_manager_ids_user_t AS n
WHERE n.committee_id = c.id
) AS manager_ids,
(SELECT array_agg(ct.id
ORDER BY ct.id) FROM committee_t AS ct
WHERE ct.parent_id = c.id
) AS child_ids,
(SELECT array_agg(n.all_parent_id
ORDER BY n.all_parent_id) FROM nm_committee_all_child_ids_committee_t AS n
WHERE n.all_child_id = c.id
) AS all_parent_ids,
(SELECT array_agg(n.all_child_id
ORDER BY n.all_child_id) FROM nm_committee_all_child_ids_committee_t AS n
WHERE n.all_parent_id = c.id
) AS all_child_ids,
(SELECT array_agg(u.id
ORDER BY u.id) FROM user_t AS u
WHERE u.home_committee_id = c.id
) AS native_user_ids,
(SELECT array_agg(n.forward_to_committee_id
ORDER BY n.forward_to_committee_id)
FROM nm_committee_forward_to_committee_ids_committee_t AS n
WHERE n.receive_forwardings_from_committee_id = c.id
) AS forward_to_committee_ids,
(SELECT array_agg(n.receive_forwardings_from_committee_id
ORDER BY n.receive_forwardings_from_committee_id)
FROM nm_committee_forward_to_committee_ids_committee_t AS n
WHERE n.forward_to_committee_id = c.id
) AS receive_forwardings_from_committee_ids,
(SELECT array_agg(g.organization_tag_id
ORDER BY g.organization_tag_id) FROM gm_organization_tag_tagged_ids_t AS g
WHERE g.tagged_id_committee_id = c.id
) AS organization_tag_ids
FROM committee_t AS c;
...
*,
(SELECT array_agg(n.meeting_user_id
ORDER BY n.meeting_user_id) FROM nm_group_meeting_user_ids_meeting_user_t AS n
WHERE n.group_id = g.id
) AS meeting_user_ids,
(SELECT m.id FROM meeting_t AS m
WHERE m.default_group_id = g.id
) AS default_group_for_meeting_id,
(SELECT m.id FROM meeting_t AS m
WHERE m.admin_group_id = g.id
) AS admin_group_for_meeting_id,
(SELECT m.id FROM meeting_t AS m
WHERE m.anonymous_group_id = g.id
) AS anonymous_group_for_meeting_id,
(SELECT array_agg(n.meeting_mediafile_id
ORDER BY n.meeting_mediafile_id) FROM nm_group_mmagi_meeting_mediafile_t AS n
WHERE n.group_id = g.id
) AS meeting_mediafile_access_group_ids,
(SELECT array_agg(n.meeting_mediafile_id
ORDER BY n.meeting_mediafile_id) FROM nm_group_mmiagi_meeting_mediafile_t AS n
WHERE n.group_id = g.id
) AS meeting_mediafile_inherited_access_group_ids,
(SELECT array_agg(n.motion_comment_section_id
ORDER BY n.motion_comment_section_id)
FROM nm_group_read_comment_section_ids_motion_comment_section_t AS n
WHERE n.group_id = g.id
) AS read_comment_section_ids,
(SELECT array_agg(n.motion_comment_section_id
ORDER BY n.motion_comment_section_id)
FROM nm_group_write_comment_section_ids_motion_comment_section_t AS n
WHERE n.group_id = g.id
) AS write_comment_section_ids,
(SELECT array_agg(n.chat_group_id
ORDER BY n.chat_group_id) FROM nm_chat_group_read_group_ids_group_t AS n
WHERE n.group_id = g.id
) AS read_chat_group_ids,
(SELECT array_agg(n.chat_group_id
ORDER BY n.chat_group_id) FROM nm_chat_group_write_group_ids_group_t AS n
WHERE n.group_id = g.id
) AS write_chat_group_ids,
(SELECT array_agg(n.poll_id
ORDER BY n.poll_id) FROM nm_group_poll_ids_poll_t AS n
WHERE n.group_id = g.id
) AS poll_ids
FROM group_t AS g;
...
CREATE VIEW "history_entry" AS SELECT * FROM history_entry_t;


CREATE VIEW "history_position" AS SELECT
*,
(SELECT array_agg(he.id
ORDER BY he.id) FROM history_entry_t AS he
WHERE he.position_id = h.id
) AS entry_ids
FROM history_position_t AS h;


CREATE VIEW "import_preview" AS SELECT * FROM import_preview_t;


CREATE VIEW "list_of_speakers" AS SELECT
*,
(SELECT array_agg(s.id
ORDER BY s.id) FROM speaker_t AS s
WHERE s.list_of_speakers_id = l.id
) AS speaker_ids,
(SELECT array_agg(s.id
ORDER BY s.id) FROM structure_level_list_of_speakers_t AS s
WHERE s.list_of_speakers_id = l.id
) AS structure_level_list_of_speakers_ids,
(SELECT array_agg(p.id
ORDER BY p.id) FROM projection_t AS p
WHERE p.content_object_id_list_of_speakers_id = l.id
) AS projection_ids
FROM list_of_speakers_t AS l;


CREATE VIEW "mediafile" AS SELECT
*,
(SELECT array_agg(mt.id
ORDER BY mt.id) FROM mediafile_t AS mt
WHERE mt.parent_id = m.id
) AS child_ids,
(SELECT array_agg(mm.id
ORDER BY mm.id) FROM meeting_mediafile_t AS mm
WHERE mm.mediafile_id = m.id
) AS meeting_mediafile_ids
FROM mediafile_t AS m;


CREATE VIEW "meeting" AS SELECT
*,
(SELECT array_agg(g.id
ORDER BY g.id) FROM group_t AS g
WHERE g.used_as_motion_poll_default_id = m.id
) AS motion_poll_default_group_ids,
(SELECT array_agg(p.id
ORDER BY p.id) FROM poll_candidate_list_t AS p
WHERE p.meeting_id = m.id
) AS poll_candidate_list_ids,
(SELECT array_agg(p.id
ORDER BY p.id) FROM poll_candidate_t AS p
WHERE p.meeting_id = m.id
) AS poll_candidate_ids,
(SELECT array_agg(mu.id
ORDER BY mu.id) FROM meeting_user_t AS mu
WHERE mu.meeting_id = m.id
) AS meeting_user_ids,
(SELECT array_agg(g.id
ORDER BY g.id) FROM group_t AS g
WHERE g.used_as_assignment_poll_default_id = m.id
) AS assignment_poll_default_group_ids,
(SELECT array_agg(g.id
ORDER BY g.id) FROM group_t AS g
WHERE g.used_as_poll_default_id = m.id
) AS poll_default_group_ids,
(SELECT array_agg(g.id
ORDER BY g.id) FROM group_t AS g
WHERE g.used_as_topic_poll_default_id = m.id
) AS topic_poll_default_group_ids,
(SELECT array_agg(p.id
ORDER BY p.id) FROM projector_t AS p
WHERE p.meeting_id = m.id
) AS projector_ids,
(SELECT array_agg(p.id
ORDER BY p.id) FROM projection_t AS p
WHERE p.meeting_id = m.id
) AS all_projection_ids,
(SELECT array_agg(p.id
ORDER BY p.id) FROM projector_message_t AS p
WHERE p.meeting_id = m.id
) AS projector_message_ids,
(SELECT array_agg(p.id
ORDER BY p.id) FROM projector_countdown_t AS p
WHERE p.meeting_id = m.id
) AS projector_countdown_ids,
(SELECT array_agg(t.id
ORDER BY t.id) FROM tag_t AS t
WHERE t.meeting_id = m.id
) AS tag_ids,
(SELECT array_agg(a.id
ORDER BY a.id) FROM agenda_item_t AS a
WHERE a.meeting_id = m.id
) AS agenda_item_ids,
(SELECT array_agg(l.id
ORDER BY l.id) FROM list_of_speakers_t AS l
WHERE l.meeting_id = m.id
) AS list_of_speakers_ids,
(SELECT array_agg(s.id
ORDER BY s.id) FROM structure_level_list_of_speakers_t AS s
WHERE s.meeting_id = m.id
) AS structure_level_list_of_speakers_ids,
(SELECT array_agg(p.id
ORDER BY p.id) FROM point_of_order_category_t AS p
WHERE p.meeting_id = m.id
) AS point_of_order_category_ids,
(SELECT array_agg(s.id
ORDER BY s.id) FROM speaker_t AS s
WHERE s.meeting_id = m.id
) AS speaker_ids,
(SELECT array_agg(t.id
ORDER BY t.id) FROM topic_t AS t
WHERE t.meeting_id = m.id
) AS topic_ids,
(SELECT array_agg(g.id
ORDER BY g.id) FROM group_t AS g
...
(SELECT array_agg(mt.id
ORDER BY mt.id) FROM mediafile_t AS mt
WHERE mt.owner_id_meeting_id = m.id
) AS mediafile_ids,
(SELECT array_agg(mt.id
ORDER BY mt.id) FROM motion_t AS mt
WHERE mt.meeting_id = m.id
) AS motion_ids,
(SELECT array_agg(mt.id
ORDER BY mt.id) FROM motion_t AS mt
WHERE mt.origin_meeting_id = m.id
) AS forwarded_motion_ids,
(SELECT array_agg(mc.id
ORDER BY mc.id) FROM motion_comment_section_t AS mc
WHERE mc.meeting_id = m.id
) AS motion_comment_section_ids,
(SELECT array_agg(mc.id
ORDER BY mc.id) FROM motion_category_t AS mc
WHERE mc.meeting_id = m.id
) AS motion_category_ids,
(SELECT array_agg(mb.id
ORDER BY mb.id) FROM motion_block_t AS mb
WHERE mb.meeting_id = m.id
) AS motion_block_ids,
(SELECT array_agg(mw.id
ORDER BY mw.id) FROM motion_workflow_t AS mw
WHERE mw.meeting_id = m.id
) AS motion_workflow_ids,
(SELECT array_agg(mc.id
ORDER BY mc.id) FROM motion_comment_t AS mc
WHERE mc.meeting_id = m.id
) AS motion_comment_ids,
(SELECT array_agg(ms.id
ORDER BY ms.id) FROM motion_submitter_t AS ms
WHERE ms.meeting_id = m.id
) AS motion_submitter_ids,
(SELECT array_agg(ms.id
ORDER BY ms.id) FROM motion_supporter_t AS ms
WHERE ms.meeting_id = m.id
) AS motion_supporter_ids,
(SELECT array_agg(me.id
ORDER BY me.id) FROM motion_editor_t AS me
WHERE me.meeting_id = m.id
) AS motion_editor_ids,
(SELECT array_agg(mw.id
ORDER BY mw.id) FROM motion_working_group_speaker_t AS mw
WHERE mw.meeting_id = m.id
) AS motion_working_group_speaker_ids,
(SELECT array_agg(mc.id
ORDER BY mc.id) FROM motion_change_recommendation_t AS mc
WHERE mc.meeting_id = m.id
) AS motion_change_recommendation_ids,
(SELECT array_agg(ms.id
ORDER BY ms.id) FROM motion_state_t AS ms
WHERE ms.meeting_id = m.id
) AS motion_state_ids,
(SELECT array_agg(p.id
ORDER BY p.id) FROM poll_t AS p
...
ORDER BY o.id) FROM option_t AS o
WHERE o.meeting_id = m.id
) AS option_ids,
(SELECT array_agg(v.id
ORDER BY v.id) FROM vote_t AS v
WHERE v.meeting_id = m.id
) AS vote_ids,
(SELECT array_agg(a.id
ORDER BY a.id) FROM assignment_t AS a
WHERE a.meeting_id = m.id
) AS assignment_ids,
(SELECT array_agg(a.id
ORDER BY a.id) FROM assignment_candidate_t AS a
WHERE a.meeting_id = m.id
) AS assignment_candidate_ids,
(SELECT array_agg(p.id
ORDER BY p.id) FROM personal_note_t AS p
WHERE p.meeting_id = m.id
) AS personal_note_ids,
(SELECT array_agg(c.id
ORDER BY c.id) FROM chat_group_t AS c
WHERE c.meeting_id = m.id
) AS chat_group_ids,
(SELECT array_agg(c.id
ORDER BY c.id) FROM chat_message_t AS c
WHERE c.meeting_id = m.id
) AS chat_message_ids,
(SELECT array_agg(s.id
ORDER BY s.id) FROM structure_level_t AS s
WHERE s.meeting_id = m.id
) AS structure_level_ids,
(SELECT c.id FROM committee_t AS c
WHERE c.default_meeting_id = m.id
) AS default_meeting_for_committee_id,
(SELECT array_agg(g.organization_tag_id
ORDER BY g.organization_tag_id) FROM gm_organization_tag_tagged_ids_t AS g
WHERE g.tagged_id_meeting_id = m.id
) AS organization_tag_ids,
(SELECT array_agg(n.user_id
ORDER BY n.user_id) FROM nm_meeting_present_user_ids_user_t AS n
WHERE n.meeting_id = m.id
) AS present_user_ids,
(
  SELECT array_agg(DISTINCT mu.user_id
ORDER BY mu.user_id)
...
(SELECT array_agg(p.id
ORDER BY p.id) FROM projection_t AS p
WHERE p.content_object_id_meeting_id = m.id
) AS projection_ids,
(SELECT array_agg(p.id
ORDER BY p.id) FROM projector_t AS p
WHERE p.used_as_default_projector_for_agenda_item_list_in_meeting_id = m.id
) AS default_projector_agenda_item_list_ids,
(SELECT array_agg(p.id
ORDER BY p.id) FROM projector_t AS p
WHERE p.used_as_default_projector_for_topic_in_meeting_id = m.id
) AS default_projector_topic_ids,
(SELECT array_agg(p.id
ORDER BY p.id) FROM projector_t AS p
WHERE p.used_as_default_projector_for_list_of_speakers_in_meeting_id = m.id
) AS default_projector_list_of_speakers_ids,
(SELECT array_agg(p.id
ORDER BY p.id) FROM projector_t AS p
WHERE p.used_as_default_projector_for_current_los_in_meeting_id = m.id
) AS default_projector_current_los_ids,
(SELECT array_agg(p.id
ORDER BY p.id) FROM projector_t AS p
WHERE p.used_as_default_projector_for_motion_in_meeting_id = m.id
) AS default_projector_motion_ids,
(SELECT array_agg(p.id
ORDER BY p.id) FROM projector_t AS p
WHERE p.used_as_default_projector_for_amendment_in_meeting_id = m.id
) AS default_projector_amendment_ids,
(SELECT array_agg(p.id
ORDER BY p.id) FROM projector_t AS p
WHERE p.used_as_default_projector_for_motion_block_in_meeting_id = m.id
) AS default_projector_motion_block_ids,
(SELECT array_agg(p.id
ORDER BY p.id) FROM projector_t AS p
WHERE p.used_as_default_projector_for_assignment_in_meeting_id = m.id
) AS default_projector_assignment_ids,
(SELECT array_agg(p.id
ORDER BY p.id) FROM projector_t AS p
WHERE p.used_as_default_projector_for_mediafile_in_meeting_id = m.id
) AS default_projector_mediafile_ids,
(SELECT array_agg(p.id
ORDER BY p.id) FROM projector_t AS p
WHERE p.used_as_default_projector_for_message_in_meeting_id = m.id
) AS default_projector_message_ids,
(SELECT array_agg(p.id
ORDER BY p.id) FROM projector_t AS p
WHERE p.used_as_default_projector_for_countdown_in_meeting_id = m.id
) AS default_projector_countdown_ids,
(SELECT array_agg(p.id
ORDER BY p.id) FROM projector_t AS p
WHERE p.used_as_default_projector_for_assignment_poll_in_meeting_id = m.id
) AS default_projector_assignment_poll_ids,
(SELECT array_agg(p.id
ORDER BY p.id) FROM projector_t AS p
WHERE p.used_as_default_projector_for_motion_poll_in_meeting_id = m.id
) AS default_projector_motion_poll_ids,
(SELECT array_agg(p.id
ORDER BY p.id) FROM projector_t AS p
WHERE p.used_as_default_projector_for_poll_in_meeting_id = m.id
) AS default_projector_poll_ids,
(SELECT array_agg(h.id
ORDER BY h.id) FROM history_entry_t AS h
WHERE h.meeting_id = m.id
) AS relevant_history_entry_ids
FROM meeting_t AS m;
...
*,
(SELECT array_agg(n.group_id
ORDER BY n.group_id) FROM nm_group_mmiagi_meeting_mediafile_t AS n
WHERE n.meeting_mediafile_id = m.id
) AS inherited_access_group_ids,
(SELECT array_agg(n.group_id
ORDER BY n.group_id) FROM nm_group_mmagi_meeting_mediafile_t AS n
WHERE n.meeting_mediafile_id = m.id
) AS access_group_ids,
(SELECT l.id FROM list_of_speakers_t AS l
WHERE l.content_object_id_meeting_mediafile_id = m.id
) AS list_of_speakers_id,
(SELECT array_agg(p.id
ORDER BY p.id) FROM projection_t AS p
WHERE p.content_object_id_meeting_mediafile_id = m.id
) AS projection_ids,
(SELECT array_agg(g.attachment_id
ORDER BY g.attachment_id) FROM gm_meeting_mediafile_attachment_ids_t AS g
WHERE g.meeting_mediafile_id = m.id
) AS attachment_ids,
(SELECT m1.id FROM meeting_t AS m1
WHERE m1.logo_projector_main_id = m.id
) AS used_as_logo_projector_main_in_meeting_id,
(SELECT m1.id FROM meeting_t AS m1
WHERE m1.logo_projector_header_id = m.id
) AS used_as_logo_projector_header_in_meeting_id,
(SELECT m1.id FROM meeting_t AS m1
WHERE m1.logo_web_header_id = m.id
) AS used_as_logo_web_header_in_meeting_id,
(SELECT m1.id FROM meeting_t AS m1
WHERE m1.logo_pdf_header_l_id = m.id
) AS used_as_logo_pdf_header_l_in_meeting_id,
(SELECT m1.id FROM meeting_t AS m1
WHERE m1.logo_pdf_header_r_id = m.id
) AS used_as_logo_pdf_header_r_in_meeting_id,
(SELECT m1.id FROM meeting_t AS m1
WHERE m1.logo_pdf_footer_l_id = m.id
) AS used_as_logo_pdf_footer_l_in_meeting_id,
(SELECT m1.id FROM meeting_t AS m1
WHERE m1.logo_pdf_footer_r_id = m.id
) AS used_as_logo_pdf_footer_r_in_meeting_id,
(SELECT m1.id FROM meeting_t AS m1
WHERE m1.logo_pdf_ballot_paper_id = m.id
) AS used_as_logo_pdf_ballot_paper_in_meeting_id,
(SELECT m1.id FROM meeting_t AS m1
WHERE m1.font_regular_id = m.id
) AS used_as_font_regular_in_meeting_id,
(SELECT m1.id FROM meeting_t AS m1
WHERE m1.font_italic_id = m.id
) AS used_as_font_italic_in_meeting_id,
(SELECT m1.id FROM meeting_t AS m1
WHERE m1.font_bold_id = m.id
) AS used_as_font_bold_in_meeting_id,
(SELECT m1.id FROM meeting_t AS m1
WHERE m1.font_bold_italic_id = m.id
) AS used_as_font_bold_italic_in_meeting_id,
(SELECT m1.id FROM meeting_t AS m1
WHERE m1.font_monospace_id = m.id
) AS used_as_font_monospace_in_meeting_id,
(SELECT m1.id FROM meeting_t AS m1
WHERE m1.font_chyron_speaker_name_id = m.id
) AS used_as_font_chyron_speaker_name_in_meeting_id,
(SELECT m1.id FROM meeting_t AS m1
WHERE m1.font_projector_h1_id = m.id
) AS used_as_font_projector_h1_in_meeting_id,
(SELECT m1.id FROM meeting_t AS m1
WHERE m1.font_projector_h2_id = m.id
) AS used_as_font_projector_h2_in_meeting_id
FROM meeting_mediafile_t AS m;
...
(SELECT array_agg(p.id
ORDER BY p.id) FROM personal_note_t AS p
WHERE p.meeting_user_id = m.id
) AS personal_note_ids,
(SELECT array_agg(s.id
ORDER BY s.id) FROM speaker_t AS s
WHERE s.meeting_user_id = m.id
) AS speaker_ids,
(SELECT array_agg(ms.id
ORDER BY ms.id) FROM motion_supporter_t AS ms
WHERE ms.meeting_user_id = m.id
) AS motion_supporter_ids,
(SELECT array_agg(me.id
ORDER BY me.id) FROM motion_editor_t AS me
WHERE me.meeting_user_id = m.id
) AS motion_editor_ids,
(SELECT array_agg(mw.id
ORDER BY mw.id) FROM motion_working_group_speaker_t AS mw
WHERE mw.meeting_user_id = m.id
) AS motion_working_group_speaker_ids,
(SELECT array_agg(ms.id
ORDER BY ms.id) FROM motion_submitter_t AS ms
WHERE ms.meeting_user_id = m.id
...
(SELECT array_agg(a.id
ORDER BY a.id) FROM assignment_candidate_t AS a
WHERE a.meeting_user_id = m.id
) AS assignment_candidate_ids,
(SELECT array_agg(mu.id
ORDER BY mu.id) FROM meeting_user_t AS mu
WHERE mu.vote_delegated_to_id = m.id
) AS vote_delegations_from_ids,
(SELECT array_agg(c.id
ORDER BY c.id) FROM chat_message_t AS c
WHERE c.meeting_user_id = m.id
) AS chat_message_ids,
(SELECT array_agg(n.group_id
ORDER BY n.group_id) FROM nm_group_meeting_user_ids_meeting_user_t AS n
WHERE n.meeting_user_id = m.id
) AS group_ids,
(SELECT array_agg(n.structure_level_id
ORDER BY n.structure_level_id)
FROM nm_meeting_user_structure_level_ids_structure_level_t AS n
WHERE n.meeting_user_id = m.id
) AS structure_level_ids
FROM meeting_user_t AS m;
...
(SELECT array_agg(mt.id
ORDER BY mt.id) FROM motion_t AS mt
WHERE mt.lead_motion_id = m.id
) AS amendment_ids,
(SELECT array_agg(mt.id
ORDER BY mt.id) FROM motion_t AS mt
WHERE mt.sort_parent_id = m.id
) AS sort_child_ids,
(SELECT array_agg(mt.id
ORDER BY mt.id) FROM motion_t AS mt
WHERE mt.origin_id = m.id
) AS derived_motion_ids,
(SELECT array_agg(n.all_origin_id
ORDER BY n.all_origin_id) FROM nm_motion_all_derived_motion_ids_motion_t AS n
WHERE n.all_derived_motion_id = m.id
) AS all_origin_ids,
(SELECT array_agg(n.all_derived_motion_id
ORDER BY n.all_derived_motion_id)
FROM nm_motion_all_derived_motion_ids_motion_t AS n
WHERE n.all_origin_id = m.id
) AS all_derived_motion_ids,
(SELECT array_cat((SELECT array_agg(n.identical_motion_id_1
ORDER BY n.identical_motion_id_1)
FROM nm_motion_identical_motion_ids_motion_t AS n
WHERE n.identical_motion_id_2 = m.id), (SELECT array_agg(n.identical_motion_id_2
ORDER BY n.identical_motion_id_2)
FROM nm_motion_identical_motion_ids_motion_t AS n
WHERE n.identical_motion_id_1 = m.id))
) AS identical_motion_ids,
(SELECT array_agg(g.state_extension_reference_id
ORDER BY g.state_extension_reference_id)
FROM gm_motion_state_extension_reference_ids_t AS g
WHERE g.motion_id = m.id
) AS state_extension_reference_ids,
(SELECT array_agg(g.motion_id
ORDER BY g.motion_id) FROM gm_motion_state_extension_reference_ids_t AS g
WHERE g.state_extension_reference_id_motion_id = m.id
) AS referenced_in_motion_state_extension_ids,
(SELECT array_agg(g.recommendation_extension_reference_id
ORDER BY g.recommendation_extension_reference_id)
FROM gm_motion_recommendation_extension_reference_ids_t AS g
WHERE g.motion_id = m.id
) AS recommendation_extension_reference_ids,
(SELECT array_agg(g.motion_id
ORDER BY g.motion_id)
FROM gm_motion_recommendation_extension_reference_ids_t AS g
WHERE g.recommendation_extension_reference_id_motion_id = m.id
) AS referenced_in_motion_recommendation_extension_ids,
(SELECT array_agg(ms.id
ORDER BY ms.id) FROM motion_submitter_t AS ms
WHERE ms.motion_id = m.id
) AS submitter_ids,
(SELECT array_agg(ms.id
ORDER BY ms.id) FROM motion_supporter_t AS ms
WHERE ms.motion_id = m.id
...
(SELECT array_agg(me.id
ORDER BY me.id) FROM motion_editor_t AS me
WHERE me.motion_id = m.id
) AS editor_ids,
(SELECT array_agg(mw.id
ORDER BY mw.id) FROM motion_working_group_speaker_t AS mw
WHERE mw.motion_id = m.id
) AS working_group_speaker_ids,
(SELECT array_agg(p.id
ORDER BY p.id) FROM poll_t AS p
WHERE p.content_object_id_motion_id = m.id
) AS poll_ids,
(SELECT array_agg(o.id
ORDER BY o.id) FROM option_t AS o
WHERE o.content_object_id_motion_id = m.id
) AS option_ids,
(SELECT array_agg(mc.id
ORDER BY mc.id) FROM motion_change_recommendation_t AS mc
WHERE mc.motion_id = m.id
) AS change_recommendation_ids,
(SELECT array_agg(mc.id
ORDER BY mc.id) FROM motion_comment_t AS mc
WHERE mc.motion_id = m.id
) AS comment_ids,
(SELECT a.id FROM agenda_item_t AS a
WHERE a.content_object_id_motion_id = m.id
) AS agenda_item_id,
(SELECT l.id FROM list_of_speakers_t AS l
WHERE l.content_object_id_motion_id = m.id
) AS list_of_speakers_id,
(SELECT array_agg(g.tag_id
ORDER BY g.tag_id) FROM gm_tag_tagged_ids_t AS g
WHERE g.tagged_id_motion_id = m.id
) AS tag_ids,
(SELECT array_agg(g.meeting_mediafile_id
ORDER BY g.meeting_mediafile_id) FROM gm_meeting_mediafile_attachment_ids_t AS g
WHERE g.attachment_id_motion_id = m.id
) AS attachment_meeting_mediafile_ids,
(SELECT array_agg(p.id
ORDER BY p.id) FROM projection_t AS p
WHERE p.content_object_id_motion_id = m.id
...
(SELECT array_agg(h.id
ORDER BY h.id) FROM history_entry_t AS h
WHERE h.model_id_motion_id = m.id
) AS history_entry_ids
FROM motion_t AS m;
...
(SELECT array_agg(mt.id
ORDER BY mt.id) FROM motion_t AS mt
WHERE mt.block_id = m.id
) AS motion_ids,
(SELECT a.id FROM agenda_item_t AS a
WHERE a.content_object_id_motion_block_id = m.id
) AS agenda_item_id,
(SELECT l.id FROM list_of_speakers_t AS l
WHERE l.content_object_id_motion_block_id = m.id
) AS list_of_speakers_id,
(SELECT array_agg(p.id
ORDER BY p.id) FROM projection_t AS p
WHERE p.content_object_id_motion_block_id = m.id
) AS projection_ids
FROM motion_block_t AS m;
...
(SELECT array_agg(mt.id
ORDER BY mt.id) FROM motion_t AS mt
WHERE mt.category_id = m.id
) AS motion_ids
FROM motion_category_t AS m;


CREATE VIEW "motion_change_recommendation" AS
SELECT *
FROM motion_change_recommendation_t;


CREATE VIEW "motion_comment" AS SELECT * FROM motion_comment_t;
...
(SELECT array_agg(mc.id
ORDER BY mc.id) FROM motion_comment_t AS mc
WHERE mc.section_id = m.id
) AS comment_ids,
(SELECT array_agg(n.group_id
ORDER BY n.group_id)
FROM nm_group_read_comment_section_ids_motion_comment_section_t AS n
WHERE n.motion_comment_section_id = m.id
) AS read_group_ids,
(SELECT array_agg(n.group_id
ORDER BY n.group_id)
FROM nm_group_write_comment_section_ids_motion_comment_section_t AS n
...
FROM motion_comment_section_t AS m;


CREATE VIEW "motion_editor" AS SELECT * FROM motion_editor_t;
...
(SELECT array_agg(ms.id
ORDER BY ms.id) FROM motion_state_t AS ms
WHERE ms.submitter_withdraw_state_id = m.id
) AS submitter_withdraw_back_ids,
(SELECT array_agg(n.next_state_id
ORDER BY n.next_state_id)
FROM nm_motion_state_next_state_ids_motion_state_t AS n
WHERE n.previous_state_id = m.id
) AS next_state_ids,
(SELECT array_agg(n.previous_state_id
ORDER BY n.previous_state_id)
FROM nm_motion_state_next_state_ids_motion_state_t AS n
WHERE n.next_state_id = m.id
) AS previous_state_ids,
(SELECT array_agg(mt.id
ORDER BY mt.id) FROM motion_t AS mt
WHERE mt.state_id = m.id
) AS motion_ids,
(SELECT array_agg(mt.id
ORDER BY mt.id) FROM motion_t AS mt
WHERE mt.recommendation_id = m.id
) AS motion_recommendation_ids,
(SELECT mw.id FROM motion_workflow_t AS mw
WHERE mw.first_state_id = m.id
) AS first_state_of_workflow_id
FROM motion_state_t AS m;


CREATE VIEW "motion_submitter" AS SELECT * FROM motion_submitter_t;


CREATE VIEW "motion_supporter" AS SELECT * FROM motion_supporter_t;
...
(SELECT array_agg(ms.id
ORDER BY ms.id) FROM motion_state_t AS ms
WHERE ms.workflow_id = m.id
) AS state_ids,
(SELECT m1.id FROM meeting_t AS m1
WHERE m1.motions_default_workflow_id = m.id
) AS default_workflow_meeting_id,
(SELECT m1.id FROM meeting_t AS m1
WHERE m1.motions_default_amendment_workflow_id = m.id
) AS default_amendment_workflow_meeting_id
FROM motion_workflow_t AS m;


CREATE VIEW "motion_working_group_speaker" AS
SELECT *
...
*,
(SELECT p.id FROM poll_t AS p
WHERE p.global_option_id = o.id
) AS used_as_global_option_in_poll_id,
(SELECT array_agg(v.id
ORDER BY v.id) FROM vote_t AS v
...
*,
(SELECT array_agg(g.id
ORDER BY g.id) FROM gender_t AS g
WHERE g.organization_id = o.id
) AS gender_ids,
(SELECT array_agg(c.id
ORDER BY c.id) FROM committee_t AS c
WHERE c.organization_id = o.id
) AS committee_ids,
(SELECT array_agg(m.id
ORDER BY m.id) FROM meeting_t AS m
WHERE m.is_active_in_organization_id = o.id
) AS active_meeting_ids,
(SELECT array_agg(m.id
ORDER BY m.id) FROM meeting_t AS m
WHERE m.is_archived_in_organization_id = o.id
) AS archived_meeting_ids,
(SELECT array_agg(m.id
ORDER BY m.id) FROM meeting_t AS m
WHERE m.template_for_organization_id = o.id
) AS template_meeting_ids,
(SELECT array_agg(ot.id
ORDER BY ot.id) FROM organization_tag_t AS ot
WHERE ot.organization_id = o.id
) AS organization_tag_ids,
(SELECT array_agg(t.id
ORDER BY t.id) FROM theme_t AS t
WHERE t.organization_id = o.id
) AS theme_ids,
(SELECT array_agg(m.id
ORDER BY m.id) FROM mediafile_t AS m
WHERE m.owner_id_organization_id = o.id
) AS mediafile_ids,
(SELECT array_agg(m.id
ORDER BY m.id) FROM mediafile_t AS m
WHERE m.published_to_meetings_in_organization_id = o.id
) AS published_mediafile_ids,
(SELECT array_agg(u.id
ORDER BY u.id) FROM user_t AS u
WHERE u.organization_id = o.id
) AS user_ids
FROM organization_t AS o;


CREATE VIEW "organization_tag" AS SELECT
*,
(SELECT array_agg(g.tagged_id
ORDER BY g.tagged_id) FROM gm_organization_tag_tagged_ids_t AS g
WHERE g.organization_tag_id = o.id
) AS tagged_ids
FROM organization_tag_t AS o;


CREATE VIEW "personal_note" AS SELECT * FROM personal_note_t;
...
(SELECT array_agg(s.id
ORDER BY s.id) FROM speaker_t AS s
WHERE s.point_of_order_category_id = p.id
) AS speaker_ids
FROM point_of_order_category_t AS p;
...
(SELECT array_agg(n.user_id
ORDER BY n.user_id) FROM nm_poll_voted_ids_user_t AS n
WHERE n.poll_id = p.id
) AS voted_ids,
(SELECT array_agg(n.group_id
ORDER BY n.group_id) FROM nm_group_poll_ids_poll_t AS n
WHERE n.poll_id = p.id
) AS entitled_group_ids,
(SELECT array_agg(pt.id
ORDER BY pt.id) FROM projection_t AS pt
WHERE pt.content_object_id_poll_id = p.id
) AS projection_ids
FROM poll_t AS p;


CREATE VIEW "poll_candidate" AS SELECT * FROM poll_candidate_t;


CREATE VIEW "poll_candidate_list" AS SELECT
*,
(SELECT array_agg(pc.id
ORDER BY pc.id) FROM poll_candidate_t AS pc
WHERE pc.poll_candidate_list_id = p.id
) AS poll_candidate_ids,
(SELECT o.id FROM option_t AS o
WHERE o.content_object_id_poll_candidate_list_id = p.id
) AS option_id
FROM poll_candidate_list_t AS p;


CREATE VIEW "projection" AS SELECT * FROM projection_t;
...
(SELECT array_agg(pt.id
ORDER BY pt.id) FROM projection_t AS pt
WHERE pt.current_projector_id = p.id
) AS current_projection_ids,
(SELECT array_agg(pt.id
ORDER BY pt.id) FROM projection_t AS pt
WHERE pt.preview_projector_id = p.id
) AS preview_projection_ids,
(SELECT array_agg(pt.id
ORDER BY pt.id) FROM projection_t AS pt
WHERE pt.history_projector_id = p.id
) AS history_projection_ids,
(SELECT m.id FROM meeting_t AS m
WHERE m.reference_projector_id = p.id
) AS used_as_reference_projector_meeting_id
FROM projector_t AS p;
...
(SELECT array_agg(pt.id
ORDER BY pt.id) FROM projection_t AS pt
WHERE pt.content_object_id_projector_countdown_id = p.id
) AS projection_ids,
(SELECT m.id FROM meeting_t AS m
WHERE m.list_of_speakers_countdown_id = p.id
) AS used_as_list_of_speakers_countdown_meeting_id,
(SELECT m.id FROM meeting_t AS m
WHERE m.poll_countdown_id = p.id
) AS used_as_poll_countdown_meeting_id
FROM projector_countdown_t AS p;
...
(SELECT array_agg(pt.id
ORDER BY pt.id) FROM projection_t AS pt
WHERE pt.content_object_id_projector_message_id = p.id
) AS projection_ids
FROM projector_message_t AS p;


CREATE VIEW "speaker" AS SELECT * FROM speaker_t;


CREATE VIEW "structure_level" AS SELECT
*,
(SELECT array_agg(n.meeting_user_id
ORDER BY n.meeting_user_id)
FROM nm_meeting_user_structure_level_ids_structure_level_t AS n
WHERE n.structure_level_id = s.id
) AS meeting_user_ids,
(SELECT array_agg(sl.id
ORDER BY sl.id) FROM structure_level_list_of_speakers_t AS sl
WHERE sl.structure_level_id = s.id
) AS structure_level_list_of_speakers_ids
FROM structure_level_t AS s;


CREATE VIEW "structure_level_list_of_speakers" AS SELECT
*,
(SELECT array_agg(st.id
ORDER BY st.id) FROM speaker_t AS st
WHERE st.structure_level_list_of_speakers_id = s.id
) AS speaker_ids
FROM structure_level_list_of_speakers_t AS s;


CREATE VIEW "tag" AS SELECT
*,
(SELECT array_agg(g.tagged_id
ORDER BY g.tagged_id) FROM gm_tag_tagged_ids_t AS g
WHERE g.tag_id = t.id
) AS tagged_ids
FROM tag_t AS t;
...
*,
(SELECT o.id FROM organization_t AS o
WHERE o.theme_id = t.id
) AS theme_for_organization_id
FROM theme_t AS t;
...
(SELECT array_agg(g.meeting_mediafile_id
ORDER BY g.meeting_mediafile_id) FROM gm_meeting_mediafile_attachment_ids_t AS g
WHERE g.attachment_id_topic_id = t.id
) AS attachment_meeting_mediafile_ids,
(SELECT a.id FROM agenda_item_t AS a
WHERE a.content_object_id_topic_id = t.id
) AS agenda_item_id,
(SELECT l.id FROM list_of_speakers_t AS l
WHERE l.content_object_id_topic_id = t.id
) AS list_of_speakers_id,
(SELECT array_agg(p.id
ORDER BY p.id) FROM poll_t AS p
WHERE p.content_object_id_topic_id = t.id
) AS poll_ids,
(SELECT array_agg(p.id
ORDER BY p.id) FROM projection_t AS p
WHERE p.content_object_id_topic_id = t.id
...
*,
(SELECT array_agg(n.meeting_id
ORDER BY n.meeting_id) FROM nm_meeting_present_user_ids_user_t AS n
WHERE n.user_id = u.id
) AS is_present_in_meeting_ids,
(
  SELECT array_agg(DISTINCT ci.committee_id
ORDER BY ci.committee_id)
  FROM (
    -- Select committee_ids from meetings the user is part of
    SELECT m.committee_id
    FROM meeting_user_t AS mu
    INNER JOIN meeting_t AS m ON mu.meeting_id = m.id
...
,
(SELECT array_agg(n.committee_id
ORDER BY n.committee_id) FROM nm_committee_manager_ids_user_t AS n
WHERE n.user_id = u.id
) AS committee_management_ids,
(SELECT array_agg(m.id
ORDER BY m.id) FROM meeting_user_t AS m
WHERE m.user_id = u.id
) AS meeting_user_ids,
(SELECT array_agg(n.poll_id
ORDER BY n.poll_id) FROM nm_poll_voted_ids_user_t AS n
WHERE n.user_id = u.id
) AS poll_voted_ids,
(SELECT array_agg(o.id
ORDER BY o.id) FROM option_t AS o
WHERE o.content_object_id_user_id = u.id
) AS option_ids,
(SELECT array_agg(v.id
ORDER BY v.id) FROM vote_t AS v
...
(SELECT array_agg(v.id
ORDER BY v.id) FROM vote_t AS v
WHERE v.delegated_user_id = u.id
) AS delegated_vote_ids,
(SELECT array_agg(p.id
ORDER BY p.id) FROM poll_candidate_t AS p
WHERE p.user_id = u.id
) AS poll_candidate_ids,
(SELECT array_agg(h.id
ORDER BY h.id) FROM history_position_t AS h
WHERE h.user_id = u.id
) AS history_position_ids,
(SELECT array_agg(h.id
ORDER BY h.id) FROM history_entry_t AS h
WHERE h.model_id_user_id = u.id
) AS history_entry_ids,
(
  SELECT array_agg(DISTINCT mu.meeting_id
ORDER BY mu.meeting_id)
  FROM meeting_user_t AS mu
  WHERE mu.user_id = u.id
) AS meeting_ids
...
CREATE VIEW "vote" AS SELECT * FROM vote_t;


-- Alter table relations
ALTER TABLE agenda_item_t ADD CONSTRAINT fk_agenda_item_t_content_object_id_motion_id_motion_t_id
    FOREIGN KEY (content_object_id_motion_id) REFERENCES motion_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_agenda_item_t_content_object_id_motion_id ON agenda_item_t (
    content_object_id_motion_id
);
ALTER TABLE agenda_item_t ADD CONSTRAINT fk_agenda_item_t_content_object_id_motion_block_id_motiofc82ae2
    FOREIGN KEY (content_object_id_motion_block_id) REFERENCES motion_block_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_agenda_item_t_content_object_id_motion_block_id ON agenda_item_t (
    content_object_id_motion_block_id
);
ALTER TABLE agenda_item_t ADD CONSTRAINT fk_agenda_item_t_content_object_id_assignment_id_assignmd1e068c
    FOREIGN KEY (content_object_id_assignment_id) REFERENCES assignment_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_agenda_item_t_content_object_id_assignment_id ON agenda_item_t (
    content_object_id_assignment_id
);
ALTER TABLE agenda_item_t ADD CONSTRAINT fk_agenda_item_t_content_object_id_topic_id_topic_t_id
    FOREIGN KEY (content_object_id_topic_id) REFERENCES topic_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_agenda_item_t_content_object_id_topic_id ON agenda_item_t (
    content_object_id_topic_id
);
ALTER TABLE agenda_item_t ADD CONSTRAINT fk_agenda_item_t_parent_id_agenda_item_t_id
    FOREIGN KEY (parent_id) REFERENCES agenda_item_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_agenda_item_t_parent_id ON agenda_item_t (parent_id);
ALTER TABLE agenda_item_t ADD CONSTRAINT fk_agenda_item_t_meeting_id_meeting_t_id
    FOREIGN KEY (meeting_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_agenda_item_t_meeting_id ON agenda_item_t (meeting_id);

ALTER TABLE assignment_t ADD CONSTRAINT fk_assignment_t_meeting_id_meeting_t_id
    FOREIGN KEY (meeting_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_assignment_t_meeting_id ON assignment_t (meeting_id);

ALTER TABLE assignment_candidate_t ADD CONSTRAINT fk_assignment_candidate_t_assignment_id_assignment_t_id
    FOREIGN KEY (assignment_id) REFERENCES assignment_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_assignment_candidate_t_assignment_id ON assignment_candidate_t (
    assignment_id
);
ALTER TABLE assignment_candidate_t ADD CONSTRAINT fk_assignment_candidate_t_meeting_user_id_meeting_user_t_id
    FOREIGN KEY (meeting_user_id) REFERENCES meeting_user_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_assignment_candidate_t_meeting_user_id ON assignment_candidate_t (
    meeting_user_id
);
ALTER TABLE assignment_candidate_t ADD CONSTRAINT fk_assignment_candidate_t_meeting_id_meeting_t_id
    FOREIGN KEY (meeting_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_assignment_candidate_t_meeting_id ON assignment_candidate_t (
    meeting_id
);

ALTER TABLE chat_group_t ADD CONSTRAINT fk_chat_group_t_meeting_id_meeting_t_id
    FOREIGN KEY (meeting_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_chat_group_t_meeting_id ON chat_group_t (meeting_id);

ALTER TABLE chat_message_t ADD CONSTRAINT fk_chat_message_t_meeting_user_id_meeting_user_t_id
    FOREIGN KEY (meeting_user_id) REFERENCES meeting_user_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_chat_message_t_meeting_user_id ON chat_message_t (
    meeting_user_id
);
ALTER TABLE chat_message_t ADD CONSTRAINT fk_chat_message_t_chat_group_id_chat_group_t_id
    FOREIGN KEY (chat_group_id) REFERENCES chat_group_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_chat_message_t_chat_group_id ON chat_message_t (chat_group_id);
ALTER TABLE chat_message_t ADD CONSTRAINT fk_chat_message_t_meeting_id_meeting_t_id
    FOREIGN KEY (meeting_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_chat_message_t_meeting_id ON chat_message_t (meeting_id);

ALTER TABLE committee_t ADD CONSTRAINT fk_committee_t_default_meeting_id_meeting_t_id
    FOREIGN KEY (default_meeting_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_committee_t_default_meeting_id ON committee_t (
    default_meeting_id
);
ALTER TABLE committee_t ADD CONSTRAINT fk_committee_t_parent_id_committee_t_id
    FOREIGN KEY (parent_id) REFERENCES committee_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_committee_t_parent_id ON committee_t (parent_id);
ALTER TABLE committee_t ADD CONSTRAINT fk_committee_t_organization_id_organization_t_id
    FOREIGN KEY (organization_id) REFERENCES organization_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_committee_t_organization_id ON committee_t (organization_id);

ALTER TABLE gender_t ADD CONSTRAINT fk_gender_t_organization_id_organization_t_id
    FOREIGN KEY (organization_id) REFERENCES organization_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_gender_t_organization_id ON gender_t (organization_id);

ALTER TABLE group_t ADD CONSTRAINT fk_group_t_used_as_motion_poll_default_id_meeting_t_id
    FOREIGN KEY (used_as_motion_poll_default_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_group_t_used_as_motion_poll_default_id ON group_t (
    used_as_motion_poll_default_id
);
ALTER TABLE group_t ADD CONSTRAINT fk_group_t_used_as_assignment_poll_default_id_meeting_t_id
    FOREIGN KEY (used_as_assignment_poll_default_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_group_t_used_as_assignment_poll_default_id ON group_t (
    used_as_assignment_poll_default_id
);
ALTER TABLE group_t ADD CONSTRAINT fk_group_t_used_as_topic_poll_default_id_meeting_t_id
    FOREIGN KEY (used_as_topic_poll_default_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_group_t_used_as_topic_poll_default_id ON group_t (
    used_as_topic_poll_default_id
);
ALTER TABLE group_t ADD CONSTRAINT fk_group_t_used_as_poll_default_id_meeting_t_id
    FOREIGN KEY (used_as_poll_default_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_group_t_used_as_poll_default_id ON group_t (
    used_as_poll_default_id
);
ALTER TABLE group_t ADD CONSTRAINT fk_group_t_meeting_id_meeting_t_id
    FOREIGN KEY (meeting_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_group_t_meeting_id ON group_t (meeting_id);

ALTER TABLE history_entry_t ADD CONSTRAINT fk_history_entry_t_model_id_user_id_user_t_id
    FOREIGN KEY (model_id_user_id) REFERENCES user_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_history_entry_t_model_id_user_id ON history_entry_t (
    model_id_user_id
);
ALTER TABLE history_entry_t ADD CONSTRAINT fk_history_entry_t_model_id_motion_id_motion_t_id
    FOREIGN KEY (model_id_motion_id) REFERENCES motion_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_history_entry_t_model_id_motion_id ON history_entry_t (
    model_id_motion_id
);
ALTER TABLE history_entry_t ADD CONSTRAINT fk_history_entry_t_model_id_assignment_id_assignment_t_id
    FOREIGN KEY (model_id_assignment_id) REFERENCES assignment_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_history_entry_t_model_id_assignment_id ON history_entry_t (
    model_id_assignment_id
);
ALTER TABLE history_entry_t ADD CONSTRAINT fk_history_entry_t_position_id_history_position_t_id
    FOREIGN KEY (position_id) REFERENCES history_position_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_history_entry_t_position_id ON history_entry_t (position_id);
ALTER TABLE history_entry_t ADD CONSTRAINT fk_history_entry_t_meeting_id_meeting_t_id
    FOREIGN KEY (meeting_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_history_entry_t_meeting_id ON history_entry_t (meeting_id);

ALTER TABLE history_position_t ADD CONSTRAINT fk_history_position_t_user_id_user_t_id
    FOREIGN KEY (user_id) REFERENCES user_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_history_position_t_user_id ON history_position_t (user_id);

ALTER TABLE list_of_speakers_t ADD CONSTRAINT fk_list_of_speakers_t_content_object_id_motion_id_motion_t_id
    FOREIGN KEY (content_object_id_motion_id) REFERENCES motion_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_list_of_speakers_t_content_object_id_motion_id ON list_of_speakers_t (
    content_object_id_motion_id
);
ALTER TABLE list_of_speakers_t ADD CONSTRAINT fk_list_of_speakers_t_content_object_id_motion_block_id_62f90e7
    FOREIGN KEY (content_object_id_motion_block_id) REFERENCES motion_block_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_list_of_speakers_t_content_object_id_motion_block_id ON list_of_speakers_t (
    content_object_id_motion_block_id
);
ALTER TABLE list_of_speakers_t ADD CONSTRAINT fk_list_of_speakers_t_content_object_id_assignment_id_as9909666
    FOREIGN KEY (content_object_id_assignment_id) REFERENCES assignment_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_list_of_speakers_t_content_object_id_assignment_id ON list_of_speakers_t (
    content_object_id_assignment_id
);
ALTER TABLE list_of_speakers_t ADD CONSTRAINT fk_list_of_speakers_t_content_object_id_topic_id_topic_t_id
    FOREIGN KEY (content_object_id_topic_id) REFERENCES topic_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_list_of_speakers_t_content_object_id_topic_id ON list_of_speakers_t (
    content_object_id_topic_id
);
ALTER TABLE list_of_speakers_t ADD CONSTRAINT fk_list_of_speakers_t_content_object_id_meeting_mediafilc591d1a
    FOREIGN KEY (content_object_id_meeting_mediafile_id) REFERENCES meeting_mediafile_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_list_of_speakers_t_content_object_id_meeting_mediafile_id ON list_of_speakers_t (
    content_object_id_meeting_mediafile_id
);
ALTER TABLE list_of_speakers_t ADD CONSTRAINT fk_list_of_speakers_t_meeting_id_meeting_t_id
    FOREIGN KEY (meeting_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_list_of_speakers_t_meeting_id ON list_of_speakers_t (
    meeting_id
);

ALTER TABLE mediafile_t ADD CONSTRAINT fk_mediafile_t_published_to_meetings_in_organization_id_471fd9a
    FOREIGN KEY (published_to_meetings_in_organization_id) REFERENCES organization_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_mediafile_t_published_to_meetings_in_organization_id ON mediafile_t (
    published_to_meetings_in_organization_id
);
ALTER TABLE mediafile_t ADD CONSTRAINT fk_mediafile_t_parent_id_mediafile_t_id
    FOREIGN KEY (parent_id) REFERENCES mediafile_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_mediafile_t_parent_id ON mediafile_t (parent_id);
ALTER TABLE mediafile_t ADD CONSTRAINT fk_mediafile_t_owner_id_meeting_id_meeting_t_id
    FOREIGN KEY (owner_id_meeting_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_mediafile_t_owner_id_meeting_id ON mediafile_t (
    owner_id_meeting_id
);
ALTER TABLE mediafile_t ADD CONSTRAINT fk_mediafile_t_owner_id_organization_id_organization_t_id
    FOREIGN KEY (owner_id_organization_id) REFERENCES organization_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_mediafile_t_owner_id_organization_id ON mediafile_t (
    owner_id_organization_id
);

ALTER TABLE meeting_t ADD CONSTRAINT fk_meeting_t_is_active_in_organization_id_organization_t_id
    FOREIGN KEY (is_active_in_organization_id) REFERENCES organization_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_meeting_t_is_active_in_organization_id ON meeting_t (
    is_active_in_organization_id
);
ALTER TABLE meeting_t ADD CONSTRAINT fk_meeting_t_is_archived_in_organization_id_organization_t_id
    FOREIGN KEY (is_archived_in_organization_id) REFERENCES organization_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_meeting_t_is_archived_in_organization_id ON meeting_t (
    is_archived_in_organization_id
);
ALTER TABLE meeting_t ADD CONSTRAINT fk_meeting_t_template_for_organization_id_organization_t_id
    FOREIGN KEY (template_for_organization_id) REFERENCES organization_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_meeting_t_template_for_organization_id ON meeting_t (
    template_for_organization_id
);
ALTER TABLE meeting_t ADD CONSTRAINT fk_meeting_t_motions_default_workflow_id_motion_workflow_t_id
    FOREIGN KEY (motions_default_workflow_id) REFERENCES motion_workflow_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_meeting_t_motions_default_workflow_id ON meeting_t (
    motions_default_workflow_id
);
ALTER TABLE meeting_t ADD CONSTRAINT fk_meeting_t_motions_default_amendment_workflow_id_motio34ae2bf
    FOREIGN KEY (motions_default_amendment_workflow_id) REFERENCES motion_workflow_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_meeting_t_motions_default_amendment_workflow_id ON meeting_t (
    motions_default_amendment_workflow_id
);
ALTER TABLE meeting_t ADD CONSTRAINT fk_meeting_t_logo_projector_main_id_meeting_mediafile_t_id
    FOREIGN KEY (logo_projector_main_id) REFERENCES meeting_mediafile_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_meeting_t_logo_projector_main_id ON meeting_t (
    logo_projector_main_id
);
ALTER TABLE meeting_t ADD CONSTRAINT fk_meeting_t_logo_projector_header_id_meeting_mediafile_t_id
    FOREIGN KEY (logo_projector_header_id) REFERENCES meeting_mediafile_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_meeting_t_logo_projector_header_id ON meeting_t (
    logo_projector_header_id
);
ALTER TABLE meeting_t ADD CONSTRAINT fk_meeting_t_logo_web_header_id_meeting_mediafile_t_id
    FOREIGN KEY (logo_web_header_id) REFERENCES meeting_mediafile_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_meeting_t_logo_web_header_id ON meeting_t (logo_web_header_id);
ALTER TABLE meeting_t ADD CONSTRAINT fk_meeting_t_logo_pdf_header_l_id_meeting_mediafile_t_id
    FOREIGN KEY (logo_pdf_header_l_id) REFERENCES meeting_mediafile_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_meeting_t_logo_pdf_header_l_id ON meeting_t (
    logo_pdf_header_l_id
);
ALTER TABLE meeting_t ADD CONSTRAINT fk_meeting_t_logo_pdf_header_r_id_meeting_mediafile_t_id
    FOREIGN KEY (logo_pdf_header_r_id) REFERENCES meeting_mediafile_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_meeting_t_logo_pdf_header_r_id ON meeting_t (
    logo_pdf_header_r_id
);
ALTER TABLE meeting_t ADD CONSTRAINT fk_meeting_t_logo_pdf_footer_l_id_meeting_mediafile_t_id
    FOREIGN KEY (logo_pdf_footer_l_id) REFERENCES meeting_mediafile_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_meeting_t_logo_pdf_footer_l_id ON meeting_t (
    logo_pdf_footer_l_id
);
ALTER TABLE meeting_t ADD CONSTRAINT fk_meeting_t_logo_pdf_footer_r_id_meeting_mediafile_t_id
    FOREIGN KEY (logo_pdf_footer_r_id) REFERENCES meeting_mediafile_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_meeting_t_logo_pdf_footer_r_id ON meeting_t (
    logo_pdf_footer_r_id
);
ALTER TABLE meeting_t ADD CONSTRAINT fk_meeting_t_logo_pdf_ballot_paper_id_meeting_mediafile_t_id
    FOREIGN KEY (logo_pdf_ballot_paper_id) REFERENCES meeting_mediafile_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_meeting_t_logo_pdf_ballot_paper_id ON meeting_t (
    logo_pdf_ballot_paper_id
);
ALTER TABLE meeting_t ADD CONSTRAINT fk_meeting_t_font_regular_id_meeting_mediafile_t_id
    FOREIGN KEY (font_regular_id) REFERENCES meeting_mediafile_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_meeting_t_font_regular_id ON meeting_t (font_regular_id);
ALTER TABLE meeting_t ADD CONSTRAINT fk_meeting_t_font_italic_id_meeting_mediafile_t_id
    FOREIGN KEY (font_italic_id) REFERENCES meeting_mediafile_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_meeting_t_font_italic_id ON meeting_t (font_italic_id);
ALTER TABLE meeting_t ADD CONSTRAINT fk_meeting_t_font_bold_id_meeting_mediafile_t_id
    FOREIGN KEY (font_bold_id) REFERENCES meeting_mediafile_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_meeting_t_font_bold_id ON meeting_t (font_bold_id);
ALTER TABLE meeting_t ADD CONSTRAINT fk_meeting_t_font_bold_italic_id_meeting_mediafile_t_id
    FOREIGN KEY (font_bold_italic_id) REFERENCES meeting_mediafile_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_meeting_t_font_bold_italic_id ON meeting_t (
    font_bold_italic_id
);
ALTER TABLE meeting_t ADD CONSTRAINT fk_meeting_t_font_monospace_id_meeting_mediafile_t_id
    FOREIGN KEY (font_monospace_id) REFERENCES meeting_mediafile_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_meeting_t_font_monospace_id ON meeting_t (font_monospace_id);
ALTER TABLE meeting_t ADD CONSTRAINT fk_meeting_t_font_chyron_speaker_name_id_meeting_mediafile_t_id
    FOREIGN KEY (font_chyron_speaker_name_id) REFERENCES meeting_mediafile_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_meeting_t_font_chyron_speaker_name_id ON meeting_t (
    font_chyron_speaker_name_id
);
ALTER TABLE meeting_t ADD CONSTRAINT fk_meeting_t_font_projector_h1_id_meeting_mediafile_t_id
    FOREIGN KEY (font_projector_h1_id) REFERENCES meeting_mediafile_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_meeting_t_font_projector_h1_id ON meeting_t (
    font_projector_h1_id
);
ALTER TABLE meeting_t ADD CONSTRAINT fk_meeting_t_font_projector_h2_id_meeting_mediafile_t_id
    FOREIGN KEY (font_projector_h2_id) REFERENCES meeting_mediafile_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_meeting_t_font_projector_h2_id ON meeting_t (
    font_projector_h2_id
);
ALTER TABLE meeting_t ADD CONSTRAINT fk_meeting_t_committee_id_committee_t_id
    FOREIGN KEY (committee_id) REFERENCES committee_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_meeting_t_committee_id ON meeting_t (committee_id);
ALTER TABLE meeting_t ADD CONSTRAINT fk_meeting_t_reference_projector_id_projector_t_id
    FOREIGN KEY (reference_projector_id) REFERENCES projector_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_meeting_t_reference_projector_id ON meeting_t (
    reference_projector_id
);
ALTER TABLE meeting_t ADD CONSTRAINT fk_meeting_t_list_of_speakers_countdown_id_projector_cou566cfb1
    FOREIGN KEY (list_of_speakers_countdown_id) REFERENCES projector_countdown_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_meeting_t_list_of_speakers_countdown_id ON meeting_t (
    list_of_speakers_countdown_id
);
ALTER TABLE meeting_t ADD CONSTRAINT fk_meeting_t_poll_countdown_id_projector_countdown_t_id
    FOREIGN KEY (poll_countdown_id) REFERENCES projector_countdown_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_meeting_t_poll_countdown_id ON meeting_t (poll_countdown_id);
ALTER TABLE meeting_t ADD CONSTRAINT fk_meeting_t_default_group_id_group_t_id
    FOREIGN KEY (default_group_id) REFERENCES group_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_meeting_t_default_group_id ON meeting_t (default_group_id);
ALTER TABLE meeting_t ADD CONSTRAINT fk_meeting_t_admin_group_id_group_t_id
    FOREIGN KEY (admin_group_id) REFERENCES group_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_meeting_t_admin_group_id ON meeting_t (admin_group_id);
ALTER TABLE meeting_t ADD CONSTRAINT fk_meeting_t_anonymous_group_id_group_t_id
    FOREIGN KEY (anonymous_group_id) REFERENCES group_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_meeting_t_anonymous_group_id ON meeting_t (anonymous_group_id);

ALTER TABLE meeting_mediafile_t ADD CONSTRAINT fk_meeting_mediafile_t_mediafile_id_mediafile_t_id
    FOREIGN KEY (mediafile_id) REFERENCES mediafile_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_meeting_mediafile_t_mediafile_id ON meeting_mediafile_t (
    mediafile_id
);
ALTER TABLE meeting_mediafile_t ADD CONSTRAINT fk_meeting_mediafile_t_meeting_id_meeting_t_id
    FOREIGN KEY (meeting_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_meeting_mediafile_t_meeting_id ON meeting_mediafile_t (
    meeting_id
);

ALTER TABLE meeting_user_t ADD CONSTRAINT fk_meeting_user_t_user_id_user_t_id
    FOREIGN KEY (user_id) REFERENCES user_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_meeting_user_t_user_id ON meeting_user_t (user_id);
ALTER TABLE meeting_user_t ADD CONSTRAINT fk_meeting_user_t_meeting_id_meeting_t_id
    FOREIGN KEY (meeting_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_meeting_user_t_meeting_id ON meeting_user_t (meeting_id);
ALTER TABLE meeting_user_t ADD CONSTRAINT fk_meeting_user_t_vote_delegated_to_id_meeting_user_t_id
    FOREIGN KEY (vote_delegated_to_id) REFERENCES meeting_user_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_meeting_user_t_vote_delegated_to_id ON meeting_user_t (
    vote_delegated_to_id
);

ALTER TABLE motion_t ADD CONSTRAINT fk_motion_t_lead_motion_id_motion_t_id
    FOREIGN KEY (lead_motion_id) REFERENCES motion_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_motion_t_lead_motion_id ON motion_t (lead_motion_id);
ALTER TABLE motion_t ADD CONSTRAINT fk_motion_t_sort_parent_id_motion_t_id
    FOREIGN KEY (sort_parent_id) REFERENCES motion_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_motion_t_sort_parent_id ON motion_t (sort_parent_id);
ALTER TABLE motion_t ADD CONSTRAINT fk_motion_t_origin_id_motion_t_id
    FOREIGN KEY (origin_id) REFERENCES motion_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_motion_t_origin_id ON motion_t (origin_id);
ALTER TABLE motion_t ADD CONSTRAINT fk_motion_t_origin_meeting_id_meeting_t_id
    FOREIGN KEY (origin_meeting_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_motion_t_origin_meeting_id ON motion_t (origin_meeting_id);
ALTER TABLE motion_t ADD CONSTRAINT fk_motion_t_state_id_motion_state_t_id
    FOREIGN KEY (state_id) REFERENCES motion_state_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_motion_t_state_id ON motion_t (state_id);
ALTER TABLE motion_t ADD CONSTRAINT fk_motion_t_recommendation_id_motion_state_t_id
    FOREIGN KEY (recommendation_id) REFERENCES motion_state_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_motion_t_recommendation_id ON motion_t (recommendation_id);
ALTER TABLE motion_t ADD CONSTRAINT fk_motion_t_category_id_motion_category_t_id
    FOREIGN KEY (category_id) REFERENCES motion_category_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_motion_t_category_id ON motion_t (category_id);
ALTER TABLE motion_t ADD CONSTRAINT fk_motion_t_block_id_motion_block_t_id
    FOREIGN KEY (block_id) REFERENCES motion_block_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_motion_t_block_id ON motion_t (block_id);
ALTER TABLE motion_t ADD CONSTRAINT fk_motion_t_meeting_id_meeting_t_id
    FOREIGN KEY (meeting_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_motion_t_meeting_id ON motion_t (meeting_id);

ALTER TABLE motion_block_t ADD CONSTRAINT fk_motion_block_t_meeting_id_meeting_t_id
    FOREIGN KEY (meeting_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_motion_block_t_meeting_id ON motion_block_t (meeting_id);

ALTER TABLE motion_category_t ADD CONSTRAINT fk_motion_category_t_parent_id_motion_category_t_id
    FOREIGN KEY (parent_id) REFERENCES motion_category_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_motion_category_t_parent_id ON motion_category_t (parent_id);
ALTER TABLE motion_category_t ADD CONSTRAINT fk_motion_category_t_meeting_id_meeting_t_id
    FOREIGN KEY (meeting_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_motion_category_t_meeting_id ON motion_category_t (meeting_id);

ALTER TABLE motion_change_recommendation_t ADD CONSTRAINT fk_motion_change_recommendation_t_motion_id_motion_t_id
    FOREIGN KEY (motion_id) REFERENCES motion_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_motion_change_recommendation_t_motion_id ON motion_change_recommendation_t (
    motion_id
);
ALTER TABLE motion_change_recommendation_t ADD CONSTRAINT fk_motion_change_recommendation_t_meeting_id_meeting_t_id
    FOREIGN KEY (meeting_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_motion_change_recommendation_t_meeting_id ON motion_change_recommendation_t (
    meeting_id
);

ALTER TABLE motion_comment_t ADD CONSTRAINT fk_motion_comment_t_motion_id_motion_t_id
    FOREIGN KEY (motion_id) REFERENCES motion_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_motion_comment_t_motion_id ON motion_comment_t (motion_id);
ALTER TABLE motion_comment_t ADD CONSTRAINT fk_motion_comment_t_section_id_motion_comment_section_t_id
    FOREIGN KEY (section_id) REFERENCES motion_comment_section_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_motion_comment_t_section_id ON motion_comment_t (section_id);
ALTER TABLE motion_comment_t ADD CONSTRAINT fk_motion_comment_t_meeting_id_meeting_t_id
    FOREIGN KEY (meeting_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_motion_comment_t_meeting_id ON motion_comment_t (meeting_id);

ALTER TABLE motion_comment_section_t ADD CONSTRAINT fk_motion_comment_section_t_meeting_id_meeting_t_id
    FOREIGN KEY (meeting_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_motion_comment_section_t_meeting_id ON motion_comment_section_t (
    meeting_id
);

ALTER TABLE motion_editor_t ADD CONSTRAINT fk_motion_editor_t_meeting_user_id_meeting_user_t_id
    FOREIGN KEY (meeting_user_id) REFERENCES meeting_user_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_motion_editor_t_meeting_user_id ON motion_editor_t (
    meeting_user_id
);
ALTER TABLE motion_editor_t ADD CONSTRAINT fk_motion_editor_t_motion_id_motion_t_id
    FOREIGN KEY (motion_id) REFERENCES motion_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_motion_editor_t_motion_id ON motion_editor_t (motion_id);
ALTER TABLE motion_editor_t ADD CONSTRAINT fk_motion_editor_t_meeting_id_meeting_t_id
    FOREIGN KEY (meeting_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_motion_editor_t_meeting_id ON motion_editor_t (meeting_id);

ALTER TABLE motion_state_t ADD CONSTRAINT fk_motion_state_t_submitter_withdraw_state_id_motion_state_t_id
    FOREIGN KEY (submitter_withdraw_state_id) REFERENCES motion_state_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_motion_state_t_submitter_withdraw_state_id ON motion_state_t (
    submitter_withdraw_state_id
);
ALTER TABLE motion_state_t ADD CONSTRAINT fk_motion_state_t_workflow_id_motion_workflow_t_id
    FOREIGN KEY (workflow_id) REFERENCES motion_workflow_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_motion_state_t_workflow_id ON motion_state_t (workflow_id);
ALTER TABLE motion_state_t ADD CONSTRAINT fk_motion_state_t_meeting_id_meeting_t_id
    FOREIGN KEY (meeting_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_motion_state_t_meeting_id ON motion_state_t (meeting_id);

ALTER TABLE motion_submitter_t ADD CONSTRAINT fk_motion_submitter_t_meeting_user_id_meeting_user_t_id
    FOREIGN KEY (meeting_user_id) REFERENCES meeting_user_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_motion_submitter_t_meeting_user_id ON motion_submitter_t (
    meeting_user_id
);
ALTER TABLE motion_submitter_t ADD CONSTRAINT fk_motion_submitter_t_motion_id_motion_t_id
    FOREIGN KEY (motion_id) REFERENCES motion_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_motion_submitter_t_motion_id ON motion_submitter_t (motion_id);
ALTER TABLE motion_submitter_t ADD CONSTRAINT fk_motion_submitter_t_meeting_id_meeting_t_id
    FOREIGN KEY (meeting_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_motion_submitter_t_meeting_id ON motion_submitter_t (
    meeting_id
);

ALTER TABLE motion_supporter_t ADD CONSTRAINT fk_motion_supporter_t_meeting_user_id_meeting_user_t_id
    FOREIGN KEY (meeting_user_id) REFERENCES meeting_user_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_motion_supporter_t_meeting_user_id ON motion_supporter_t (
    meeting_user_id
);
ALTER TABLE motion_supporter_t ADD CONSTRAINT fk_motion_supporter_t_motion_id_motion_t_id
    FOREIGN KEY (motion_id) REFERENCES motion_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_motion_supporter_t_motion_id ON motion_supporter_t (motion_id);
ALTER TABLE motion_supporter_t ADD CONSTRAINT fk_motion_supporter_t_meeting_id_meeting_t_id
    FOREIGN KEY (meeting_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_motion_supporter_t_meeting_id ON motion_supporter_t (
    meeting_id
);

ALTER TABLE motion_workflow_t ADD CONSTRAINT fk_motion_workflow_t_first_state_id_motion_state_t_id
    FOREIGN KEY (first_state_id) REFERENCES motion_state_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_motion_workflow_t_first_state_id ON motion_workflow_t (
    first_state_id
);
ALTER TABLE motion_workflow_t ADD CONSTRAINT fk_motion_workflow_t_meeting_id_meeting_t_id
    FOREIGN KEY (meeting_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_motion_workflow_t_meeting_id ON motion_workflow_t (meeting_id);

ALTER TABLE motion_working_group_speaker_t ADD CONSTRAINT fk_motion_working_group_speaker_t_meeting_user_id_meetinbc3a7bb
    FOREIGN KEY (meeting_user_id) REFERENCES meeting_user_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_motion_working_group_speaker_t_meeting_user_id ON motion_working_group_speaker_t (
    meeting_user_id
);
ALTER TABLE motion_working_group_speaker_t ADD CONSTRAINT fk_motion_working_group_speaker_t_motion_id_motion_t_id
    FOREIGN KEY (motion_id) REFERENCES motion_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_motion_working_group_speaker_t_motion_id ON motion_working_group_speaker_t (
    motion_id
);
ALTER TABLE motion_working_group_speaker_t ADD CONSTRAINT fk_motion_working_group_speaker_t_meeting_id_meeting_t_id
    FOREIGN KEY (meeting_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_motion_working_group_speaker_t_meeting_id ON motion_working_group_speaker_t (
    meeting_id
);

ALTER TABLE option_t ADD CONSTRAINT fk_option_t_poll_id_poll_t_id
    FOREIGN KEY (poll_id) REFERENCES poll_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_option_t_poll_id ON option_t (poll_id);
ALTER TABLE option_t ADD CONSTRAINT fk_option_t_content_object_id_motion_id_motion_t_id
    FOREIGN KEY (content_object_id_motion_id) REFERENCES motion_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_option_t_content_object_id_motion_id ON option_t (
    content_object_id_motion_id
);
ALTER TABLE option_t ADD CONSTRAINT fk_option_t_content_object_id_user_id_user_t_id
    FOREIGN KEY (content_object_id_user_id) REFERENCES user_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_option_t_content_object_id_user_id ON option_t (
    content_object_id_user_id
);
ALTER TABLE option_t ADD CONSTRAINT fk_option_t_content_object_id_poll_candidate_list_id_pold428251
    FOREIGN KEY (content_object_id_poll_candidate_list_id) REFERENCES poll_candidate_list_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_option_t_content_object_id_poll_candidate_list_id ON option_t (
    content_object_id_poll_candidate_list_id
);
ALTER TABLE option_t ADD CONSTRAINT fk_option_t_meeting_id_meeting_t_id
    FOREIGN KEY (meeting_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_option_t_meeting_id ON option_t (meeting_id);

ALTER TABLE organization_t ADD CONSTRAINT fk_organization_t_theme_id_theme_t_id
    FOREIGN KEY (theme_id) REFERENCES theme_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_organization_t_theme_id ON organization_t (theme_id);

ALTER TABLE organization_tag_t ADD CONSTRAINT fk_organization_tag_t_organization_id_organization_t_id
    FOREIGN KEY (organization_id) REFERENCES organization_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_organization_tag_t_organization_id ON organization_tag_t (
    organization_id
);

ALTER TABLE personal_note_t ADD CONSTRAINT fk_personal_note_t_meeting_user_id_meeting_user_t_id
    FOREIGN KEY (meeting_user_id) REFERENCES meeting_user_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_personal_note_t_meeting_user_id ON personal_note_t (
    meeting_user_id
);
ALTER TABLE personal_note_t ADD CONSTRAINT fk_personal_note_t_content_object_id_motion_id_motion_t_id
    FOREIGN KEY (content_object_id_motion_id) REFERENCES motion_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_personal_note_t_content_object_id_motion_id ON personal_note_t (
    content_object_id_motion_id
);
ALTER TABLE personal_note_t ADD CONSTRAINT fk_personal_note_t_meeting_id_meeting_t_id
    FOREIGN KEY (meeting_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_personal_note_t_meeting_id ON personal_note_t (meeting_id);

ALTER TABLE point_of_order_category_t ADD CONSTRAINT fk_point_of_order_category_t_meeting_id_meeting_t_id
    FOREIGN KEY (meeting_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_point_of_order_category_t_meeting_id ON point_of_order_category_t (
    meeting_id
);

ALTER TABLE poll_t ADD CONSTRAINT fk_poll_t_content_object_id_motion_id_motion_t_id
    FOREIGN KEY (content_object_id_motion_id) REFERENCES motion_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_poll_t_content_object_id_motion_id ON poll_t (
    content_object_id_motion_id
);
ALTER TABLE poll_t ADD CONSTRAINT fk_poll_t_content_object_id_assignment_id_assignment_t_id
    FOREIGN KEY (content_object_id_assignment_id) REFERENCES assignment_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_poll_t_content_object_id_assignment_id ON poll_t (
    content_object_id_assignment_id
);
ALTER TABLE poll_t ADD CONSTRAINT fk_poll_t_content_object_id_topic_id_topic_t_id
    FOREIGN KEY (content_object_id_topic_id) REFERENCES topic_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_poll_t_content_object_id_topic_id ON poll_t (
    content_object_id_topic_id
);
ALTER TABLE poll_t ADD CONSTRAINT fk_poll_t_global_option_id_option_t_id
    FOREIGN KEY (global_option_id) REFERENCES option_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_poll_t_global_option_id ON poll_t (global_option_id);
ALTER TABLE poll_t ADD CONSTRAINT fk_poll_t_meeting_id_meeting_t_id
    FOREIGN KEY (meeting_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_poll_t_meeting_id ON poll_t (meeting_id);

ALTER TABLE poll_candidate_t ADD CONSTRAINT fk_poll_candidate_t_poll_candidate_list_id_poll_candidat7fec070
    FOREIGN KEY (poll_candidate_list_id) REFERENCES poll_candidate_list_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_poll_candidate_t_poll_candidate_list_id ON poll_candidate_t (
    poll_candidate_list_id
);
ALTER TABLE poll_candidate_t ADD CONSTRAINT fk_poll_candidate_t_user_id_user_t_id
    FOREIGN KEY (user_id) REFERENCES user_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_poll_candidate_t_user_id ON poll_candidate_t (user_id);
ALTER TABLE poll_candidate_t ADD CONSTRAINT fk_poll_candidate_t_meeting_id_meeting_t_id
    FOREIGN KEY (meeting_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_poll_candidate_t_meeting_id ON poll_candidate_t (meeting_id);

ALTER TABLE poll_candidate_list_t ADD CONSTRAINT fk_poll_candidate_list_t_meeting_id_meeting_t_id
    FOREIGN KEY (meeting_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_poll_candidate_list_t_meeting_id ON poll_candidate_list_t (
    meeting_id
);

ALTER TABLE projection_t ADD CONSTRAINT fk_projection_t_current_projector_id_projector_t_id
    FOREIGN KEY (current_projector_id) REFERENCES projector_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_projection_t_current_projector_id ON projection_t (
    current_projector_id
);
ALTER TABLE projection_t ADD CONSTRAINT fk_projection_t_preview_projector_id_projector_t_id
    FOREIGN KEY (preview_projector_id) REFERENCES projector_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_projection_t_preview_projector_id ON projection_t (
    preview_projector_id
);
ALTER TABLE projection_t ADD CONSTRAINT fk_projection_t_history_projector_id_projector_t_id
    FOREIGN KEY (history_projector_id) REFERENCES projector_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_projection_t_history_projector_id ON projection_t (
    history_projector_id
);
ALTER TABLE projection_t ADD CONSTRAINT fk_projection_t_content_object_id_meeting_id_meeting_t_id
    FOREIGN KEY (content_object_id_meeting_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_projection_t_content_object_id_meeting_id ON projection_t (
    content_object_id_meeting_id
);
ALTER TABLE projection_t ADD CONSTRAINT fk_projection_t_content_object_id_motion_id_motion_t_id
    FOREIGN KEY (content_object_id_motion_id) REFERENCES motion_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_projection_t_content_object_id_motion_id ON projection_t (
    content_object_id_motion_id
);
ALTER TABLE projection_t ADD CONSTRAINT fk_projection_t_content_object_id_meeting_mediafile_id_m09270d6
    FOREIGN KEY (content_object_id_meeting_mediafile_id) REFERENCES meeting_mediafile_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_projection_t_content_object_id_meeting_mediafile_id ON projection_t (
    content_object_id_meeting_mediafile_id
);
ALTER TABLE projection_t ADD CONSTRAINT fk_projection_t_content_object_id_list_of_speakers_id_li392b5e1
    FOREIGN KEY (content_object_id_list_of_speakers_id) REFERENCES list_of_speakers_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_projection_t_content_object_id_list_of_speakers_id ON projection_t (
    content_object_id_list_of_speakers_id
);
ALTER TABLE projection_t ADD CONSTRAINT fk_projection_t_content_object_id_motion_block_id_motioncbb3c5b
    FOREIGN KEY (content_object_id_motion_block_id) REFERENCES motion_block_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_projection_t_content_object_id_motion_block_id ON projection_t (
    content_object_id_motion_block_id
);
ALTER TABLE projection_t ADD CONSTRAINT fk_projection_t_content_object_id_assignment_id_assignment_t_id
    FOREIGN KEY (content_object_id_assignment_id) REFERENCES assignment_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_projection_t_content_object_id_assignment_id ON projection_t (
    content_object_id_assignment_id
);
ALTER TABLE projection_t ADD CONSTRAINT fk_projection_t_content_object_id_agenda_item_id_agenda_bd5d814
    FOREIGN KEY (content_object_id_agenda_item_id) REFERENCES agenda_item_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_projection_t_content_object_id_agenda_item_id ON projection_t (
    content_object_id_agenda_item_id
);
ALTER TABLE projection_t ADD CONSTRAINT fk_projection_t_content_object_id_topic_id_topic_t_id
    FOREIGN KEY (content_object_id_topic_id) REFERENCES topic_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_projection_t_content_object_id_topic_id ON projection_t (
    content_object_id_topic_id
);
ALTER TABLE projection_t ADD CONSTRAINT fk_projection_t_content_object_id_poll_id_poll_t_id
    FOREIGN KEY (content_object_id_poll_id) REFERENCES poll_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_projection_t_content_object_id_poll_id ON projection_t (
    content_object_id_poll_id
);
ALTER TABLE projection_t ADD CONSTRAINT fk_projection_t_content_object_id_projector_message_id_p5c0a05e
    FOREIGN KEY (content_object_id_projector_message_id) REFERENCES projector_message_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_projection_t_content_object_id_projector_message_id ON projection_t (
    content_object_id_projector_message_id
);
ALTER TABLE projection_t ADD CONSTRAINT fk_projection_t_content_object_id_projector_countdown_id1942f3a
    FOREIGN KEY (content_object_id_projector_countdown_id) REFERENCES projector_countdown_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_projection_t_content_object_id_projector_countdown_id ON projection_t (
    content_object_id_projector_countdown_id
);
ALTER TABLE projection_t ADD CONSTRAINT fk_projection_t_meeting_id_meeting_t_id
    FOREIGN KEY (meeting_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_projection_t_meeting_id ON projection_t (meeting_id);

ALTER TABLE projector_t ADD CONSTRAINT fk_projector_t_used_as_default_projector_for_agenda_itemca4cc75
    FOREIGN KEY (used_as_default_projector_for_agenda_item_list_in_meeting_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_projector_t_used_as_default_projector_for_agenda_ite41b5bd9 ON projector_t (
    used_as_default_projector_for_agenda_item_list_in_meeting_id
);
ALTER TABLE projector_t ADD CONSTRAINT fk_projector_t_used_as_default_projector_for_topic_in_me0971ea7
    FOREIGN KEY (used_as_default_projector_for_topic_in_meeting_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_projector_t_used_as_default_projector_for_topic_in_mab361e8 ON projector_t (
    used_as_default_projector_for_topic_in_meeting_id
);
ALTER TABLE projector_t ADD CONSTRAINT fk_projector_t_used_as_default_projector_for_list_of_spe619d36f
    FOREIGN KEY (used_as_default_projector_for_list_of_speakers_in_meeting_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_projector_t_used_as_default_projector_for_list_of_spb099704 ON projector_t (
    used_as_default_projector_for_list_of_speakers_in_meeting_id
);
ALTER TABLE projector_t ADD CONSTRAINT fk_projector_t_used_as_default_projector_for_current_lose8cefef
    FOREIGN KEY (used_as_default_projector_for_current_los_in_meeting_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_projector_t_used_as_default_projector_for_current_locdd5b27 ON projector_t (
    used_as_default_projector_for_current_los_in_meeting_id
);
ALTER TABLE projector_t ADD CONSTRAINT fk_projector_t_used_as_default_projector_for_motion_in_m3a0e1e4
    FOREIGN KEY (used_as_default_projector_for_motion_in_meeting_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_projector_t_used_as_default_projector_for_motion_in_f66691f ON projector_t (
    used_as_default_projector_for_motion_in_meeting_id
);
ALTER TABLE projector_t ADD CONSTRAINT fk_projector_t_used_as_default_projector_for_amendment_i4b417bf
    FOREIGN KEY (used_as_default_projector_for_amendment_in_meeting_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_projector_t_used_as_default_projector_for_amendment_2235e12 ON projector_t (
    used_as_default_projector_for_amendment_in_meeting_id
);
ALTER TABLE projector_t ADD CONSTRAINT fk_projector_t_used_as_default_projector_for_motion_blocd05a71e
    FOREIGN KEY (used_as_default_projector_for_motion_block_in_meeting_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_projector_t_used_as_default_projector_for_motion_blo7b0c3c8 ON projector_t (
    used_as_default_projector_for_motion_block_in_meeting_id
);
ALTER TABLE projector_t ADD CONSTRAINT fk_projector_t_used_as_default_projector_for_assignment_ddcffde
    FOREIGN KEY (used_as_default_projector_for_assignment_in_meeting_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_projector_t_used_as_default_projector_for_assignment78622ce ON projector_t (
    used_as_default_projector_for_assignment_in_meeting_id
);
ALTER TABLE projector_t ADD CONSTRAINT fk_projector_t_used_as_default_projector_for_mediafile_i9b33fdb
    FOREIGN KEY (used_as_default_projector_for_mediafile_in_meeting_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_projector_t_used_as_default_projector_for_mediafile_1c31a99 ON projector_t (
    used_as_default_projector_for_mediafile_in_meeting_id
);
ALTER TABLE projector_t ADD CONSTRAINT fk_projector_t_used_as_default_projector_for_message_in_21dcbf0
    FOREIGN KEY (used_as_default_projector_for_message_in_meeting_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_projector_t_used_as_default_projector_for_message_in339958e ON projector_t (
    used_as_default_projector_for_message_in_meeting_id
);
ALTER TABLE projector_t ADD CONSTRAINT fk_projector_t_used_as_default_projector_for_countdown_if9b333e
    FOREIGN KEY (used_as_default_projector_for_countdown_in_meeting_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_projector_t_used_as_default_projector_for_countdown_07665b4 ON projector_t (
    used_as_default_projector_for_countdown_in_meeting_id
);
ALTER TABLE projector_t ADD CONSTRAINT fk_projector_t_used_as_default_projector_for_assignment_8b61ac6
    FOREIGN KEY (used_as_default_projector_for_assignment_poll_in_meeting_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_projector_t_used_as_default_projector_for_assignment5a08584 ON projector_t (
    used_as_default_projector_for_assignment_poll_in_meeting_id
);
ALTER TABLE projector_t ADD CONSTRAINT fk_projector_t_used_as_default_projector_for_motion_poll5bff78e
    FOREIGN KEY (used_as_default_projector_for_motion_poll_in_meeting_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_projector_t_used_as_default_projector_for_motion_pol0605aa8 ON projector_t (
    used_as_default_projector_for_motion_poll_in_meeting_id
);
ALTER TABLE projector_t ADD CONSTRAINT fk_projector_t_used_as_default_projector_for_poll_in_mee417a148
    FOREIGN KEY (used_as_default_projector_for_poll_in_meeting_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_projector_t_used_as_default_projector_for_poll_in_me446bdba ON projector_t (
    used_as_default_projector_for_poll_in_meeting_id
);
ALTER TABLE projector_t ADD CONSTRAINT fk_projector_t_meeting_id_meeting_t_id
    FOREIGN KEY (meeting_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_projector_t_meeting_id ON projector_t (meeting_id);

ALTER TABLE projector_countdown_t ADD CONSTRAINT fk_projector_countdown_t_meeting_id_meeting_t_id
    FOREIGN KEY (meeting_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_projector_countdown_t_meeting_id ON projector_countdown_t (
    meeting_id
);

ALTER TABLE projector_message_t ADD CONSTRAINT fk_projector_message_t_meeting_id_meeting_t_id
    FOREIGN KEY (meeting_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_projector_message_t_meeting_id ON projector_message_t (
    meeting_id
);

ALTER TABLE speaker_t ADD CONSTRAINT fk_speaker_t_list_of_speakers_id_list_of_speakers_t_id
    FOREIGN KEY (list_of_speakers_id) REFERENCES list_of_speakers_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_speaker_t_list_of_speakers_id ON speaker_t (
    list_of_speakers_id
);
ALTER TABLE speaker_t ADD CONSTRAINT fk_speaker_t_structure_level_list_of_speakers_id_structu559f22d
    FOREIGN KEY (structure_level_list_of_speakers_id) REFERENCES structure_level_list_of_speakers_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_speaker_t_structure_level_list_of_speakers_id ON speaker_t (
    structure_level_list_of_speakers_id
);
ALTER TABLE speaker_t ADD CONSTRAINT fk_speaker_t_meeting_user_id_meeting_user_t_id
    FOREIGN KEY (meeting_user_id) REFERENCES meeting_user_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_speaker_t_meeting_user_id ON speaker_t (meeting_user_id);
ALTER TABLE speaker_t ADD CONSTRAINT fk_speaker_t_point_of_order_category_id_point_of_order_ce6dbc9a
    FOREIGN KEY (point_of_order_category_id) REFERENCES point_of_order_category_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_speaker_t_point_of_order_category_id ON speaker_t (
    point_of_order_category_id
);
ALTER TABLE speaker_t ADD CONSTRAINT fk_speaker_t_meeting_id_meeting_t_id
    FOREIGN KEY (meeting_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_speaker_t_meeting_id ON speaker_t (meeting_id);

ALTER TABLE structure_level_t ADD CONSTRAINT fk_structure_level_t_meeting_id_meeting_t_id
    FOREIGN KEY (meeting_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_structure_level_t_meeting_id ON structure_level_t (meeting_id);

ALTER TABLE structure_level_list_of_speakers_t ADD CONSTRAINT fk_structure_level_list_of_speakers_t_structure_level_idee3e20c
    FOREIGN KEY (structure_level_id) REFERENCES structure_level_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_structure_level_list_of_speakers_t_structure_level_id ON structure_level_list_of_speakers_t (
    structure_level_id
);
ALTER TABLE structure_level_list_of_speakers_t ADD CONSTRAINT fk_structure_level_list_of_speakers_t_list_of_speakers_idbd2794
    FOREIGN KEY (list_of_speakers_id) REFERENCES list_of_speakers_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_structure_level_list_of_speakers_t_list_of_speakers_id ON structure_level_list_of_speakers_t (
    list_of_speakers_id
);
ALTER TABLE structure_level_list_of_speakers_t ADD CONSTRAINT fk_structure_level_list_of_speakers_t_meeting_id_meeting_t_id
    FOREIGN KEY (meeting_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_structure_level_list_of_speakers_t_meeting_id ON structure_level_list_of_speakers_t (
    meeting_id
);

ALTER TABLE tag_t ADD CONSTRAINT fk_tag_t_meeting_id_meeting_t_id
    FOREIGN KEY (meeting_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_tag_t_meeting_id ON tag_t (meeting_id);

ALTER TABLE theme_t ADD CONSTRAINT fk_theme_t_organization_id_organization_t_id
    FOREIGN KEY (organization_id) REFERENCES organization_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_theme_t_organization_id ON theme_t (organization_id);

ALTER TABLE topic_t ADD CONSTRAINT fk_topic_t_meeting_id_meeting_t_id
    FOREIGN KEY (meeting_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_topic_t_meeting_id ON topic_t (meeting_id);

ALTER TABLE user_t ADD CONSTRAINT fk_user_t_gender_id_gender_t_id
    FOREIGN KEY (gender_id) REFERENCES gender_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_user_t_gender_id ON user_t (gender_id);
ALTER TABLE user_t ADD CONSTRAINT fk_user_t_home_committee_id_committee_t_id
    FOREIGN KEY (home_committee_id) REFERENCES committee_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_user_t_home_committee_id ON user_t (home_committee_id);
ALTER TABLE user_t ADD CONSTRAINT fk_user_t_organization_id_organization_t_id
    FOREIGN KEY (organization_id) REFERENCES organization_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_user_t_organization_id ON user_t (organization_id);

ALTER TABLE vote_t ADD CONSTRAINT fk_vote_t_option_id_option_t_id
    FOREIGN KEY (option_id) REFERENCES option_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_vote_t_option_id ON vote_t (option_id);
ALTER TABLE vote_t ADD CONSTRAINT fk_vote_t_user_id_user_t_id
    FOREIGN KEY (user_id) REFERENCES user_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_vote_t_user_id ON vote_t (user_id);
ALTER TABLE vote_t ADD CONSTRAINT fk_vote_t_delegated_user_id_user_t_id
    FOREIGN KEY (delegated_user_id) REFERENCES user_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_vote_t_delegated_user_id ON vote_t (delegated_user_id);
ALTER TABLE vote_t ADD CONSTRAINT fk_vote_t_meeting_id_meeting_t_id
    FOREIGN KEY (meeting_id) REFERENCES meeting_t(id) INITIALLY DEFERRED;
...
FOR EACH ROW EXECUTE FUNCTION generate_sequence(
    'assignment_t', 'sequential_number', 'meeting_id'
);
...
FOR EACH ROW EXECUTE FUNCTION generate_sequence(
    'list_of_speakers_t', 'sequential_number', 'meeting_id'
);
...
FOR EACH ROW EXECUTE FUNCTION generate_sequence(
    'motion_t', 'sequential_number', 'meeting_id'
);
...
FOR EACH ROW EXECUTE FUNCTION generate_sequence(
    'motion_block_t', 'sequential_number', 'meeting_id'
);
...
FOR EACH ROW EXECUTE FUNCTION generate_sequence(
    'motion_category_t', 'sequential_number', 'meeting_id'
);
...
FOR EACH ROW EXECUTE FUNCTION generate_sequence(
    'motion_comment_section_t', 'sequential_number', 'meeting_id'
);
...
FOR EACH ROW EXECUTE FUNCTION generate_sequence(
    'motion_workflow_t', 'sequential_number', 'meeting_id'
);
...
FOR EACH ROW EXECUTE FUNCTION generate_sequence(
    'poll_t', 'sequential_number', 'meeting_id'
);
...
FOR EACH ROW EXECUTE FUNCTION generate_sequence(
    'projector_t', 'sequential_number', 'meeting_id'
);
...
FOR EACH ROW EXECUTE FUNCTION generate_sequence(
    'topic_t', 'sequential_number', 'meeting_id'
);
...
FOR EACH ROW EXECUTE FUNCTION check_not_null_for_1_1(
...
FOR EACH ROW EXECUTE FUNCTION check_not_null_for_1_1(
    'topic', 'agenda_item_id', 'agenda_item', 'content_object_id_topic_id'
);
...
FOR EACH ROW EXECUTE FUNCTION check_not_null_for_1_n(
    'meeting_t',
...
FOR EACH ROW EXECUTE FUNCTION check_not_null_for_n_m(
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'motion', 'content_object_id_motion_id', 'agenda_item_id'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'motion_block', 'content_object_id_motion_block_id', 'agenda_item_id'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'assignment', 'content_object_id_assignment_id', 'agenda_item_id'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'meeting', 'meeting_id', 'assignment_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'assignment', 'assignment_id', 'candidate_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'meeting_user', 'meeting_user_id', 'assignment_candidate_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'meeting', 'meeting_id', 'assignment_candidate_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'meeting', 'meeting_id', 'chat_group_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'meeting', 'default_meeting_id', 'default_meeting_for_committee_id'
);

CREATE TRIGGER tr_log_i_committee_user_ids_from_meeting_user_t BEFORE INSERT ON meeting_user_t
FOR EACH ROW EXECUTE FUNCTION log_iu_modified_calculated_id_array_field(
    'committee',
    '',
...
FOR EACH ROW EXECUTE FUNCTION log_ud_modified_calculated_id_array_field(
    'committee',
    '',
    'SELECT committee_id FROM meeting_t WHERE id = ($1).meeting_id',
    'user_ids',
...
FOR EACH ROW EXECUTE FUNCTION log_iu_modified_calculated_id_array_field(
    'committee', 'committee_id', '', 'user_ids', 'user_id', ''
);
...
FOR EACH ROW EXECUTE FUNCTION log_iu_modified_calculated_id_array_field(
    'committee', 'home_committee_id', '', 'user_ids', 'id', ''
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'organization', 'organization_id', 'committee_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'organization', 'organization_id', 'gender_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'group', 'group_id', 'poll_ids', 'poll', 'poll_id', 'entitled_group_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'meeting', 'used_as_motion_poll_default_id', 'motion_poll_default_group_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'meeting', 'used_as_topic_poll_default_id', 'topic_poll_default_group_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'user', 'model_id_user_id', 'history_entry_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'motion', 'model_id_motion_id', 'history_entry_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'user', 'user_id', 'history_position_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'motion', 'content_object_id_motion_id', 'list_of_speakers_id'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'motion_block', 'content_object_id_motion_block_id', 'list_of_speakers_id'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'assignment', 'content_object_id_assignment_id', 'list_of_speakers_id'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'topic', 'content_object_id_topic_id', 'list_of_speakers_id'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'meeting', 'meeting_id', 'list_of_speakers_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'organization', 'owner_id_organization_id', 'mediafile_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'organization', 'is_active_in_organization_id', 'active_meeting_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'organization', 'is_archived_in_organization_id', 'archived_meeting_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'organization', 'template_for_organization_id', 'template_meeting_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'committee', 'committee_id', 'meeting_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION log_iu_modified_calculated_id_array_field(
    'meeting', 'meeting_id', '', 'user_ids', 'user_id', ''
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'mediafile', 'mediafile_id', 'meeting_mediafile_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'meeting', 'meeting_id', 'meeting_mediafile_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'meeting_user', 'vote_delegated_to_id', 'vote_delegations_from_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'meeting', 'meeting_id', 'motion_block_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'motion', 'motion_id', 'change_recommendation_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'meeting', 'meeting_id', 'motion_change_recommendation_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'meeting', 'meeting_id', 'motion_comment_section_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'motion_state', 'submitter_withdraw_state_id', 'submitter_withdraw_back_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'meeting_user', 'meeting_user_id', 'motion_working_group_speaker_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'motion', 'motion_id', 'working_group_speaker_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'meeting', 'meeting_id', 'motion_working_group_speaker_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'motion', 'content_object_id_motion_id', 'option_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'user', 'content_object_id_user_id', 'option_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'theme', 'theme_id', 'theme_for_organization_id'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'organization', 'organization_id', 'organization_tag_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'meeting_user', 'meeting_user_id', 'personal_note_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'meeting', 'meeting_id', 'point_of_order_category_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'motion', 'content_object_id_motion_id', 'poll_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'assignment', 'content_object_id_assignment_id', 'poll_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'meeting', 'meeting_id', 'poll_candidate_list_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'projector', 'current_projector_id', 'current_projection_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'projector', 'preview_projector_id', 'preview_projection_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'projector', 'history_projector_id', 'history_projection_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'meeting', 'content_object_id_meeting_id', 'projection_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'motion', 'content_object_id_motion_id', 'projection_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'motion_block', 'content_object_id_motion_block_id', 'projection_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'assignment', 'content_object_id_assignment_id', 'projection_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'agenda_item', 'content_object_id_agenda_item_id', 'projection_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'topic', 'content_object_id_topic_id', 'projection_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'poll', 'content_object_id_poll_id', 'projection_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'meeting', 'meeting_id', 'all_projection_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'meeting', 'meeting_id', 'projector_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'meeting', 'meeting_id', 'projector_countdown_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'meeting', 'meeting_id', 'projector_message_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'list_of_speakers', 'list_of_speakers_id', 'speaker_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'meeting_user', 'meeting_user_id', 'speaker_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'meeting', 'meeting_id', 'structure_level_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'meeting', 'meeting_id', 'structure_level_list_of_speakers_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'tag', 'tag_id', 'tagged_ids', 'motion', 'tagged_id_motion_id', 'tag_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'organization', 'organization_id', 'theme_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION log_iu_modified_calculated_id_array_field(
    'user', 'user_id', '', 'committee_ids', 'committee_id', ''
);
...
FOR EACH ROW EXECUTE FUNCTION log_iu_modified_calculated_id_array_field(
    'user', 'id', '', 'committee_ids', 'home_committee_id', ''
);
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models(
    'organization', 'organization_id', 'user_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'agenda_item', 'motion', 'content_object_id_motion_id', 'meeting_id', False
);
CREATE CONSTRAINT TRIGGER equal_meeting_id_on_motion_t_agenda_item_id AFTER INSERT ON motion_t INITIALLY DEFERRED
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'agenda_item', 'motion', 'content_object_id_motion_id', 'meeting_id', True
);
...
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'agenda_item', 'topic', 'content_object_id_topic_id', 'meeting_id', False
);
CREATE CONSTRAINT TRIGGER equal_meeting_id_on_topic_t_agenda_item_id AFTER INSERT ON topic_t INITIALLY DEFERRED
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'agenda_item', 'topic', 'content_object_id_topic_id', 'meeting_id', True
);
...
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'assignment_candidate', 'assignment', 'assignment_id', 'meeting_id', False
);
CREATE CONSTRAINT TRIGGER equal_meeting_id_on_assignment_t_candidate_ids AFTER INSERT ON assignment_t INITIALLY DEFERRED
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'assignment_candidate', 'assignment', 'assignment_id', 'meeting_id', True
);
...
FOR EACH ROW EXECUTE FUNCTION check_equals_multi(
...
FOR EACH ROW EXECUTE FUNCTION check_equals_intermediate(
    'chat_group_id',
    'chat_group',
    'group_id',
    'group',
...
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'chat_message', 'meeting_user', 'meeting_user_id', 'meeting_id', False
);
...
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'chat_message', 'meeting_user', 'meeting_user_id', 'meeting_id', True
);
...
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'chat_message', 'chat_group', 'chat_group_id', 'meeting_id', False
);
...
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'chat_message', 'chat_group', 'chat_group_id', 'meeting_id', True
);
...
FOR EACH ROW EXECUTE FUNCTION check_equals_intermediate(
    'group_id', 'group', 'poll_id', 'poll', 'meeting_id', 'poll_ids'
);
...
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'meeting_user', 'meeting_user', 'vote_delegated_to_id', 'meeting_id', False
);
...
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'meeting_user', 'meeting_user', 'vote_delegated_to_id', 'meeting_id', True
);
...
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'motion', 'motion_state', 'state_id', 'meeting_id', False
);
...
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'motion', 'motion_state', 'state_id', 'meeting_id', True
);
...
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'motion', 'motion_state', 'recommendation_id', 'meeting_id', False
);
...
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'motion', 'motion_state', 'recommendation_id', 'meeting_id', True
);
...
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'motion', 'motion_category', 'category_id', 'meeting_id', False
);
...
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'motion', 'motion_category', 'category_id', 'meeting_id', True
);
...
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'motion', 'motion_block', 'block_id', 'meeting_id', False
);
...
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'motion', 'motion_block', 'block_id', 'meeting_id', True
);
...
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'motion_category', 'motion_category', 'parent_id', 'meeting_id', False
);
...
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'motion_category', 'motion_category', 'parent_id', 'meeting_id', True
);
...
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'motion_change_recommendation', 'motion', 'motion_id', 'meeting_id', False
);
...
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'motion_change_recommendation', 'motion', 'motion_id', 'meeting_id', True
);
...
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'motion_comment', 'motion_comment_section', 'section_id', 'meeting_id', True
);
...
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'motion_editor', 'meeting_user', 'meeting_user_id', 'meeting_id', False
);
...
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'motion_editor', 'meeting_user', 'meeting_user_id', 'meeting_id', True
);
...
CREATE CONSTRAINT TRIGGER equal_workflow_id_on_motion_state_t_submitter_withdraw_state_id AFTER INSERT OR UPDATE OF submitter_withdraw_state_id,
workflow_id ON motion_state_t INITIALLY DEFERRED
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'motion_state',
    'motion_state',
    'submitter_withdraw_state_id',
...
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'motion_state', 'motion_workflow', 'workflow_id', 'meeting_id', False
);
...
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'motion_state', 'motion_workflow', 'workflow_id', 'meeting_id', True
);
...
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'motion_submitter', 'meeting_user', 'meeting_user_id', 'meeting_id', False
);
...
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'motion_submitter', 'meeting_user', 'meeting_user_id', 'meeting_id', True
);
...
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'motion_supporter', 'meeting_user', 'meeting_user_id', 'meeting_id', False
);
...
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'motion_supporter', 'meeting_user', 'meeting_user_id', 'meeting_id', True
);
...
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'motion_workflow', 'motion_state', 'first_state_id', 'meeting_id', False
);
...
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'motion_workflow', 'motion_state', 'first_state_id', 'meeting_id', True
);
...
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'motion_working_group_speaker', 'motion', 'motion_id', 'meeting_id', False
);
...
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'option', 'motion', 'content_object_id_motion_id', 'meeting_id', False
);
CREATE CONSTRAINT TRIGGER equal_meeting_id_on_motion_t_option_ids AFTER INSERT ON motion_t INITIALLY DEFERRED
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'option', 'motion', 'content_object_id_motion_id', 'meeting_id', True
);
...
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'personal_note', 'meeting_user', 'meeting_user_id', 'meeting_id', False
);
...
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'personal_note', 'meeting_user', 'meeting_user_id', 'meeting_id', True
);
...
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'personal_note', 'motion', 'content_object_id_motion_id', 'meeting_id', True
);
...
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'poll', 'motion', 'content_object_id_motion_id', 'meeting_id', False
);
CREATE CONSTRAINT TRIGGER equal_meeting_id_on_motion_t_poll_ids AFTER INSERT ON motion_t INITIALLY DEFERRED
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'poll', 'motion', 'content_object_id_motion_id', 'meeting_id', True
);
...
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'poll', 'assignment', 'content_object_id_assignment_id', 'meeting_id', False
);
CREATE CONSTRAINT TRIGGER equal_meeting_id_on_assignment_t_poll_ids AFTER INSERT ON assignment_t INITIALLY DEFERRED
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'poll', 'assignment', 'content_object_id_assignment_id', 'meeting_id', True
);
...
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'poll', 'topic', 'content_object_id_topic_id', 'meeting_id', False
);
CREATE CONSTRAINT TRIGGER equal_meeting_id_on_topic_t_poll_ids AFTER INSERT ON topic_t INITIALLY DEFERRED
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'poll', 'topic', 'content_object_id_topic_id', 'meeting_id', True
);
...
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'poll', 'option', 'global_option_id', 'meeting_id', False
);
...
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'poll', 'option', 'global_option_id', 'meeting_id', True
);
...
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'projection', 'projector', 'current_projector_id', 'meeting_id', False
);
...
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'projection', 'projector', 'current_projector_id', 'meeting_id', True
);
...
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'projection', 'projector', 'preview_projector_id', 'meeting_id', False
);
...
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'projection', 'projector', 'preview_projector_id', 'meeting_id', True
);
...
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'projection', 'projector', 'history_projector_id', 'meeting_id', False
);
...
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'projection', 'projector', 'history_projector_id', 'meeting_id', True
);
...
FOR EACH ROW EXECUTE FUNCTION check_equals_meeting_id_for_meeting(
    'projection', 'content_object_id_meeting_id'
);
...
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'projection', 'motion', 'content_object_id_motion_id', 'meeting_id', False
);
CREATE CONSTRAINT TRIGGER equal_meeting_id_on_motion_t_projection_ids AFTER INSERT ON motion_t INITIALLY DEFERRED
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'projection', 'motion', 'content_object_id_motion_id', 'meeting_id', True
);
...
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'projection', 'topic', 'content_object_id_topic_id', 'meeting_id', False
);
CREATE CONSTRAINT TRIGGER equal_meeting_id_on_topic_t_projection_ids AFTER INSERT ON topic_t INITIALLY DEFERRED
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'projection', 'topic', 'content_object_id_topic_id', 'meeting_id', True
);
...
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'projection', 'poll', 'content_object_id_poll_id', 'meeting_id', False
);
CREATE CONSTRAINT TRIGGER equal_meeting_id_on_poll_t_projection_ids AFTER INSERT ON poll_t INITIALLY DEFERRED
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'projection', 'poll', 'content_object_id_poll_id', 'meeting_id', True
);
...
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'speaker', 'list_of_speakers', 'list_of_speakers_id', 'meeting_id', False
);
...
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'speaker', 'list_of_speakers', 'list_of_speakers_id', 'meeting_id', True
);
...
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'speaker', 'meeting_user', 'meeting_user_id', 'meeting_id', False
);
...
FOR EACH ROW EXECUTE FUNCTION check_equals(
    'speaker', 'meeting_user', 'meeting_user_id', 'meeting_id', True
);
