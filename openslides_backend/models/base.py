    ...
    def get_required_fields(self) -> Iterable[fields.Field]:
        ...
        for model_field in self.get_fields():
            if model_field.required and isinstance(model_field, fields.Field):
                if isinstance(model_field, fields.RelationListField) or isinstance(model_field, fields.GenericRelationListField):
                    raise NotImplementedError(f"NotImplementedError: {self.collection.collection}.{model_field.own_field_name}")
                yield model_field
